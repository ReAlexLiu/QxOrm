...
QxSingletonX::QxSingletonX() : QxSingleton<QxSingletonX>(QStringLiteral("qx::QxSingletonX"))
{
#if _QX_USE_QX_SINGLETON_X
    int iResult = std::atexit(&QxSingletonX::deleteAllSingleton);
    Q_UNUSED(iResult);
    QString sAssertMsg = QStringLiteral("cannot register 'qx::QxSingletonX::deleteAllSingleton()' function at exit program (using 'std::atexit')"); 
    Q_UNUSED(sAssertMsg);
    qAssertMsg((iResult == 0), qPrintable(sAssertMsg));
...
bool QxSingletonX::addSingleton(const QString& sKey, IxSingleton* pSingleton)
{
    ...
    qAssertMsg((!bExist && !sKey.isEmpty()), qPrintable(sAssertMsg));
    if (!pSingleton || bExist || sKey.isEmpty())
    {
        return false;
    }
...
bool QxSingletonX::removeSingleton(const QString& sKey)
{
    ...
    QMutex* pMutex = (QCoreApplication::instance() ? getMutexSingletonX() : NULL);
    QMutexLocker locker(pMutex);
    QString sAssertMsg = QStringLiteral("singleton key '%1' doesn't exist in the singleton manager").arg(sKey); 
    Q_UNUSED(sAssertMsg);
    qAssertMsg((getMapSingletonX().contains(sKey)), qPrintable(sAssertMsg));
...
#if _QX_USE_QX_SINGLETON_X
#ifdef _QX_TRACE_CONSTRUCTOR_DESTRUCTOR
    xdebug("execute deleteAllSingleton() function");
#endif // _QX_TRACE_CONSTRUCTOR_DESTRUCTOR
    QMutex* pMutex = (QCoreApplication::instance() ? getMutexSingletonX() : NULL);
    QMutexLocker locker(pMutex);
    bool& bOnClearSingletonX = getOnClearSingletonX();
