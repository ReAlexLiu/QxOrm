...
QxSqlQuery::QxSqlQuery() : m_iSqlElementIndex(0), m_iParenthesisCount(0), m_bDistinct(false)
{
    m_sQuery << QString();
}
...
QxSqlQuery::QxSqlQuery(const char* query, const QVariantList& values /* = QVariantList() */) : m_iSqlElementIndex(0), m_iParenthesisCount(0), m_bDistinct(false)
{
    if (values.count() <= 0)
    {
        m_sQuery << QLatin1String(query);
    }
    else
    {
        addFreeText(QLatin1String(query), values);
    }
}
...
QxSqlQuery::QxSqlQuery(const QString& type, const QString& query) : m_iSqlElementIndex(0), m_iParenthesisCount(0), m_bDistinct(false), m_sType(type)
{
    m_sQuery << query;
}
...
{
    QJsonObject obj; for (std::initializer_list<QPair<QString, QJsonValue> >::const_iterator itr = json.begin(); itr != json.end(); ++itr)
    {
        obj.insert(itr->first, itr->second);
    }
...
{
    QJsonObject obj1; for (std::initializer_list<QPair<QString, QJsonValue> >::const_iterator itr = json.begin(); itr != json.end(); ++itr)
    {
        obj1.insert(itr->first, itr->second);
    }
    QJsonObject obj2; for (std::initializer_list<QPair<QString, QJsonValue> >::const_iterator itr = opts.begin(); itr != opts.end(); ++itr)
    {
        obj2.insert(itr->first, itr->second);
    }
...
void QxSqlQuery::verifyQuery() const
{
    ...
    if (queryAt(0).isEmpty() || (m_lstSqlElement.count() <= 0))
    {
        return;
    }
    xwarn("invalid SQL query, you cannot mix classic SQL and C++ syntax");
    qAssert(false);
...
QString QxSqlQuery::query()
{
    ...
    {
        sQuery += m_lstSqlElement.at(i)->toString() + QStringLiteral(" ");
    }
    return sQuery;
}
...
QxSqlQuery& QxSqlQuery::bind(const QString& sKey, const QVariant& vValue, QSql::ParamType paramType /* = QSql::In */)
{
    ...
    if (sKey.isEmpty() || m_lstValue.exist(sKey))
    {
        qAssert(false); return (*this);
    }
    if (!queryAt(0).contains(sKey))
    {
        qAssert(false); return (*this);
    }
    m_lstValue.insert(sKey, type_bind_value(vValue, paramType));
...
{
    if ((iPosition < 0) || (iPosition >= m_lstValue.count()))
    {
        qAssert(false); return QVariant();
    }
...
void QxSqlQuery::resolve(QSqlQuery& query) const
{
    ...
    while (itr.next())
    {
        if (bKey)
        {
            query.bindValue(itr.key(), std::get<0>(itr.value()), std::get<1>(itr.value()));
        }
        else
        {
            query.addBindValue(std::get<0>(itr.value()), std::get<1>(itr.value()));
        }
    }
}
...
void QxSqlQuery::resolveOutput(QSqlQuery& query, bool bFetchSqlResult)
{
    ...
    for (long l = 0; l < m_lstValue.count(); l++)
    {
        ...
        {
            lst.insert(key, val); continue;
        }
        if (bKey)
        {
            outputValue = query.boundValue(key);
        }
        else
        {
            outputValue = query.boundValue(l);
        }
...
QString QxSqlQuery::getJoinQueryHash()
{
    ...
    while (itr.hasNext())
    {
        ...
        hash += QStringLiteral("|%1|%2").arg(itr.key()).arg(tmp.query());
    }
    return hash;
}
...
{
    QString sBoundValues = QStringLiteral("");
    ...
    if (qx::QxSqlDatabase::getSingleton()->getSqlPlaceHolderStyle() == qx::QxSqlDatabase::ph_style_question_mark)
    {
        ...
        {
            sBoundValues += QStringLiteral("\n  - position '%1' : %2").arg(i).arg(lst.at(i).toString());
        }
    }
    else
    {
        ...
        {
            itr.next();
            sBoundValues += QStringLiteral("\n  - %1 : %2").arg(itr.key()).arg(itr.value().toString());
        }
    }

    if (!sBoundValues.isEmpty())
    {
        xinfo("dump sql query bound values : {}", qPrintable(sBoundValues));
    }
}
...
void QxSqlQuery::fetchSqlResult(QSqlQuery& query)
{
    ...
    if (query.size() > 0)
    {
        m_pSqlResult->values.reserve(query.size());
    }
    while (query.next())
    {
        if (bCheckRecord)
...
QVariant QxSqlQuery::getSqlResultAt(long row, long column) const
{
    ...
    if ((row < 0) || (row >= m_pSqlResult->values.count()))
    {
        return QVariant();
    }
    if ((column < 0) || (column >= m_pSqlResult->positionByKey.count()))
    {
        return QVariant();
    }
...
QVector<QString> QxSqlQuery::getSqlResultAllColumns() const
{
    ...
    while (itr.hasNext())
    {
        itr.next(); lstAllColumns[itr.value()] = itr.key();
    }
...
void QxSqlQuery::dumpSqlResult()
{
    ...
    xinfo("start dump sql result : '{}'", qPrintable(sql));
    QVector<QString> lstColumns = this->getSqlResultAllColumns();
    for (long i = 0; i < lstColumns.count(); i++)
    {
        sColumns += QStringLiteral("|") + lstColumns.at(i);
    }
    xinfo(qPrintable(sColumns));
    for (long j = 0; j < m_pSqlResult->values.count(); j++)
    {
        sOutput = QString::number(j);
        for (long k = 0; k < m_pSqlResult->positionByKey.count(); k++)
        {
            sOutput += QStringLiteral("|") + m_pSqlResult->values.at(j).at(k).toString();
        }
        xinfo(qPrintable(sOutput));
    }
    xinfo("end dump sql result : '{}'", qPrintable(sql));
}
...
{
    if (query.m_lstSqlElement.count() <= 0)
    {
        addSqlIn(QVariantList() << QVariant(query.queryAt(0)), qx::dao::detail::QxSqlIn::_in_select);
    }
    else
    {
        addEmbedQuery(query, qx::dao::detail::QxSqlEmbedQuery::_in, true);
    }
    return (*this);
}
...
QxSqlQuery& QxSqlQuery::notIn_Select(const QxSqlQuery& query)
{
    if (query.m_lstSqlElement.count() <= 0)
    {
        addSqlIn(QVariantList() << QVariant(query.queryAt(0)), qx::dao::detail::QxSqlIn::_not_in_select);
    }
    else
    {
        addEmbedQuery(query, qx::dao::detail::QxSqlEmbedQuery::_not_in, true);
    }
    return (*this);
}
...
QxSqlQuery& QxSqlQuery::isEqualTo_Select(const QxSqlQuery& query)
{
    if (query.m_lstSqlElement.count() <= 0)
    {
        addSqlCompare(query.queryAt(0), qx::dao::detail::QxSqlCompare::_is_equal_to_select);
    }
    else
    {
        addEmbedQuery(query, qx::dao::detail::QxSqlEmbedQuery::_is_equal_to, true);
    }
    return (*this);
}
...
QxSqlQuery& QxSqlQuery::isNotEqualTo_Select(const QxSqlQuery& query)
{
    if (query.m_lstSqlElement.count() <= 0)
    {
        addSqlCompare(query.queryAt(0), qx::dao::detail::QxSqlCompare::_is_not_equal_to_select);
    }
    else
    {
        addEmbedQuery(query, qx::dao::detail::QxSqlEmbedQuery::_is_not_equal_to, true);
    }
    return (*this);
}
...
QxSqlQuery& QxSqlQuery::addSqlCompare(const QVariant& val, qx::dao::detail::QxSqlCompare::type type, const QString& sCustomOperator /* = QString() */)
{
    ...
    {
        xwarn("invalid SQL query, need a column name"); qAssert(false); return (*this);
    }
...
QxSqlQuery& QxSqlQuery::addSqlIsBetween(const QVariant& val1, const QVariant& val2, qx::dao::detail::QxSqlIsBetween::type type)
{
    if (!m_pSqlElementTemp)
    {
        xwarn("invalid SQL query, need a column name");
        qAssert(false);
        return (*this);
    }
...
{
    return qx::dao::helper::call_query_helper(query, pDatabase, true);
}

QSqlError call_query_without_prepare(qx::QxSqlQuery& query, QSqlDatabase* pDatabase /* = NULL */)
{
    return qx::dao::helper::call_query_helper(query, pDatabase, false);
}
...
QSqlError call_query_helper(qx::QxSqlQuery& query, QSqlDatabase* pDatabase, bool bPrepare)
{
    ...
    if (qx::QxSqlDatabase::getSingleton()->getDriverName() == QStringLiteral("QXMONGODB"))
    {
    ...
    do
    {
        if (bPrepare && !q.prepare(sql))
        {
            dbError = q.lastError(); break;
        }
        query.resolve(q);
        if (bPrepare)
        {
    ...
    if (dbError.isValid())
    {
        QString log = QStringLiteral("custom sql query failed (%1 ms) : %2").arg(ms).arg(sql);
        xinfo(qPrintable(log));
        QString ierr = dbError.nativeErrorCode();
        QString tmp = dbError.driverText();
        xinfo("Database error number '{}' : {}", qPrintable(ierr), qPrintable(tmp));
        tmp = dbError.databaseText();
        xinfo(qPrintable(tmp));
    }
    else if (qx::QxSqlDatabase::getSingleton()->getTraceSqlQuery())
    {
        ...
        {
            QString log = QStringLiteral("custom sql query (%1 ms) : %2").arg(ms).arg(sql);
            xinfo(qPrintable(log));
        }
    }

    if ((dbError.isValid() && bBoundValuesOnError) || (bBoundValues))
    {
        qx::QxSqlQuery::dumpBoundValues(q);
    }
    return dbError;
}
...
inline void qx_save(Archive& ar, const qx::QxSqlQuery& t, const unsigned int file_version)
{
    ...
    Q_FOREACH(qx::dao::detail::IxSqlElement_ptr pSqlElement, t.m_lstSqlElement)
    {
        if (!pSqlElement)
        {
            ar << boost::serialization::make_nvp("sql_element_type", eNoSqlType); continue;
        }
...
inline void qx_load(Archive& ar, qx::QxSqlQuery& t, const unsigned int file_version)
{
    ...
    for (long l = 0; l < lSqlElementCount; l++)
    {
        ...
        if (eTypeSqlElement != qx::dao::detail::IxSqlElement::_no_type)
        {
            pSqlElement = qx::dao::detail::create_sql_element(eTypeSqlElement);
            qAssert(pSqlElement);
            if (pSqlElement)
            {
                pSqlElement->qxLoad(ar);
            }
        }
...
QDataStream& operator<< (QDataStream& stream, const qx::QxSqlQuery& t)
{
    ...
    Q_FOREACH(qx::dao::detail::IxSqlElement_ptr pSqlElement, t.m_lstSqlElement)
    {
        if (!pSqlElement)
        {
            stream << (qint32)(eNoSqlType); continue;
        }
        qx::dao::detail::IxSqlElement::type_class eTypeSqlElement = pSqlElement->getTypeClass();
        stream << (qint32)(eTypeSqlElement);
        stream << (*pSqlElement);
