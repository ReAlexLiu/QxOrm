...
QString QxSqlSort::toString() const
{
   ...
   switch (m_type)
   {
      case _order_asc:     sReturn = QStringLiteral("ORDER BY ");     break;
      case _order_desc:    sReturn = QStringLiteral("ORDER BY ");     break;
      case _group_by:      sReturn = QStringLiteral("GROUP BY ");     break;
      default:             qAssert(false);
   }

   for (int i = 0; i < m_lstColumns.count(); i++)
   {
      sReturn += ((i == 0) ? QStringLiteral("") : QStringLiteral(", "));
      ...
      {
         case _order_asc:   
             sReturn += sColumn + QStringLiteral(" ASC");  
             break;
         case _order_desc: 
             sReturn += sColumn + QStringLiteral(" DESC"); 
             break;
         case _group_by:  
             sReturn += sColumn;       
             break;
