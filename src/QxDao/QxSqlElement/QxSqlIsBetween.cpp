...
QString QxSqlIsBetween::toString() const
{
   ...
   QString sKey1(bQuestionMark ? QStringLiteral("?") : (sKey + QStringLiteral("_1"))), sKey2(bQuestionMark ? QStringLiteral("?") : (sKey + QStringLiteral("_2")));

   switch (m_type)
   {
      case _is_between:       
          sReturn = QStringLiteral("%1 BETWEEN %2 AND %3").arg(sColumn).arg(sKey1).arg(sKey2);
          break;
      case _is_not_between:  
          sReturn = QStringLiteral("%1 NOT BETWEEN %2 AND %3").arg(sColumn).arg(sKey1).arg(sKey2);
          break;
      default:               
          qAssert(false);
   }
...
void QxSqlIsBetween::resolve(QSqlQuery & query) const
{
   ...
   bool bQuestionMark = (qx::QxSqlDatabase::getSingleton()->getSqlPlaceHolderStyle() == qx::QxSqlDatabase::ph_style_question_mark);
   QString sKey1(bQuestionMark ? QStringLiteral("?") : (m_lstKeys.at(0) + QStringLiteral("_1"))), sKey2(bQuestionMark ? QStringLiteral("?") : (m_lstKeys.at(0) + QStringLiteral("_2")));
