...
QString QxSqlEmbedQuery::toString() const
{
   ...
   switch (m_pImpl->m_type)
   {
      case _none: 
          ...
          break;
      case _in:  
          result = QStringLiteral("%1 IN (%2)").arg(column).arg(sql);       
          break;
      case _not_in: 
          result = QStringLiteral("%1 NOT IN (%2)").arg(column).arg(sql);   
          break;
      case _is_equal_to:   
          result = QStringLiteral("%1 = (%2)").arg(column).arg(sql);   
          break;
...
{
#ifndef _QX_NO_JSON
   return QStringLiteral("%1|%2").arg(static_cast<int>(m_pImpl->m_type)).arg(qx::serialization::json::to_string(m_pImpl->m_query));
#else // _QX_NO_JSON
   return QString();
#endif // _QX_NO_JSON
}
