...
IxSqlElement::type_class QxSqlLimit::getTypeClass() const
{
    return IxSqlElement::_sql_limit;
}
...
QString QxSqlLimit::getStartRow_ParamKey() const
{
    ...
    sStartRow += QStringLiteral("_%1_0").arg(m_iIndex);
    if (qx::QxSqlDatabase::getSingleton()->getSqlPlaceHolderStyle() == qx::QxSqlDatabase::ph_style_question_mark)
    {
        sStartRow = QStringLiteral("?");
    }
    else if (qx::QxSqlDatabase::getSingleton()->getSqlPlaceHolderStyle() == qx::QxSqlDatabase::ph_style_at_name)
    {
        sStartRow = QStringLiteral("@") + sStartRow;
    }
    else
    {
        sStartRow = QStringLiteral(":") + sStartRow;
...
QString QxSqlLimit::getRowsCount_ParamKey() const
{
    ...
    sRowsCount += QStringLiteral("_%1_0").arg(m_iIndex);
    if (qx::QxSqlDatabase::getSingleton()->getSqlPlaceHolderStyle() == qx::QxSqlDatabase::ph_style_question_mark)
    {
    ...
    {
        sRowsCount = QStringLiteral("@") + sRowsCount;
    }
    else
    {
        sRowsCount = QStringLiteral(":") + sRowsCount;
...
QString QxSqlLimit::getMaxRow_ParamKey() const
{
    ...
    sMaxRow += QStringLiteral("_%1_0").arg(m_iIndex);
    if (qx::QxSqlDatabase::getSingleton()->getSqlPlaceHolderStyle() == qx::QxSqlDatabase::ph_style_question_mark)
    {
    ...
    {
        sMaxRow = QStringLiteral("@") + sMaxRow;
    }
    else
    {
        sMaxRow = QStringLiteral(":") + sMaxRow;
...
void QxSqlLimit::setExtraSettings(const QString& s)
{
    Q_UNUSED(s);
}
