...
QString QxSqlIn::toString() const
{
   ...
   {
      case _in:      
          sReturn = sColumn + QStringLiteral(" IN (");     
          break;
      case _not_in:  
          sReturn = sColumn + QStringLiteral(" NOT IN (");
          break;
      case _in_select: 
          sReturn = sColumn + QStringLiteral(" IN (");  
          break;
   ...
   for (int i = 0; i < m_lstValues.count(); i++)
   {
      ...
      QString sCurrKey = (bQuestionMark ? QStringLiteral("?") : QStringLiteral("%1_%2").arg(sKey).arg(i));
