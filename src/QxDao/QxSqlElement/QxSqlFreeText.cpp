...
void QxSqlFreeText::resolve(QSqlQuery & query) const
{
   ...
   QString toFind = ((phStyle == qx::QxSqlDatabase::ph_style_2_point_name) ? QStringLiteral(":") : QStringLiteral("@"));
   ...
   for (int i = 0; i < m_lstValues.count(); i++)
   {
      ...
      else
      {
         ...
         posEnd = txt.indexOf(QStringLiteral(" "), (posBegin + 1));
         if ((posEnd == -1) && (i == (m_lstValues.count() - 1))) { posEnd = txt.size(); }
         if ((posBegin == -1) || (posEnd == -1) || (posEnd <= posBegin)) { qAssert(false); break; }
         QString key = txt.mid(posBegin, (posEnd - posBegin)).replace(QStringLiteral(")"), QStringLiteral(""));
