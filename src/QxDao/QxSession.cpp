...
QxSession::QxSession() : m_bTransaction(false), m_bThrowInEvent(false), m_bAutoOpenClose(false)
{
    ...
    if (qx::QxSqlDatabase::getSingleton()->getSessionAutoTransaction())
    {
        open();
    }
}
...
bool QxSession::open()
{
    ...
    {
        m_bAutoOpenClose = m_database.open(); if (!m_bAutoOpenClose)
        {
            appendSqlError(m_database.lastError()); return false;
        }
    }
...
{
    bool bCloseOk = true;
    if (m_bTransaction && isValid())
    {
        bCloseOk = commit();
    }
    else if (m_bTransaction)
    {
        bCloseOk = rollback();
    }
    if (m_bAutoOpenClose)
    {
        m_database.close(); m_bAutoOpenClose = false;
    }
    return bCloseOk;
}
...
bool QxSession::commit()
{
    if (m_bTransaction && !isValid())
    {
        xwarn("qx::QxSession is not valid and 'commit()' method is called");
    }
    if (!m_bTransaction)
    {
        clear(); return false;
...
void QxSession::appendSqlError(const QSqlError& err)
{
    ...
    if (m_bThrowable)
    {
        xerror(qPrintable(err.text()));
    }
    if (m_bThrowable)
    {
        m_bThrowInEvent = true; throw qx::dao::sql_error(err);
    }
