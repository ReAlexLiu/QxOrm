...
qx::IxCollection_ptr IxRepository::_fetchAll(const QString & repositoryKey, const QStringList & columns /* = QStringList() */, const QStringList & relation /* = QStringList() */)
{
   ...
   if (! pRepository) { throw qx::dao::sql_error(QSqlError(QStringLiteral("qx::IxRepository::_fetchAll() : 'invalid repository key, unable to get repository pointer'"), QStringLiteral(""), QSqlError::UnknownError)); }
   qx::IxCollection_ptr lst = pRepository->_newCollection();
   if (! lst) { throw qx::dao::sql_error(QSqlError(QStringLiteral("qx::IxRepository::_fetchAll() : 'unable to create a new collection from repository'"), QStringLiteral(""), QSqlError::UnknownError)); }
...
{
   IxRepository * pRepository = QxRepositoryX::get(repositoryKey);
   if (! pRepository) { throw qx::dao::sql_error(QSqlError(QStringLiteral("qx::IxRepository::_fetchByQuery() : 'invalid repository key, unable to get repository pointer'"), QStringLiteral(""), QSqlError::UnknownError)); }
   qx::IxCollection_ptr lst = pRepository->_newCollection();
   if (! lst) { throw qx::dao::sql_error(QSqlError(QStringLiteral("qx::IxRepository::_fetchByQuery() : 'unable to create a new collection from repository'"), QStringLiteral(""), QSqlError::UnknownError)); }
