...
struct QxSqlDatabase::QxSqlDatabaseImpl
{
    ...
    bool isValid() const
    {
        return (m_pParent ? (!m_pParent->getDriverName().isEmpty() && !m_pParent->getDatabaseName().isEmpty()) : (!m_sDriverName.isEmpty() && !m_sDatabaseName.isEmpty()));
    }
    QString computeDatabaseKey(QSqlDatabase* p) const
    {
        return (p ? (p->driverName() + p->hostName() + p->databaseName()) : (m_sDriverName + m_sHostName + m_sDatabaseName));
    }
    ...
    bool setSetting(const QString& key, const QVariant& val, bool bJustForCurrentThread, QSqlDatabase* pJustForThisDatabase)
    {
        ...
        if (pJustForThisDatabase != NULL)
        {
            ...
            {
                xinfo("database parameters are empty ==> cannot add setting database '{}'", qPrintable(key)); qAssert(false); return false;
            }
            QPair<QString, QString> pairDatabase = qMakePair(sDatabaseKey, key);
...
QxSqlDatabase::QxSqlDatabase() : QxSingleton<QxSqlDatabase>(QStringLiteral("qx::QxSqlDatabase")), m_pImpl(new QxSqlDatabaseImpl(this))
{}
...
{
    if ((m_pImpl->m_lstSettingsByThread.count() <= 0) && (m_pImpl->m_lstSettingsByDatabase.count() <= 0))
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("DriverName"), s, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("ConnectOptions"), s, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("DatabaseName"), s, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("UserName"), s, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("Password"), s, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("HostName"), s, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("Port"), i, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
        m_pImpl->m_iPort = i;
    }
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("TraceSqlQuery"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("TraceSqlRecord"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("TraceSqlBoundValues"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("TraceSqlBoundValuesOnError"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
void QxSqlDatabase::setSqlPlaceHolderStyle(QxSqlDatabase::ph_style e, bool bJustForCurrentThread /* = false */, QSqlDatabase* pJustForThisDatabase /* = NULL */)
{
    ...
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("SqlPlaceHolderStyle"), i, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("SessionThrowable"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("SessionAutoTransaction"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("ValidatorThrowable"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
void QxSqlDatabase::setSqlGenerator(qx::dao::detail::IxSqlGenerator_ptr p, bool bJustForCurrentThread /* = false */, QSqlDatabase* pJustForThisDatabase /* = NULL */)
{
    ...
    if (p && (pJustForThisDatabase != NULL))
    {
        ...
        if (sDatabaseKey.isEmpty())
        {
            xinfo("database parameters are empty ==> cannot add setting database 'SqlGenerator'"); qAssert(false); return;
        }
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("AutoReplaceSqlAliasIntoQuery"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("VerifyOffsetRelation"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("AddAutoIncrementIdToUpdateQuery"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("ForceParentIdToAllChildren"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("AddSqlSquareBracketsForTableName"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("AddSqlSquareBracketsForColumnName"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("FormatSqlQueryBeforeLogging"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("SqlDelimiterForTableName"), lst, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("SqlDelimiterForColumnName"), lst, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("TraceSqlOnlySlowQueriesDatabase"), i, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("TraceSqlOnlySlowQueriesTotal"), i, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    bool bUpdateGlobal = m_pImpl->setSetting(QStringLiteral("DisplayTimerDetails"), b, bJustForCurrentThread, pJustForThisDatabase);
    if (bUpdateGlobal)
    {
...
{
    return QxSqlDatabase::getSingleton()->m_pImpl->getDatabaseByCurrThreadId(dbError);
}
...
QSqlDatabase QxSqlDatabase::QxSqlDatabaseImpl::getDatabaseByCurrThreadId(QSqlError& dbError)
{
    ...
    if (!isValid())
    {
        xwarn("parameters are not valid");
        dbError = QSqlError(QStringLiteral("qx::QxSqlDatabase : 'parameters are not valid'"), QStringLiteral(""), QSqlError::UnknownError);
    ...
    if (!lCurrThreadId)
    {
        xwarn("unable to find current thread id");
        dbError = QSqlError(QStringLiteral("qx::QxSqlDatabase : 'unable to find current thread id'"), QStringLiteral(""), QSqlError::UnknownError);
        qAssert(false); return QSqlDatabase();
    }
...
QSqlDatabase QxSqlDatabase::QxSqlDatabaseImpl::createDatabase(QSqlError& dbError)
{
...

    {
        ...
        if (!db.open())
        {
            displayLastError(db, QStringLiteral("unable to open connection to database")); bError = true;
        }
        if (bError)
        {
        ...
        {
            dbError = QSqlError(QStringLiteral("qx::QxSqlDatabase : 'unable to open connection to database'"), QStringLiteral(""), QSqlError::UnknownError);
        }
    }

    if (bError)
    {
        QSqlDatabase::removeDatabase(sDbKeyNew); return QSqlDatabase();
    }
    m_lstDbByThread.insert(lCurrThreadId, sDbKeyNew);
    xwarn("create new database connection in thread '{}' with key '{}'", qPrintable(sCurrThreadId), qPrintable(sDbKeyNew));
...
void QxSqlDatabase::QxSqlDatabaseImpl::displayLastError(const QSqlDatabase& db, const QString& sDesc) const
{
    ...
    if (sDesc.isEmpty())
    {
        xwarn(qPrintable(sLastError));
    }
    else
    {
        xwarn("'{}'\n{}", qPrintable(sDesc), qPrintable(sLastError));
    }
}
...
{
    QString sLastError;
    if (!db.lastError().nativeErrorCode().isEmpty())
    {
        sLastError += QStringLiteral("Error number '%1' : ").arg(db.lastError().nativeErrorCode());
    }
...
qx::dao::detail::IxSqlGenerator* QxSqlDatabase::getSqlGenerator()
{
    ...
    if (m_pImpl->m_sDriverName == QStringLiteral("QMYSQL"))
    {
        m_pImpl->m_pSqlGenerator = std::make_shared<qx::dao::detail::QxSqlGenerator_MySQL>();
    }
    else if (m_pImpl->m_sDriverName == QStringLiteral("QPSQL"))
    {
        m_pImpl->m_pSqlGenerator = std::make_shared<qx::dao::detail::QxSqlGenerator_PostgreSQL>();
    }
    else if (m_pImpl->m_sDriverName == QStringLiteral("QSQLITE"))
    {
        m_pImpl->m_pSqlGenerator = std::make_shared<qx::dao::detail::QxSqlGenerator_SQLite>();
    }
    else if (m_pImpl->m_sDriverName == QStringLiteral("QOCI"))
    {
        m_pImpl->m_pSqlGenerator = std::make_shared<qx::dao::detail::QxSqlGenerator_Oracle>();
    }

    if (!m_pImpl->m_pSqlGenerator)
    {
        m_pImpl->m_pSqlGenerator = std::make_shared<qx::dao::detail::QxSqlGenerator_Standard>();
    }
...
void QxSqlDatabase::clearAllDatabases()
{
    ...
    pSingleton->m_pImpl->m_sDriverName = QStringLiteral("");
    pSingleton->m_pImpl->m_sConnectOptions = QStringLiteral("");
    pSingleton->m_pImpl->m_sDatabaseName = QStringLiteral("");
    pSingleton->m_pImpl->m_sUserName = QStringLiteral("");
    pSingleton->m_pImpl->m_sPassword = QStringLiteral("");
    pSingleton->m_pImpl->m_sHostName = QStringLiteral("");
...
void QxSqlDatabase::clearAllSettingsForCurrentThread()
{
    ...
    while (itr.hasNext())
    {
        itr.next(); if (itr.key().first == lCurrThreadId)
        {
            itr.remove();
        }
    }
...
void QxSqlDatabase::clearAllSettingsForDatabase(QSqlDatabase* p)
{
    ...
    while (itr.hasNext())
    {
        itr.next(); if (itr.key().first == sDatabaseKey)
        {
            itr.remove();
        }
    }
    ...
    while (itr2.hasNext())
    {
        itr2.next(); if (itr2.key() == sDatabaseKey)
        {
            itr2.remove();
        }
    }
