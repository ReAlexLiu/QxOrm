...
struct QxSqlRelationLinked::QxSqlRelationLinkedImpl
{
    ...
    enum e_hierarchy_action
    {
        e_hierarchy_select, e_hierarchy_from, e_hierarchy_join, e_hierarchy_where_soft_delete
    };
    ...
    {
        return (m_lstRootColumns.isEmpty() || (m_bRootColumnsModeRemove ? (!m_lstRootColumns.contains(s)) : m_lstRootColumns.contains(s)));
    }
    ...
    void hierarchyAction(QxSqlRelationParams& params, e_hierarchy_action action)
    {
        if (m_bRoot)
        {
            params.setIndex(0); params.setIndexOwner(0); params.setCustomAlias(m_sRootCustomAlias); params.setCustomAliasOwner(m_sRootCustomAlias);
        }
        IxSqlRelationX* pAllRelationX = (m_allRelationXOrdered ? m_allRelationXOrdered.get() : m_allRelationX); if (!pAllRelationX)
        {
            qAssert(false); return;
        }
        for (auto itr = pAllRelationX->begin(); itr != pAllRelationX->end(); ++itr)
        {
            IxSqlRelation* p = itr->second; if (!p)
            {
                continue;
            }
            ...
            QxSqlRelationLinked_ptr pRelationLinked = m_relationLinkedX.value(p->getKey()); if (!pRelationLinked)
            {
                continue;
            }
    ...
    static std::shared_ptr<QxSqlRelationLinked> getRelationLinkedSaved(const QPair<IxClass*, QByteArray>& key)
    {
        QMutexLocker locker(&m_mutex); return m_relationLinkedSaved.value(key);
    }
    static void insertRelationLinkedSaved(const QPair<IxClass*, QByteArray>& key, std::shared_ptr<QxSqlRelationLinked> ptr)
    {
        QMutexLocker locker(&m_mutex); m_relationLinkedSaved.insert(key, ptr);
    }
...
bool QxSqlRelationLinked::isRoot() const
{
    return m_pImpl->m_bRoot;
}

bool QxSqlRelationLinked::checkRootColumns(const QString& s) const
{
    return m_pImpl->checkRootColumns(s);
}

long QxSqlRelationLinked::getRootColumnsCount() const
{
    return m_pImpl->m_lstRootColumns.count();
}

long QxSqlRelationLinked::getRootColumnsOffset() const
{
    return m_pImpl->m_lRootColumnsOffset;
}

void QxSqlRelationLinked::setRootColumnsOffset(long l)
{
    m_pImpl->m_lRootColumnsOffset = l;
}

QString QxSqlRelationLinked::getRootCustomAlias() const
{
    return m_pImpl->m_sRootCustomAlias;
}

QxSqlRelationLinked::type_lst_relation_linked QxSqlRelationLinked::getRelationLinkedX() const
{
    return m_pImpl->m_relationLinkedX;
}

QxSqlRelationLinked::type_lst_relation QxSqlRelationLinked::getRelationX() const
{
    return m_pImpl->m_relationX;
}
...
std::shared_ptr<QxSqlRelationLinked> QxSqlRelationLinked::getHierarchy(IxClass* pClass, const QStringList& sRelationX, qx_bool& bOk, qx::dao::detail::IxDao_Helper* pDaoHelper /* = NULL */)
{
    if (!pClass)
    {
        qAssert(false); bOk = qx_bool(false, QStringLiteral("class is empty")); return std::shared_ptr<QxSqlRelationLinked>();
    }
    QByteArray hash = sRelationX.join(QStringLiteral("|")).toLatin1();
    ...
    if (ptr)
    {
        (*result->m_pImpl) = (*ptr->m_pImpl); return result;
    }
    ...
    if (!bOk)
    {
        return std::shared_ptr<QxSqlRelationLinked>();
    }
    if (!bHasJoinSubQuery)
    {
...
qx_bool QxSqlRelationLinked::QxSqlRelationLinkedImpl::buildHierarchy(IxSqlRelationX* pRelationX, const QStringList& sRelationX, qx::dao::detail::IxDao_Helper* pDaoHelper)
{
    if (!pRelationX)
    {
        qAssert(false); return qx_bool(false);
    }
    if (m_bRoot)
    {
        m_relationLinkedX.clear(); m_relationX.clear();
    }
    ...
    Q_FOREACH(QString sRelation, sRelationX)
    {
        ...
        if (sRelation.left(iJoin) == QX_LEFT_OUTER_JOIN)
        {
            eJoinType = qx::dao::sql_join::left_outer_join;
        }
        else if (sRelation.left(iJoin) == QX_INNER_JOIN)
        {
            eJoinType = qx::dao::sql_join::inner_join;
        }
        ...
        {
            iPos = ((iPosLeftOuter < iPosInner) ? iPosLeftOuter : iPosInner);
        }

        if (iPos >= 0)
        {
            sKey = sRelation.mid(iOffset, (iPos - iOffset));
        }
        else
        {
            sKey = sRelation.mid(iOffset);
        }
        ...
        if (sKey.startsWith(QStringLiteral("*")))
        {
    ...
    if (bRelationXOrdered && m_allRelationXOrdered)
    {
        ...
        while (itr.next())
        {
            QString sKey = itr.key(); if (sKey.isEmpty())
            {
                qAssert(false); continue;
            }
            IxSqlRelation* pRelation = itr.value(); if (!pRelation)
            {
                qAssert(false); continue;
            }
            if (!m_allRelationXOrdered->exist(sKey))
            {
                m_allRelationXOrdered->insert(sKey, pRelation);
            }
        }
    }

    if (m_allRelationXOrdered)
    {
        qAssert(m_allRelationXOrdered->count() == m_allRelationX->count());
    }
    return qx_bool(true);
}
...
qx_bool QxSqlRelationLinked::QxSqlRelationLinkedImpl::insertRelationToHierarchy(const QStringList& sRelationX, const QString& sKey, qx::dao::sql_join::join_type eJoinType, qx::dao::detail::IxDao_Helper* pDaoHelper)
{
    ...
    if (sKey.contains(QStringLiteral("{")) && sKey.contains(QStringLiteral("}")))
    {
        int iPos1 = sKey.indexOf(QStringLiteral("{")); int iPos2 = sKey.indexOf(QStringLiteral("}"));
        if (iPos1 >= iPos2)
        {
            return qx_bool(false, QStringLiteral("invalid relation : character '}' before than character '{' (%1)").arg(sKey));
        }
        if (iPos1 > 0)
        {
            bModeRemoveColumns = (sKey.at(iPos1 - 1) == QLatin1Char('-'));
        } // syntax to remove columns instead of adding columns : -{ column1, column2, etc... }
        if (iPos1 > 1)
        {
            bModeRemoveColumns = (bModeRemoveColumns || ((sKey.at(iPos1 - 2) == QLatin1Char('-')) && (sKey.at(iPos1 - 1) == QLatin1Char(' '))));
        } // syntax to remove columns instead of adding columns : - { column1, column2, etc... }
        sKeyTemp = sKey.left(iPos1); sKeyTemp = sKeyTemp.trimmed();
        if (sKeyTemp.endsWith(QStringLiteral("-")))
        {
            sKeyTemp = sKeyTemp.left(sKeyTemp.count() - 1).trimmed();
        }
        QString sColumns = sKey.mid((iPos1 + 1), (iPos2 - iPos1 - 1));
        columns = sColumns.split(QStringLiteral(","));
        for (long l = 0; l < columns.count(); l++)
        {
            columns[l] = columns.at(l).trimmed();
        }
        if ((columns.count() == 1) && ((columns.at(0) == QStringLiteral("*")) || (columns.at(0) == QStringLiteral(""))))
        {
            columns = QStringList();
        }
    }
    ...
    if (sKeyTemp.contains(QStringLiteral("<")) && sKeyTemp.contains(QStringLiteral(">")))
    {
        int iPos1 = sKeyTemp.indexOf(QStringLiteral("<")); int iPos2 = sKeyTemp.indexOf(QStringLiteral(">"));
        if (iPos1 >= iPos2)
        {
            return qx_bool(false, QStringLiteral("invalid custom alias : character '>' before than character '<' (%1)").arg(sKeyTemp));
        }
        customAlias = sKeyTemp.mid((iPos1 + 1), (iPos2 - iPos1 - 1)).trimmed();
        if (customAlias.startsWith(QStringLiteral("...")))
        {
            customAliasSuffix = customAlias.right(customAlias.size() - 3); customAlias = QStringLiteral("");
        }
        if (customAlias.endsWith(QStringLiteral("...")))
        {
            customAliasPrefix = customAlias.left(customAlias.size() - 3); customAlias = QStringLiteral("");
        }
        if (customAlias.contains(QStringLiteral(" ")) || customAliasSuffix.contains(QStringLiteral(" ")) || customAliasPrefix.contains(QStringLiteral(" ")))
        {
            return qx_bool(false, QStringLiteral("invalid custom alias : custom alias cannot contain a space (%1)").arg(sKeyTemp));
        }
        if (customAlias.contains(QStringLiteral(".")) || customAliasSuffix.contains(QStringLiteral(".")) || customAliasPrefix.contains(QStringLiteral(".")))
        {
            return qx_bool(false, QStringLiteral("invalid custom alias : custom alias cannot contain a dot (%1)").arg(sKeyTemp));
        }
    ...
    {
        qAssert(false); return qx_bool(false, QStringLiteral("invalid relation key : '%1' (%2)").arg(sKeyTemp).arg(sKey));
    }
    IxSqlRelation* pRelation = m_allRelationX->getByKey(sKeyTemp);
    if (!pRelation)
    {
        qAssert(false); return qx_bool(false, QStringLiteral("invalid relation pointer : 'NULL pointer'"));
    }
    if (bModeRemoveColumns && (columns.count() > 0))
    {
        columns = removeColumns(columns, pRelation);
    }
    ...
    Q_FOREACH(QString sColumn, columns)
    {
        ...
        {
            return qx_bool(false, QStringLiteral("invalid relation column : '%1.%2' (%3)").arg(sKeyTemp).arg(sColumn).arg(sKey));
        }
    }
    ...
    if (bRelationXOrdered && !m_allRelationXOrdered)
    {
        m_allRelationXOrdered = std::make_shared<IxSqlRelationX>();
    }
    if (bRelationXOrdered && !m_allRelationXOrdered->exist(sKeyTemp))
    {
        m_allRelationXOrdered->insert(sKeyTemp, pRelation);
    }

    if (!customAliasSuffix.isEmpty() && pRelation->getClass())
    {
        customAlias = (pRelation->getClass()->getKey() + customAliasSuffix);
    }
    if (!customAliasPrefix.isEmpty() && pRelation->getClass())
    {
        customAlias = (customAliasPrefix + pRelation->getClass()->getKey());
    }
    if (!m_relationX.exist(sKeyTemp))
    {
        m_relationX.insert(sKeyTemp, QxSqlRelationLinkedImpl::type_relation(eJoinType, pRelation, qMakePair(QSet<QString>(columns.begin(), columns.end()), static_cast<long>(0)), customAlias));
    }
    if (sRelationX.count() <= 0)
    {
        return qx_bool(true);
    }
...
QStringList QxSqlRelationLinked::QxSqlRelationLinkedImpl::removeColumns(const QStringList& columnsToRemove, IxSqlRelation* pRelation) const
{
    ...
    {
        if (!columnsToRemoveSet.contains(pCurrData->getKey()))
        {
            columns.append(pCurrData->getKey());
        }
    }
    ...
    {
        if (!columnsToRemoveSet.contains(pCurrRelation->getKey()))
        {
            columns.append(pCurrRelation->getKey());
        }
    }
...
{
    m_pImpl->hierarchyAction(params, QxSqlRelationLinkedImpl::e_hierarchy_select);
}

void QxSqlRelationLinked::hierarchyFrom(QxSqlRelationParams& params)
{
    m_pImpl->hierarchyAction(params, QxSqlRelationLinkedImpl::e_hierarchy_from);
}

void QxSqlRelationLinked::hierarchyJoin(QxSqlRelationParams& params)
{
    m_pImpl->hierarchyAction(params, QxSqlRelationLinkedImpl::e_hierarchy_join);
}

void QxSqlRelationLinked::hierarchyWhereSoftDelete(QxSqlRelationParams& params)
{
    m_pImpl->hierarchyAction(params, QxSqlRelationLinkedImpl::e_hierarchy_where_soft_delete);
}
...
void QxSqlRelationLinked::hierarchyResolveOutput(QxSqlRelationParams& params)
{
    if (m_pImpl->m_bRoot)
    {
        params.setIndex(0); params.setIndexOwner(0); params.setCustomAlias(m_pImpl->m_sRootCustomAlias); params.setCustomAliasOwner(m_pImpl->m_sRootCustomAlias);
    }
    IxSqlRelationX* pAllRelationX = (m_pImpl->m_allRelationXOrdered ? m_pImpl->m_allRelationXOrdered.get() : m_pImpl->m_allRelationX); if (!pAllRelationX)
    {
        qAssert(false); return;
    }
    ...
    for (auto itr = pAllRelationX->begin(); itr != pAllRelationX->end(); ++itr)
    {
        ...
        if (bComplex)
        {
            vIdRelation = ((m_pImpl->m_bRoot || bEager) ? p->getIdFromQuery(bEager, params) : QVariant());
        }
        ...
        if (bByPass)
        {
            p->updateOffset(bEager, params);
        }
        else
        {
            if (bEager)
            {
                pFetched = p->eagerFetch_ResolveOutput(params);
            }
            else if (m_pImpl->m_bRoot && checkRootColumns(p->getKey()))
            {
                p->lazyFetch_ResolveOutput(params);
            }
            if (bValidId && pFetched)
            {
                params.builder().insertIdX(params.index(), params.id(), vIdRelation, pFetched);
            }
            if (!isValidDaoHelper(params))
            {
                return;
        ...
        if (bEager)
        {
            ...
            QVariant vOldId = params.id(); params.setId(QVariant(QStringLiteral("%1|%2").arg(vOldId.toString()).arg(vIdRelation.toString())));
...
QSqlError QxSqlRelationLinked::hierarchyOnBeforeSave(QxSqlRelationParams& params)
{
    ...
    {
        const QxSqlRelationLinkedImpl::type_relation& item = itr->second; if (!std::get<1>(item))
        {
            continue;
        }
...
bool QxSqlRelationLinked::getCartesianProduct() const
{
    for (auto itr = m_pImpl->m_relationX.begin(); itr != m_pImpl->m_relationX.end(); ++itr)
    {
        ...
        if (std::get<1>(item)->getCartesianProduct())
        {
            return true;
        }
    }
    ...
    {
        if (pRelationLinked && pRelationLinked->getCartesianProduct())
        {
            return true;
        }
    }
...
void QxSqlRelationLinked::updateOffset(QxSqlRelationParams& params)
{
    ...
    for (auto itr = pAllRelationX->begin(); itr != pAllRelationX->end(); ++itr)
    {
        ...
        {
            params.setColumns(NULL); params.setCustomAlias(QStringLiteral(""));
        }
        if (bEager)
        {
            p->updateOffset(true, params);
        }
        else if (m_pImpl->m_bRoot)
        {
            p->updateOffset(false, params);
        }
        QxSqlRelationLinked_ptr pRelationLinked = m_pImpl->m_relationLinkedX.value(p->getKey());
        if (!pRelationLinked)
        {
