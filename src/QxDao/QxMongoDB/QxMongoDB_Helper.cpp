...
{
    qx_scoped_wrapper<qx::dao::mongodb::qx_engine>()
    {
        mongoc_init();
    }
    ~qx_scoped_wrapper<qx::dao::mongodb::qx_engine>()
...
{
    mongoc_uri_t* m_uri;
    qx_scoped_wrapper<mongoc_uri_t>(const std::string& uri) : m_uri(NULL)
    {
        m_uri = mongoc_uri_new(uri.c_str());
    }
    ~qx_scoped_wrapper<mongoc_uri_t>()
    {
        if (m_uri)
        {
...
struct qx_scoped_wrapper<mongoc_client_pool_t>
{
    ...
    qx_scoped_wrapper<mongoc_client_pool_t>(mongoc_uri_t* uri) : m_pool(NULL)
    {
        if (uri)
        {
    ...
    ~qx_scoped_wrapper<mongoc_client_pool_t>()
    {
        if (m_pool)
        {
...
struct qx_scoped_wrapper<mongoc_client_t>
{
    ...
    mongoc_client_pool_t* m_pool;
    qx_scoped_wrapper<mongoc_client_t>(mongoc_client_pool_t* pool) : m_client(NULL), m_pool(pool)
    {
        if (m_pool)
        {
    ...
    ~qx_scoped_wrapper<mongoc_client_t>()
    {
        if (m_pool && m_client)
        {
...
struct qx_scoped_wrapper<mongoc_database_t>
{
    ...
    qx_scoped_wrapper<mongoc_database_t>(mongoc_client_t* client, const std::string& name) : m_db(NULL)
    {
        if (client)
        {
            m_db = mongoc_client_get_database(client, name.c_str());
        };
    }
    ~qx_scoped_wrapper<mongoc_database_t>()
    {
        if (m_db)
        {
...
struct qx_scoped_wrapper<mongoc_collection_t>
{
    ...
    qx_scoped_wrapper<mongoc_collection_t>(mongoc_client_t* client, const std::string& db, const std::string& name) : m_collection(NULL)
    {
        if (client)
        {
            m_collection = mongoc_client_get_collection(client, db.c_str(), name.c_str());
        };
    }
    ~qx_scoped_wrapper<mongoc_collection_t>()
    {
        if (m_collection)
        {
...
struct qx_scoped_wrapper<mongoc_cursor_t>
{
    ...
    {}
    ~qx_scoped_wrapper<mongoc_cursor_t>()
    {
        if (m_cursor)
        {
...
struct qx_scoped_wrapper<bson_t>
{
    ...
    {}
    qx_scoped_wrapper<bson_t>(const std::string& json) : m_doc(NULL)
    {
        if (json.empty())
        {
            m_doc = bson_new();
        ...
        {
            m_doc = bson_new_from_json((const uint8_t*)(json.c_str()), -1, (&m_err));
        }
    }
    ~qx_scoped_wrapper<bson_t>()
    {
        if (m_doc)
        {
            bson_destroy(m_doc);
...
struct qx_scoped_wrapper< std::vector<bson_t> >
{
    ...
    qx_scoped_wrapper< std::vector<bson_t> >(int size)
    {
        m_docs.reserve(size);
    }
    ~qx_scoped_wrapper< std::vector<bson_t> >()
    {
        for (bson_t* doc : m_docs)
        {
            if (doc)
            {
                bson_destroy(doc);
    ...
    bool append(const std::string& json)
    {
        if (!json.empty())
        {
            bson_t* doc = bson_new_from_json((const uint8_t*)(json.c_str()), -1, (&m_err)); if (doc)
            {
                m_docs.push_back(doc); return true;
            }
        }; return false;
    }
    std::vector<bson_t*>* get()
    {
        return (&m_docs);
    }
    bson_error_t* err()
...
struct qx_scoped_wrapper<mongoc_bulk_operation_t>
{
    ...
    qx_scoped_wrapper<mongoc_bulk_operation_t>(mongoc_collection_t* coll, const bson_t* opts) : m_bulk(NULL)
    {
        if (coll)
        {
            m_bulk = mongoc_collection_create_bulk_operation_with_opts(coll, opts);
        }
    }
    ~qx_scoped_wrapper<mongoc_bulk_operation_t>()
    {
        if (m_bulk)
        {
...
struct qx_scoped_wrapper<qx::dao::mongodb::qx_opts>
{
    bson_t* m_opts;
    bson_error_t m_err;
    qx_scoped_wrapper<qx::dao::mongodb::qx_opts>(const std::string& json) : m_opts(NULL)
    {
        if (!json.empty())
        {
            m_opts = bson_new_from_json((const uint8_t*)(json.c_str()), -1, (&m_err));
        }
    }
    ~qx_scoped_wrapper<qx::dao::mongodb::qx_opts>()
    {
        if (m_opts)
        {
            bson_destroy(m_opts);
    ...
    {
        return const_cast<const bson_t*>(m_opts);
    }
    bson_error_t* err()
    {
        return (&m_err);
...
struct qx_scoped_wrapper<qx::dao::mongodb::qx_reply>
{
    ...
    {}
    ~qx_scoped_wrapper<qx::dao::mongodb::qx_reply>()
    {
        if (m_destroy)
        {
    ...
    QString toString()
    {
        ...
        {
            return m_json;
        }; char* s = bson_as_relaxed_extended_json((&m_reply), NULL); m_json = QString(s); bson_free(s); return m_json;
    }
    QVariantMap& values()
    {
        if (m_values.isEmpty())
        {
            QString txt = toString(); qx::serialization::json::from_string(m_values, txt);
        }; return m_values;
    }
    QVariant value(const QString& key)
    {
        return (values().contains(key) ? values().value(key) : QVariant());
    }
    void log()
    {
        QString txt = toString(); 
        xinfo("MongoDB database reply : {}", qPrintable(txt));
    }
    bson_t* get()
    {
        return (&m_reply);
...
struct QxMongoDB_Helper::QxMongoDB_HelperImpl
{
    ...
    QxMongoDB_HelperImpl() : m_bLogDatabaseReply(false), m_bLogDatabaseInfo(true)
    {
        m_pInstance.reset(new qx_scoped_wrapper<qx::dao::mongodb::qx_engine>());
    }
    ~QxMongoDB_HelperImpl()
    {
        m_pPool.reset(); m_pUri.reset(); m_pInstance.reset();
    }
    ...
    {
        {
            ...
            if (!db->getUserName().isEmpty() && !db->getPassword().isEmpty())
            {
                uri += QStringLiteral(":") + db->getPassword() + QStringLiteral("@");
            }
            if (!db->getHostName().isEmpty())
            {
                uri += db->getHostName() + QStringLiteral(":") + QString::number(port);
            }
            if (!db->getConnectOptions().isEmpty())
            {
                uri += "/?" + db->getConnectOptions();
            }
            if (!uri.isEmpty())
            {
                uri = "mongodb://" + uri;
            ...
            if (uri.isEmpty())
            {
                return QSqlError("Please define a connection string to a MongoDB database through 'qx::QxSqlDatabase::getSingleton()' settings", QStringLiteral(""), QSqlError::UnknownError);
            }
            if (m_bLogDatabaseInfo)
            {
                xinfo("Connect to MongoDB database : {}", qPrintable(uri));
            }
            ...
            if (!m_pUri->get())
            {
                m_pUri.reset(); return QSqlError("Unable to create a 'mongoc_uri_t' instance to connect to MongoDB database (" + uri + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
            }
            m_pPool.reset(new qx_scoped_wrapper<mongoc_client_pool_t>(m_pUri->get()));
            if (!m_pPool->get())
            {
                m_pPool.reset(); m_pUri.reset(); return QSqlError("Unable to create a 'mongoc_client_pool_t' instance to connect to MongoDB database (" + uri + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
            }
        }
        ...
        if (err.isValid())
        {
            m_pPool.reset(); m_pUri.reset(); return err;
        }
        else
        {
            if (m_bLogDatabaseInfo)
            {
                xinfo("Ping to MongoDB database server : OK");
            }
        }
        ...
        else
        {
            if (m_bLogDatabaseInfo)
            {
                QString resp = buildInfo.response().toString(); 
                xinfo("Fetch build info from MongoDB database server : \n{}", qPrintable(resp));
            }
        }
    ...
    QSqlError initCollection(qx::IxClass* pClass, qx_db_collection& coll)
    {
        if (!pClass)
        {
            return QSqlError("Parameter 'qx::IxClass' is required to get MongoDB database collection name", QStringLiteral(""), QSqlError::UnknownError);
        }
        QSqlError err = initDatabase(coll); if (err.isValid())
        {
            return err;
        }
        ...
        if (!coll.collection->get())
        {
            return QSqlError("Unable to create a 'mongoc_collection_t' instance from MongoDB database (" + qx::QxSqlDatabase::getSingleton()->getDatabaseName() + QStringLiteral(" - ") + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
        }
        return QSqlError();
    }
    ...
    {
        QSqlError err = initPool(); if (err.isValid())
        {
            return err;
        }
        ...
        if (!coll.client->get())
        {
            return QSqlError("Unable to get a MongoDB client connection from pool of type 'mongoc_client_t'", QStringLiteral(""), QSqlError::UnknownError);
        }
        std::string dbName = toStdString(qx::QxSqlDatabase::getSingleton()->getDatabaseName());
        if (dbName.empty())
        {
            return QSqlError("Unable to connect to MongoDB database : please define a database name using qx::QxSqlDatabase::getSingleton()->setDatabaseName()", QStringLiteral(""), QSqlError::UnknownError);
        }
        coll.db = std::make_shared<qx_scoped_wrapper<mongoc_database_t> >(coll.client->get(), dbName);
        if (!coll.db->get())
        {
            return QSqlError("Unable to create a 'mongoc_database_t' instance from MongoDB database (" + qx::QxSqlDatabase::getSingleton()->getDatabaseName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
        }
        return QSqlError();
    }
    ...
    QString asJson(const QString& s) const
    {
        ...
        if (s.startsWith("qx_oid:"))
        {
            return "{ \"$oid\": \"" + s.right(s.size() - 7) + "\" }";
        }
        return ((isNumber ? QString() : QStringLiteral("\"")) + json.replace("\"", "\\\"") + (isNumber ? QString() : QStringLiteral("\"")));
    }
    ...
    QSqlError checkInsertOID(bson_t* doc, qx::IxClass* pClass, QString& insertedId)
    {
        ...
        if (!bson_iter_init_find((&itr), doc, "_id"))
        {
            ...
            if (!bson_append_oid(doc, "_id", -1, (&oid)))
            {
                return QSqlError("Unable to append 'bson_oid_t' to document (" + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
            }
    ...
    bool setOptions_(QxMongoDB_Helper::opts e, const QString& optsAsJson)
    {
        ...
        if (optsAsJson.isEmpty())
        {
            m_lstOpts.remove(i); return true;
        }
    ...
    QSqlError getOptions_(QxMongoDB_Helper::opts e, qx_opts_ptr& opts, const QString& customOpts = QString())
    {
        ...
        QString json = (customOpts.isEmpty() ? m_lstOpts.value(i) : customOpts); if (json.isEmpty())
        {
            return QSqlError();
        }
        ...
        if (!opts->get())
        {
            QString msg(opts->err()->message); opts.reset(); return QSqlError("Unable to create a 'bson_t' options instance (" + QString::number(i) + QStringLiteral(" - ") + json + ") from JSON : " + msg, QStringLiteral(""), QSqlError::UnknownError);
        }
        return QSqlError();
    }

    QString buildLookup(qx::dao::detail::IxDao_Helper* pDaoHelper, bool idAsArray, qx::QxSqlRelationLinked* pSubHierarchy = NULL, const QString& sTab = QStringLiteral("\t"), QStringList* pSubFieldsAs = NULL)
    {
        qx::QxSqlRelationLinked* pHierarchy = (pSubHierarchy ? pSubHierarchy : (pDaoHelper ? pDaoHelper->getSqlRelationLinked() : NULL)); if (!pHierarchy)
        {
            return QString();
        }
        QString templFrom = "\n\t" + sTab + "\"from\": \"%COLL_TARGET%\"";
        QString templLet = "\n\t" + sTab + "\"let\": { \"%COLL_LET_KEY%\": \"$%COLL_LET_VAL%\" }";
        QString templPipeline = "\n\t" + sTab + "\"pipeline\": [ { \"$match\": { \"$expr\": { \"$" + (idAsArray ? QStringLiteral(QStringLiteral("in")) : QStringLiteral("eq")) + "\": [ \"$%COLL_EQ%\", \"$$%COLL_LET_KEY%\" ] } } } %NEXT_LEVEL_LOOKUP% %PROJECTION% ]";
        QString templAs = "\n\t" + sTab + "\"as\": \"%COLL_AS%\"";
        QString templOneToMany = QStringLiteral("\n") + sTab + "{ \"$lookup\": { " + templFrom + ", " + templLet + ", " + templPipeline + ", " + templAs + " } }";
        ...
        for (auto itr = pRelations.begin(); itr != pRelations.end(); ++itr)
        {
            ...
            qx::IxSqlRelation* pRelation = std::get<1>(item); if (!pRelation)
            {
                continue;
            }
            qx::IxSqlRelation::relation_type type = pRelation->getRelationType();
            if ((type != qx::IxSqlRelation::many_to_one) && (type != qx::IxSqlRelation::one_to_many) && (type != qx::IxSqlRelation::one_to_one))
            {
                continue;
            }
            qx::IxDataMember* pDataMember = pRelation->getDataMember(); if (!pDataMember)
            {
                continue;
            }
            qx::IxDataMember* pFromId = pRelation->getDataIdOwner(); if (!pFromId)
            {
                continue;
            }
            qx::IxDataMember* pTargetId = pRelation->getDataId(); if (!pTargetId)
            {
                continue;
            }
            qx::IxClass* pFrom = pRelation->getClassOwner(); if (!pFrom)
            {
                continue;
            }
            qx::IxClass* pTarget = pRelation->getClass(); if (!pTarget)
            {
                continue;
            }
            ...
            if (pSubFieldsAs)
            {
                (*pSubFieldsAs) << pDataMember->getKey();
            }
            else
            {
                projectionFields << pDataMember->getKey();
            }
            ...
            if (columns.count() > 0)
            {
                ...
                while ((p = pRelation->nextData(l)))
                {
                    if (p && columns.contains(p->getKey()))
                    {
                        projection += ((projection.isEmpty() ? QStringLiteral("\"") : QStringLiteral(", \"")) + p->getKey() + "\": 1");
                    }
                }
                if (!projection.isEmpty())
                {
                    Q_FOREACH(QString s, subProjectionFields)
                    {
                        projection += ((projection.isEmpty() ? QStringLiteral("\"") : QStringLiteral(", \"")) + s + "\": 1");
                    }
                }
                if (!projection.isEmpty())
                {
            ...
            result += ((result.isEmpty() ? QString() : QStringLiteral(", ")) + lookup);
        }
        ...
        if (pDaoHelper && pHierarchy && !pSubHierarchy && !result.isEmpty())
        {
            ...
            while ((p = pDaoHelper->builder().nextData(l)))
            {
                if (p && pHierarchy->checkRootColumns(p->getKey()))
                {
                    project += ((project.isEmpty() ? QStringLiteral("\"") : QStringLiteral(", \"")) + p->getKey() + "\": 1");
                }
                else
                {
                    bIncludeAllFields = false;
                }
            }
            if (!project.isEmpty())
            {
                Q_FOREACH(QString s, projectionFields)
                {
                    project += ((project.isEmpty() ? QStringLiteral("\"") : QStringLiteral(", \"")) + s + "\": 1");
                }
            }
            if (!project.isEmpty())
            {
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::insertOne_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, const QString& json, QString& insertedId)
{
    ...
    qx_db_collection coll; QSqlError err = initCollection(pClass, coll); if (err.isValid())
    {
        return err;
    }
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB insert one '" + (pClass ? pClass->getName() : QString()) + "' :\n" + json);
    }

    qx_scoped_wrapper<bson_t> doc(toStdString(json));
    if (!doc.get())
    {
        return QSqlError("Unable to create a 'bson_t' document instance (" + pClass->getName() + ") from JSON : " + QString(doc.err()->message), QStringLiteral(""), QSqlError::UnknownError);
    }
    err = checkInsertOID(doc.get(), pClass, insertedId); if (err.isValid())
    {
        return err;
    }

    bson_error_t bsonError; qx_opts_ptr opts; qx_reply_ptr reply;
    if (m_bLogDatabaseReply)
    {
        reply.reset(new qx_scoped_wrapper<qx::dao::mongodb::qx_reply>());
    }
    err = getOptions_(QxMongoDB_Helper::opts_collection_insert_one, opts); if (err.isValid())
    {
        return err;
    }
    ...
    if (!ok)
    {
        return QSqlError("Unable to insert one 'bson_t' document (" + pClass->getName() + ") to MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply)
    {
        reply->log();
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::insertMany_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, const QStringList& json, QStringList& insertedId)
{
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB insert many '" + (pClass ? pClass->getName() : QString()) + "' :\n" + json.join(", \n"));
    }
    ...
    Q_FOREACH(QString s, json)
    {
        ...
        if (!docs.append(toStdString(s)))
        {
            return QSqlError("Unable to create a 'bson_t' document instance (" + pClass->getName() + ") from JSON : " + QString(docs.err()->message), QStringLiteral(""), QSqlError::UnknownError);
        }
        QString oid; err = checkInsertOID(docs.m_docs.back(), pClass, oid); if (err.isValid())
        {
            return err;
        }; insertedId.append(oid);
    }
    ...
    err = getOptions_(QxMongoDB_Helper::opts_collection_insert_many, opts); if (err.isValid())
    {
        return err;
    }
    ...
    if (!ok)
    {
        return QSqlError("Unable to insert many 'bson_t' documents (" + pClass->getName() + ") to MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply)
    {
        reply->log();
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::updateOne_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, const QString& json, const qx::QxSqlQuery* query)
{
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB update one '" + (pClass ? pClass->getName() : QString()) + "' :\n" + json + QStringLiteral("\n") + (query ? query->queryAt(0) : QString()));
    }
    ...
    if (!query_.get())
    {
        return QSqlError("Unable to create a 'bson_t' document/query instance (" + pClass->getName() + ") from JSON : " + QString(query_.err()->message), QStringLiteral(""), QSqlError::UnknownError);
    }
    if (!query || query->queryAt(0).isEmpty())
    {
        ...
        if (!oid)
        {
            return QSqlError("Unable to find/fetch document oid '_id' (" + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
        }
        if (!bson_append_value(query_.get(), "_id", -1, oid))
        {
            return QSqlError("Unable to append 'bson_value_t' oid value to document (" + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
        }
    }

    qx_scoped_wrapper<bson_t> update_("");
    const bson_t* doc_ = const_cast<const bson_t*>(doc.get());
    if (!bson_append_document(update_.get(), "$set", -1, doc_))
    {
        return QSqlError("Unable to append 'bson_t' document for update query (" + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
    }

    bson_error_t bsonError; qx_opts_ptr opts; qx_reply_ptr reply; reply.reset(new qx_scoped_wrapper<qx::dao::mongodb::qx_reply>());
    err = getOptions_(QxMongoDB_Helper::opts_collection_update_one, opts, (query ? query->queryAt(1) : QString())); if (err.isValid())
    {
        return err;
    }
    ...
    if (!ok)
    {
        return QSqlError("Unable to update one 'bson_t' document (" + pClass->getName() + ") to MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply && (reply->value("matchedCount").toInt() < 1))
    {
        return QSqlError("Nothing has been updated (" + pClass->getName() + ") in MongoDB database", QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply && m_bLogDatabaseReply)
    {
        reply->log();
    }
    return QSqlError();
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::updateMany_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, const QStringList& json, const qx::QxSqlQuery* query)
{
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB update many '" + (pClass ? pClass->getName() : QString()) + "' :\n" + json.join(", \n") + QStringLiteral("\n") + (query ? query->queryAt(0) : QString()));
    }

    qx_opts_ptr optsBulk; err = getOptions_(QxMongoDB_Helper::opts_collection_create_bulk_operation, optsBulk); if (err.isValid())
    {
        return err;
    }
    ...
    if (!bulk.get())
    {
        return QSqlError("Unable to create 'mongoc_bulk_operation_t' bulk operation instance (" + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
    }
    qx_opts_ptr opts; err = getOptions_(QxMongoDB_Helper::opts_bulk_operation_update_one, opts, (query ? query->queryAt(1) : QString())); if (err.isValid())
    {
        return err;
    }
    int iUpdateCount = 0;
    ...
    Q_FOREACH(QString s, json)
    {
        ...
        {
            return QSqlError("Unable to append update bulk operation (" + pClass->getName() + ") : " + QString(bsonErr.message), QStringLiteral(""), QSqlError::UnknownError);
        }
        iUpdateCount++;
    }
    ...
    if (!ok)
    {
        return QSqlError("Unable to update many 'bson_t' documents (" + pClass->getName() + ") to MongoDB database (bulk operation) : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply && (reply->value("nMatched").toInt() != iUpdateCount))
    {
        return QSqlError("Not everything has been updated (" + pClass->getName() + ") in MongoDB database (result:'" + QString::number(reply->value("nMatched").toInt()) + "', expected:'" + QString::number(iUpdateCount) + "')", QStringLiteral(""), QSqlError::UnknownError);
    }
    if (m_bLogDatabaseReply && reply)
    {
        reply->log();
    }
    return QSqlError();
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::deleteOne_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, const QString& json, const qx::QxSqlQuery* query)
{
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB delete one '" + (pClass ? pClass->getName() : QString()) + "' :\n" + json + QStringLiteral("\n") + (query ? query->queryAt(0) : QString()));
    }
    ...
    bson_error_t bsonError; qx_opts_ptr opts; qx_reply_ptr reply; reply.reset(new qx_scoped_wrapper<qx::dao::mongodb::qx_reply>());
    err = getOptions_(QxMongoDB_Helper::opts_collection_delete_one, opts, (query ? query->queryAt(1) : QString())); if (err.isValid())
    {
        return err;
    }
    ...
    if (!ok)
    {
        return QSqlError("Unable to delete one 'bson_t' document (" + pClass->getName() + ") in MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply && (reply->value("deletedCount").toInt() < 1))
    {
        return QSqlError("Nothing has been deleted (" + pClass->getName() + ") in MongoDB database", QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply && m_bLogDatabaseReply)
    {
        reply->log();
...
{
    if ((json.count() <= 0) && (!query))
    {
        return QSqlError();
    }
    if (query && (query->type() == "aggregate"))
    {
        QStringList empty; return aggregate_(pDaoHelper, pClass, empty, query, QStringLiteral(""), NULL, true);
    }
    QString listId = (query ? QString() : ((json.count() > 0) ? QStringLiteral("{ \"_id\": { \"$in\": [ %ID% ] } }") : QString()));
    if (!listId.isEmpty())
    {
        QString ids; Q_FOREACH(QString s, json)
        {
            ids += asJson(s) + ", ";
        }; ids = ids.left(ids.count() - 2); listId.replace("%ID%", ids);
    }
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB delete many '" + (pClass ? pClass->getName() : QString()) + "' :\n" + json.join(", \n") + QStringLiteral("\n") + (query ? query->queryAt(0) : QString()));
    }
    qx_db_collection coll; QSqlError err = initCollection(pClass, coll); if (err.isValid())
    {
        return err;
    ...
    err = getOptions_(QxMongoDB_Helper::opts_collection_delete_many, opts, (query ? query->queryAt(1) : QString())); if (err.isValid())
    {
        return err;
    }
    ...
    if (!ok)
    {
        return QSqlError("Unable to delete many 'bson_t' documents (" + pClass->getName() + ") in MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    if (reply)
    {
        reply->log();
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::findOne_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, QString& json, const qx::QxSqlQuery* query)
{
    ...
    {
        QStringList result;
        if ((!query) && (!json.isEmpty()))
        {
            qx_query query_tmp(json); err = aggregate_(pDaoHelper, pClass, result, (&query_tmp), lookup, NULL);
        }
        else
        {
            err = aggregate_(pDaoHelper, pClass, result, query, lookup, NULL);
        }
    ...
    qx_db_collection coll; err = initCollection(pClass, coll); if (err.isValid())
    {
        return err;
    }
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB find one '" + (pClass ? pClass->getName() : QString()) + "' :\n" + findQuery);
    }
    if (!query_.get())
    {
        return QSqlError("Unable to create a 'bson_t' document/query instance (" + pClass->getName() + ") from JSON : " + QString(query_.err()->message), QStringLiteral(""), QSqlError::UnknownError);
    ...
    QString projection; Q_FOREACH(QString col, columns)
    {
        projection += (col.isEmpty() ? QString() : ((projection.isEmpty() ? QStringLiteral("\"") : QStringLiteral(", \"")) + col + "\": 1"));
    }
    projection = (projection.isEmpty() ? QString() : QStringLiteral("{ \"projection\": { " + projection + " } }"));

    QString customOpts = ((query && !query->queryAt(1).isEmpty()) ? query->queryAt(1) : projection);
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB find one '" + (pClass ? pClass->getName() : QString()) + "' :\n" + findQuery + QStringLiteral("\n") + customOpts);
    }
    qx_opts_ptr opts; err = getOptions_(QxMongoDB_Helper::opts_collection_find, opts, customOpts); if (err.isValid())
    {
        return err;
    }
    ...
    if (!cursor.get())
    {
        return QSqlError("Unable to create a 'mongoc_cursor_t' cursor instance (" + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
    }
    ...
    {
        return QSqlError("MongoDB cursor error trying to find one document (" + pClass->getName() + ") : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    return QSqlError();
}
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::findMany_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, QStringList& json, const qx::QxSqlQuery* query, QxMongoDB_Fetcher* pFetcher)
{
    ...
    json.clear(); qx_db_collection coll; QSqlError err = initCollection(pClass, coll); if (err.isValid())
    {
        return err;
    }
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB find many '" + (pClass ? pClass->getName() : QString()) + "' :\n" + findQuery);
    }
    if (!query_.get())
    {
        return QSqlError("Unable to create a 'bson_t' document/query instance (" + pClass->getName() + ") from JSON : " + QString(query_.err()->message), QStringLiteral(""), QSqlError::UnknownError);
    ...
    while (mongoc_cursor_next(cursor.get(), (&doc)))
    {
        ...
        if (pFetcher)
        {
            QString tmp(val); pFetcher->fetch(tmp); if (pDaoHelper && (!pDaoHelper->isValid()))
            {
                bson_free(val); return pDaoHelper->error();
            }
        }
        else
        {
    ...
    if (mongoc_cursor_error(cursor.get(), (&bsonError)))
    {
        return QSqlError("MongoDB cursor error trying to find many documents (" + pClass->getName() + ") : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    return QSqlError();
}
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::aggregate_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, QStringList& json, const qx::QxSqlQuery* query, const QString& lookup, QxMongoDB_Fetcher* pFetcher, bool bModeDelete /* = false */)
{
    ...
    QString findQuery = (query ? query->queryAt(0) : listId);
    if (!lookup.isEmpty() && !findQuery.isEmpty())
    {
        findQuery = "[ { \"$match\": " + findQuery + " }, " + lookup + " ]";
    }
    else if (!lookup.isEmpty() && findQuery.isEmpty())
    {
        findQuery = "[ " + lookup + " ]";
    }
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB aggregate '" + (pClass ? pClass->getName() : QString()) + "' :\n" + findQuery);
    }
    if (!query_.get())
    {
        return QSqlError("Unable to create a 'bson_t' document/query instance (" + pClass->getName() + ") from JSON : " + QString(query_.err()->message), QStringLiteral(""), QSqlError::UnknownError);
    ...
    qx_opts_ptr opts; err = getOptions_(QxMongoDB_Helper::opts_collection_aggregate, opts, (query ? query->queryAt(1) : QString())); if (err.isValid())
    {
        return err;
    }
    ...
    if (bModeDelete && (listIdsToDelete.count() > 0))
    {
        return deleteMany_(pDaoHelper, pClass, listIdsToDelete, NULL);
    }
    return QSqlError();
}
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::executeCommand_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, qx::QxSqlQuery* query)
{
    if (!query)
    {
        return QSqlError("Unable to execute command in MongoDB database : 'query' parameter is required", QStringLiteral(""), QSqlError::UnknownError);
    }
    query->setResponse(QVariant()); qx_db_collection coll;
    QSqlError err = (pClass ? initCollection(pClass, coll) : initDatabase(coll)); if (err.isValid())
    {
        return err;
    }
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB command '" + (pClass ? pClass->getName() : QString()) + "' :\n" + (query ? query->queryAt(0) : QString()));
    }
    if (!cmd_.get())
    {
        return QSqlError("Unable to create a 'bson_t' document/command instance (" + collName + ") from JSON : " + QString(cmd_.err()->message), QStringLiteral(""), QSqlError::UnknownError);
    }

    bson_error_t bsonError; qx_opts_ptr opts; qx_reply_ptr reply;
    reply.reset(new qx_scoped_wrapper<qx::dao::mongodb::qx_reply>());
    err = getOptions_(QxMongoDB_Helper::opts_collection_command, opts, (query ? query->queryAt(1) : QString())); if (err.isValid())
    {
        return err;
    }
    ...
    if (!pClass)
    {
        ...
        {
            return QSqlError("Unable to execute command in MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
        }
    }
    else
    {
        ...
        {
            return QSqlError("Unable to execute command (" + collName + ") in MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
        }
    }
    ...
    if (query && (query->type() == "cursor"))
    {
        ...
        if (!cursor.get())
        {
            return QSqlError("Unable to create a 'mongoc_cursor_t' cursor instance from reply (" + pClass->getName() + QStringLiteral(")"), QStringLiteral(""), QSqlError::UnknownError);
        }

        const bson_t* doc = NULL; QString json;
        while (mongoc_cursor_next(cursor.get(), (&doc)))
        {
            char* val = bson_as_relaxed_extended_json(doc, NULL); json += ((json.isEmpty() ? QString() : QStringLiteral(",\n")) + QString(val)); bson_free(val);
        }
        if (!json.isEmpty())
        {
            json = "[ " + json + " ]";
        ...
        if (mongoc_cursor_error(cursor.get(), (&bsonError)))
        {
            return QSqlError("MongoDB cursor error trying to find many documents from reply (" + pClass->getName() + ") : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
        }
    }
    else
    {
...
QSqlError QxMongoDB_Helper::QxMongoDB_HelperImpl::count_(qx::dao::detail::IxDao_Helper* pDaoHelper, qx::IxClass* pClass, long& cnt, const qx::QxSqlQuery* query)
{
    ...
    QSqlError err = initCollection(pClass, coll); if (err.isValid())
    {
        return err;
    }
    ...
    if (pDaoHelper)
    {
        pDaoHelper->qxQuery().queryAt(0, "MongoDB count '" + (pClass ? pClass->getName() : QString()) + "' :\n" + (query ? query->queryAt(0) : QString()));
    }
    if (!query_.get())
    {
        return QSqlError("Unable to create a 'bson_t' document/query instance (" + pClass->getName() + ") from JSON : " + QString(query_.err()->message), QStringLiteral(""), QSqlError::UnknownError);
    ...
    err = getOptions_(QxMongoDB_Helper::opts_collection_count, opts, (query ? query->queryAt(1) : QString())); if (err.isValid())
    {
        return err;
    }
    ...
    if (cnt < 0)
    {
        cnt = 0; return QSqlError("Unable to count 'bson_t' documents (" + pClass->getName() + ") in MongoDB database : " + QString(bsonError.message), QStringLiteral(""), QSqlError::UnknownError);
    }
    return QSqlError();
}
...
QSqlError QxMongoDB_Helper::autoCreateIndexes(bool log /* = true */)
{
    ...
    if (!pAllClasses)
    {
        return QSqlError("Unable to get all classes registered in QxOrm context", QStringLiteral(""), QSqlError::UnknownError);
    }

    for (auto itr = pAllClasses->begin(); itr != pAllClasses->end(); ++itr)
    {
        qx::IxClass* pClass = itr->second; if (!pClass || pClass->isAbstract())
        {
            continue;
        }
        ...
        do
        {
            ...
            for (long l = 0; l < cnt; l++)
            {
                ...
                if (!pDataMember || !pDataMember->getDao() || pDataMember->getIsPrimaryKey())
                {
                    continue;
                }
                qx::IxSqlRelation* pRelation = pDataMember->getSqlRelation();
                bool isIndex = pDataMember->getIsIndex(); if (!isIndex && !pRelation)
                {
                    continue;
                }
                ...
                QString idxName;
                if (eRelationType == qx::IxSqlRelation::many_to_one)
                {
                    idxName = "idx_" + collectionName + QStringLiteral("_") + pDataMember->getKey() + "_many_to_one";
                }
                else if (eRelationType == qx::IxSqlRelation::one_to_one)
                {
                    idxName = "idx_" + collectionName + QStringLiteral("_") + pDataMember->getKey() + "_one_to_one";
                }
                else if (isIndex)
                {
                    idxName = "idx_" + collectionName + QStringLiteral("_") + pDataMember->getKey();
                }
                if (idxName.isEmpty())
                {
                    continue;
                ...
                query_ += "[ { \"key\": { \"" + pDataMember->getKey() + "\": 1 }, \"name\": \"" + idxName + "\"" + (isUnique ? QStringLiteral(", \"unique\": true") : QString()) + " } ] }";
                qx_query query(query_);

                if (log)
                {
                    QString msg = "MongoDB create index for '" + collectionName + QStringLiteral("::") + pDataMember->getKey() + "', request : " + query_; 
                    xinfo(qPrintable(msg));
                }
                QSqlError err = executeCommand(NULL, NULL, (&query)); if (err.isValid())
                {
                    return err;
                }
                if (log)
                {
                    ...
                    QString msg = "MongoDB create index for '" + collectionName + QStringLiteral("::") + pDataMember->getKey() + "', response : " + response;
                    xinfo(qPrintable(msg));
                }
            }
