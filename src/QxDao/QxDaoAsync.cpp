...
#define QX_DAO_ASYNC_QUERY_ALREADY_RUNNING   xwarn("another query is already running");
...
QxDaoAsync::~QxDaoAsync()
{
    ...
    xinfo("thread is running : quit and wait");
    quit(); wait();
}
...
void QxDaoAsync::run()
{
    ...
    if (m_pDaoParams && (m_pDaoParams->daoAction != qx::dao::detail::QxDaoAsyncParams::dao_none))
    {
        daoRunner.onQueryStarted(m_pDaoParams);
    }
    exec();
}
...
void QxDaoAsync::startQuery()
{
    ...
    if (isRunning())
    {
        Q_EMIT queryStarted(m_pDaoParams);
    }
    else
    {
        start();
...
bool QxDaoAsync::asyncCount(const QString& className, const qx::QxSqlQuery& query /* = qx::QxSqlQuery() */, QSqlDatabase* pDatabase /* = NULL */)
{
    ...
    if (m_pDaoParams)
    {
        QX_DAO_ASYNC_QUERY_ALREADY_RUNNING; return false;
    }
    if (!QxClassX::implementIxPersistable(className))
    {
...
QSqlError QxDaoAsyncRunner::runQuery(qx::dao::detail::QxDaoAsyncParams_ptr pDaoParams)
{
    if (!pDaoParams)
    {
        qAssert(false); return QSqlError(QStringLiteral("qx::QxDaoAsync : 'null dao parameters'"), QStringLiteral(""), QSqlError::UnknownError);
    }
    ...
    try
    {
        ...
        {
            qAssert(false); return QSqlError(QStringLiteral("qx::QxDaoAsync : 'null instance implementing qx::IxPersistable interface'"), QStringLiteral(""), QSqlError::UnknownError);
        }
    ...
    catch (const qx::dao::sql_error& sqlErr)
    {
        daoError = sqlErr.get();
    }
    catch (const std::exception& err)
    {
        daoError = QSqlError(QLatin1String(err.what()), QStringLiteral(""), QSqlError::UnknownError);
    }
    catch (...)
    {
        daoError = QSqlError(QStringLiteral("qx::QxDaoAsync : 'unknown error executing query'"), QStringLiteral(""), QSqlError::UnknownError);
    }
    ...
    if (pDatabase && !dbKey.isEmpty())
    {
        QSqlDatabase::removeDatabase(dbKey);
    }
    return daoError;
}
