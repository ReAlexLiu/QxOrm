...
struct IxSqlRelation::IxSqlRelationImpl
{
    ...
    IxDataMember* isValid_SqlRelation(long lIndex) const
    {
        ...
        if (bIsValid && (!m_iIsSameDataOwner) && (p != m_pDataMember))
        {
...
QString IxSqlRelation::getForeignKey() const
{
    return m_pImpl->m_sForeignKey;
}

QString IxSqlRelation::getForeignKeyOwner() const
{
    return m_pImpl->m_sForeignKeyOwner;
}

QString IxSqlRelation::getForeignKeyDataType() const
{
    return m_pImpl->m_sForeignKeyDataType;
}

QString IxSqlRelation::getExtraTable() const
{
    return m_pImpl->m_sExtraTable;
}

QxCollection<QString, IxDataMember*>* IxSqlRelation::getLstDataMember() const
{
    return m_pImpl->m_lstDataMemberPtr.get();
}

IxSqlRelationX* IxSqlRelation::getLstRelation() const
{
    return m_pImpl->m_lstSqlRelationPtr.get();
}

void IxSqlRelation::setSqlJoinType(qx::dao::sql_join::join_type e)
{
    m_pImpl->m_eJoinType = e;
}

qx::dao::sql_join::join_type IxSqlRelation::getSqlJoinType() const
{
    return m_pImpl->m_eJoinType;
}

IxSqlRelation::relation_type IxSqlRelation::getRelationType() const
{
    return m_pImpl->m_eRelationType;
}

IxClass* IxSqlRelation::getClass() const
{
    return m_pImpl->m_pClass;
}

IxClass* IxSqlRelation::getClassOwner() const
{
    return m_pImpl->m_pClassOwner;
}

IxDataMember* IxSqlRelation::getDataMember() const
{
    return m_pImpl->m_pDataMember;
}

IxDataMemberX* IxSqlRelation::getDataMemberX() const
{
    return m_pImpl->m_pDataMemberX;
}

IxDataMember* IxSqlRelation::getDataId() const
{
    return m_pImpl->m_pDataMemberId;
}

IxDataMember* IxSqlRelation::getDataIdOwner() const
{
    return m_pImpl->m_pDataMemberIdOwner;
}

void IxSqlRelation::setIsSameDataOwner(int i)
{
    m_pImpl->m_iIsSameDataOwner = i;
}

bool IxSqlRelation::canInit() const
{
    if (m_pImpl->m_bInitDone)
    {
        return false;
    }
    {
        QMutexLocker locker(&m_pImpl->m_mutex); return (!m_pImpl->m_bInitInEvent);
    }
}

void IxSqlRelation::setClass(IxClass* pClass, IxClass* pClassOwner)
{
    m_pImpl->m_pClass = pClass; m_pImpl->m_pClassOwner = pClassOwner;
}

void IxSqlRelation::setRelationType(IxSqlRelation::relation_type e)
{
    m_pImpl->m_eRelationType = e;
}

void IxSqlRelation::setForeignKey(const QString& s) const
{
    m_pImpl->m_sForeignKey = s;
}

void IxSqlRelation::setForeignKeyOwner(const QString& s) const
{
    m_pImpl->m_sForeignKeyOwner = s;
}

void IxSqlRelation::setForeignKeyDataType(const QString& s) const
{
    m_pImpl->m_sForeignKeyDataType = s;
}

void IxSqlRelation::setExtraTable(const QString& s) const
{
    m_pImpl->m_sExtraTable = s;
}

void IxSqlRelation::init()
{
    QMutexLocker locker(&m_pImpl->m_mutex);
    if (m_pImpl->m_bInitInEvent || m_pImpl->m_bInitDone)
    {
        return;
    }
    ...
    if (m_pImpl->m_pClass)
    {
        m_pImpl->m_oSoftDelete = m_pImpl->m_pClass->getSoftDelete();
    }

#ifdef _QX_MODE_DEBUG
    QString sCheckMsg = QStringLiteral("Check relationship '%1' from '%2' to '%3'").arg(this->getKey()).arg(m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString()).arg(m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString());
    if (!m_pImpl->m_pClass)
    {
        QString sAssertMsg = sCheckMsg + QStringLiteral(" : m_pImpl->m_pClass is equal to NULL");
        qAssertMsg(false, qPrintable(sAssertMsg));
    }
    if (!m_pImpl->m_pClassOwner)
    {
        QString sAssertMsg = sCheckMsg + QStringLiteral(" : m_pImpl->m_pClassOwner is equal to NULL");
        qAssertMsg(false, qPrintable(sAssertMsg));
    }
    if (!m_pImpl->m_pDataMember)
    {
        QString sAssertMsg = sCheckMsg + QStringLiteral(" : m_pImpl->m_pDataMember is equal to NULL");
        qAssertMsg(false, qPrintable(sAssertMsg));
    }
    if (!m_pImpl->m_pDataMemberX)
    {
        QString sAssertMsg = sCheckMsg + QStringLiteral(" : m_pImpl->m_pDataMemberX is equal to NULL");
        qAssertMsg(false, qPrintable(sAssertMsg));
    }
    if (!m_pImpl->m_pDataMemberId)
    {
        QString sAssertMsg = sCheckMsg + QStringLiteral(" : m_pImpl->m_pDataMemberId is equal to NULL");
        qAssertMsg(false, qPrintable(sAssertMsg));
    }
    ...
    {
        QString sTraceMsg = QStringLiteral("Init relationship '%1' from '%2' to '%3'")
            .arg(this->getKey())
            .arg(m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString())
            .arg(m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString());
        xverbose(qPrintable(sTraceMsg));
    }
    ...
    {
        p = m_pImpl->isValid_DataMember(l); if (!p)
        {
            continue;
        }
#ifdef _QX_MODE_DEBUG
        ...
        {
            QString sDebugMsg = QStringLiteral("Relationship '%1' from '%2' to '%3' : data member '%4' already exists in the collection")
                .arg(this->getKey())
                .arg(m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString())
                .arg(m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString())
                .arg(p->getKey());
            xdebug(qPrintable(sDebugMsg));
        }
#endif // _QX_MODE_DEBUG
    ...
    for (long l = 0; l < lCount; ++l)
    {
        p = m_pImpl->isValid_SqlRelation(l); if (!p)
        {
            continue;
        }
#ifdef _QX_MODE_DEBUG
        ...
        {
            QString sDebugMsg = QStringLiteral("Relationship '%1' from '%2' to '%3' : relation '%4' already exists in the collection")
                .arg(this->getKey())
                .arg(m_pImpl->m_pClassOwner ? m_pImpl->m_pClassOwner->getKey() : QString())
                .arg(m_pImpl->m_pClass ? m_pImpl->m_pClass->getKey() : QString())
                .arg(p->getKey());
...
void IxSqlRelation::setTraceRelationInit(bool bTrace)
{
    IxSqlRelation::IxSqlRelationImpl::m_bTraceRelationInit = bTrace;
}

QString IxSqlRelation::getKey() const
{
    return (m_pImpl->m_pDataMember ? m_pImpl->m_pDataMember->getKey() : QStringLiteral(""));
}
...
{
    return (m_pImpl->m_pDataMemberX ? m_pImpl->m_pDataMemberX->getName() : QStringLiteral(""));
}
...
{
    if (m_pImpl->m_lstDataMemberPtr && m_pImpl->m_lstDataMemberPtr->exist(sKey))
    {
        return m_pImpl->m_lstDataMemberPtr->getByKey(sKey);
    }
    else if (m_pImpl->m_lstSqlRelationPtr && m_pImpl->m_lstSqlRelationPtr->exist(sKey))
    {
        return m_pImpl->m_lstSqlRelationPtr->getByKey(sKey)->getDataMember();
    }
    return NULL;
}
...
{
    if ((!m_pImpl->m_lstDataMemberPtr) || (lIndex < 0) || (lIndex >= m_pImpl->m_lstDataMemberPtr->count()))
    {
        return NULL;
    }
    ++lIndex;
...
{
    if ((!m_pImpl->m_lstSqlRelationPtr) || (lIndex < 0) || (lIndex >= m_pImpl->m_lstSqlRelationPtr->count()))
    {
        return NULL;
    }
    ++lIndex;
...
{
    if (!params.getCustomAlias().isEmpty())
    {
        return params.getCustomAlias();
    }
    QString sTableAlias = (m_pImpl->m_pDataMemberX ? (QStringLiteral("%1_%2").arg(m_pImpl->m_pDataMemberX->getName()).arg(params.index())) : QStringLiteral(""));
...
QString IxSqlRelation::tableAliasOwner(QxSqlRelationParams& params) const
{
    ...
    if (!params.getCustomAliasOwner().isEmpty())
    {
        return params.getCustomAliasOwner();
    }
    QString sTableAliasOwner = QStringLiteral("%1_%2").arg(m_pImpl->m_pClassOwner->getName()).arg(params.indexOwner());
    if (params.indexOwner() <= 0)
    {
        sTableAliasOwner = params.builder().table();
    }
    if (!params.getTableAlias().isEmpty())
    {
        sTableAliasOwner = params.getTableAlias();
    }
...
{
    QString sJoin;
    if (e == qx::dao::sql_join::no_join)
    {
        e = m_pImpl->m_eJoinType;
    }
...
{
#ifdef _QX_MODE_DEBUG
    if (!qx::QxSqlDatabase::getSingleton()->getVerifyOffsetRelation())
    {
        return true;
    }
    ...
    QString sSuffixAlias = ((!bId && (params.indexOwner() > 0)) ? QStringLiteral("_") + QString::number(params.indexOwner()) : QString());
...
QVariant IxSqlRelation::getIdFromQuery_ManyToMany(bool bEager, QxSqlRelationParams& params) const
{
    ...
    {
        sId += params.query().value(params.offset() + i).toString() + QStringLiteral("|");
    }
    return QVariant(sId);
}
...
QVariant IxSqlRelation::getIdFromQuery_ManyToOne(bool bEager, QxSqlRelationParams& params) const
{
    ...
    IxDataMember* pId = this->getDataId(); if (!pId)
    {
        return QVariant();
    }
    for (int i = 0; i < pId->getNameCount(); i++)
...
void IxSqlRelation::updateOffset_ManyToMany(bool bEager, QxSqlRelationParams& params) const
{
    ...
    else if (bEager && (params.getColumnsCount() > 0))
    {
        ...
        while ((p = this->nextData(l1)))
        {
            if (!params.checkColumns(p->getKey()))
            {
                lOffsetNew = (lOffsetNew - 1); lColumnsOffset++;
            }
        }
...
void IxSqlRelation::eagerSelect_ManyToMany(QxSqlRelationParams& params) const
{
    ...
    if (pId)
    {
        sql += (pId->getSqlTablePointNameAsAlias(tableAlias) + QStringLiteral(", "));
    }
    while ((p = this->nextData(l1)))
    {
        if (params.checkColumns(p->getKey()))
        {
            sql += (p->getSqlTablePointNameAsAlias(tableAlias) + QStringLiteral(", "));
        }
    }
    ...
    if (!this->m_pImpl->m_oSoftDelete.isEmpty())
    {
        sql += (this->m_pImpl->m_oSoftDelete.buildSqlTablePointName(tableAlias) + QStringLiteral(", "));
    }
}
...
void IxSqlRelation::eagerSelect_ManyToOne(QxSqlRelationParams& params) const
{
    ...
    if (params.indexOwner() > 0)
    {
        sSuffixAlias = QStringLiteral("_") + QString::number(params.indexOwner());
    }
    if (pData)
    {
        sql += (pData->getSqlTablePointNameAsAlias(tableRef, QStringLiteral(", "), sSuffixAlias) + QStringLiteral(", "));
    }
    if (pId)
    {
        sql += (pId->getSqlTablePointNameAsAlias(tableAlias) + QStringLiteral(", "));
...
void IxSqlRelation::eagerSelect_OneToMany(QxSqlRelationParams& params) const
{
    ...
    if (pForeign)
    {
        sql += (pForeign->getSqlTablePointNameAsAlias(tableAlias) + QStringLiteral(", "));
    }
    while ((p = this->nextData(l1)))
    {
...
void IxSqlRelation::eagerJoin_ManyToMany(QxSqlRelationParams& params) const
{
    ...
    QString sExtraTableAlias = m_pImpl->m_sExtraTable + QStringLiteral("_") + QString::number(params.index());
    sql += QStringLiteral("%1%2 %3 ON ").arg(this->getSqlJoin(params.joinType())).arg(qx::IxDataMember::getSqlTableName(m_pImpl->m_sExtraTable)).arg(sExtraTableAlias);
    for (int i = 0; i < pIdOwner->getNameCount(); i++)
    {
        sql += QStringLiteral("%1 = %2.%3 AND ").arg(pIdOwner->getSqlAlias(tableOwner, true, i)).arg(sExtraTableAlias).arg(lstForeignKeyOwner.at(i));
    }
    sql = sql.left(sql.count() - 5); // Remove last QStringLiteral(" AND ")
    ...
    sql += QStringLiteral("%1%2 %3 ON ").arg(this->getSqlJoin(params.joinType())).arg(qx::IxDataMember::getSqlTableName(table)).arg(tableAlias);
    ...
    {
        sql += QStringLiteral("%1.%2 = %3 AND ").arg(sExtraTableAlias).arg(lstForeignKeyDataType.at(i)).arg(pIdData->getSqlAlias(tableAlias, true, i));
    }
    if (!this->m_pImpl->m_oSoftDelete.isEmpty() && this->m_pImpl->m_oSoftDelete.getSqlFetchInJoin())
    {
        sql += this->m_pImpl->m_oSoftDelete.buildSqlQueryToFetch(tableAlias) + QStringLiteral(" AND ");
    }
    sql = sql.left(sql.count() - 5); // Remove last QStringLiteral(" AND ")
    if (!joinQuery.isEmpty())
    {
        sql += QStringLiteral(" %1)").arg(joinQuery);
    }
}
...
void IxSqlRelation::eagerJoin_ManyToOne(QxSqlRelationParams& params) const
{
    ...
    for (int i = 0; i < pId->getNameCount(); i++)
    {
        sql += QStringLiteral("%1 = %2 AND ").arg(pId->getSqlAlias(tableAlias, true, i)).arg(pData->getSqlAlias(tableRef, true, i));
    }
    if (!this->m_pImpl->m_oSoftDelete.isEmpty() && this->m_pImpl->m_oSoftDelete.getSqlFetchInJoin())
    {
        sql += this->m_pImpl->m_oSoftDelete.buildSqlQueryToFetch(tableAlias) + QStringLiteral(" AND ");
...
void IxSqlRelation::eagerJoin_OneToMany(QxSqlRelationParams& params) const
{
    ...
    for (int i = 0; i < pId->getNameCount(); i++)
    {
        sql += QStringLiteral("%1 = %2 AND ").arg(pForeign->getSqlAlias(tableAlias, true, i)).arg(pId->getSqlAlias(tableRef, true, i));
    }
    if (!this->m_pImpl->m_oSoftDelete.isEmpty() && this->m_pImpl->m_oSoftDelete.getSqlFetchInJoin())
    {
        sql += this->m_pImpl->m_oSoftDelete.buildSqlQueryToFetch(tableAlias) + QStringLiteral(" AND ");
...
void IxSqlRelation::eagerJoin_OneToOne(QxSqlRelationParams& params) const
{
    ...
    for (int i = 0; i < pId->getNameCount(); i++)
    {
        sql += QStringLiteral("%1 = %2 AND ").arg(pId->getSqlAlias(tableAlias, true, i)).arg(pIdRef->getSqlAlias(tableRef, true, i));
    }
    if (!this->m_pImpl->m_oSoftDelete.isEmpty() && this->m_pImpl->m_oSoftDelete.getSqlFetchInJoin())
    {
        sql += this->m_pImpl->m_oSoftDelete.buildSqlQueryToFetch(tableAlias) + QStringLiteral(" AND ");
...
void IxSqlRelation::eagerWhereSoftDelete_ManyToMany(QxSqlRelationParams& params) const
{
    ...
    if (this->m_pImpl->m_oSoftDelete.getSqlFetchInJoin())
    {
        return;
    }
    QString& sql = params.sql();
...
void IxSqlRelation::lazyInsert_ManyToOne(QxSqlRelationParams& params) const
{
    ...
    if (pData)
    {
        sql += pData->getSqlName(QStringLiteral(", ")) + QStringLiteral(", ");
    }
}
...
void IxSqlRelation::lazyInsert_Values_ManyToOne(QxSqlRelationParams& params) const
{
    ...
    if (pData)
    {
        sql += pData->getSqlPlaceHolder(QStringLiteral(""), -1, QStringLiteral(", ")) + QStringLiteral(", ");
    }
}
...
void IxSqlRelation::lazyUpdate_ManyToOne(QxSqlRelationParams& params) const
{
    ...
    if (pData)
    {
        sql += pData->getSqlNameEqualToPlaceHolder(QStringLiteral(""), QStringLiteral(", ")) + QStringLiteral(", ");
    }
}
...
void IxSqlRelation::createTable_ManyToOne(QxSqlRelationParams& params) const
{
    ...
    if (pData)
    {
        sql += pData->getSqlNameAndTypeAndParams(QStringLiteral(", ")) + QStringLiteral(", "); qAssert(!pData->getSqlType().isEmpty());
    }
}
...
QString IxSqlRelation::createExtraTable_ManyToMany() const
{
    ...
    QString sql = QStringLiteral("CREATE TABLE IF NOT EXISTS %1 (").arg(this->m_pImpl->m_sExtraTable);
    sql += pIdOwner->getSqlNameAndTypeAndParams(QStringLiteral(", "), this->m_pImpl->m_sForeignKeyOwner) + QStringLiteral(", ");
    qAssert(!pIdOwner->getSqlType().isEmpty());
    sql += pIdData->getSqlNameAndTypeAndParams(QStringLiteral(", "), this->m_pImpl->m_sForeignKeyDataType) + QStringLiteral(", ");
    qAssert(!pIdData->getSqlType().isEmpty());
    ...
    {
        xinfo("create extra-table (relation many-to-many) : {}", qPrintable(sql));
    }
    return sql;
}
...
{
    IxDataMember* pIdOwner = this->getDataIdOwner(); qAssert(pIdOwner);
    QString sql = QStringLiteral("DELETE FROM %1 WHERE ").arg(this->m_pImpl->m_sExtraTable);
    QStringList lstForeignKeyOwner = this->m_pImpl->m_sForeignKeyOwner.split(QStringLiteral("|"));
    qAssert(pIdOwner->getNameCount() == lstForeignKeyOwner.count());
    for (int i = 0; i < pIdOwner->getNameCount(); i++)
    {
        sql += QStringLiteral("%1.%2 = %3 AND ").arg(this->m_pImpl->m_sExtraTable).arg(lstForeignKeyOwner.at(i)).arg(pIdOwner->getSqlPlaceHolder(QStringLiteral(""), i));
    }
    sql = sql.left(sql.count() - 5); // Remove last QStringLiteral(" AND ")
    if (this->traceSqlQuery())
    {
        xinfo("sql query (extra-table) : {}", qPrintable(sql));
    }

    QSqlQuery queryDelete(params.database());
    if (!queryDelete.prepare(sql))
    {
        return queryDelete.lastError();
    }
...
{
    if (!params.relationX() || !pRelation || !params.checkColumns(pRelation->getKey()))
    {
        return false;
    }
