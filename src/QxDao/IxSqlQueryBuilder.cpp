...
IxDataMemberX* IxSqlQueryBuilder::getDataMemberX() const
{
    return m_pImpl->m_pDataMemberX;
}

QxCollection<QString, IxDataMember*>* IxSqlQueryBuilder::getLstDataMember() const
{
    return m_pImpl->m_lstDataMemberPtr.get();
}

IxSqlRelationX* IxSqlQueryBuilder::getLstRelation() const
{
    return m_pImpl->m_lstSqlRelationPtr.get();
}

qx::dao::detail::IxDao_Helper* IxSqlQueryBuilder::getDaoHelper() const
{
    return m_pImpl->m_pDaoHelper;
}

void IxSqlQueryBuilder::setDaoHelper(qx::dao::detail::IxDao_Helper* p)
{
    m_pImpl->m_pDaoHelper = p;
}

void IxSqlQueryBuilder::setHashRelation(const QString& s)
{
    m_pImpl->m_sHashRelation = s;
}

void IxSqlQueryBuilder::setCartesianProduct(bool b)
{
    m_pImpl->m_bCartesianProduct = b;
}

QString IxSqlQueryBuilder::getSqlQuery() const
{
    return m_pImpl->m_sSqlQuery;
}

QString IxSqlQueryBuilder::getHashRelation() const
{
    return m_pImpl->m_sHashRelation;
}

QString IxSqlQueryBuilder::table() const
{
    return m_pImpl->m_sTableName;
}

QxSoftDelete IxSqlQueryBuilder::getSoftDelete() const
{
    return m_pImpl->m_oSoftDelete;
}

bool IxSqlQueryBuilder::getCartesianProduct() const
{
    return m_pImpl->m_bCartesianProduct;
}

long IxSqlQueryBuilder::getDataCount() const
{
    return (m_pImpl->m_lstDataMemberPtr ? m_pImpl->m_lstDataMemberPtr->count() : 0);
}

long IxSqlQueryBuilder::getRelationCount() const
{
    return (m_pImpl->m_lstSqlRelationPtr ? m_pImpl->m_lstSqlRelationPtr->count() : 0);
}

IxDataMember* IxSqlQueryBuilder::getDataId() const
{
    return m_pImpl->m_pDataMemberId;
}

IxDataMember* IxSqlQueryBuilder::nextData(long& l) const
{
    if ((!m_pImpl->m_lstDataMemberPtr) || (l < 0) || (l >= m_pImpl->m_lstDataMemberPtr->count()))
    {
        return NULL;
    }; ++l; return m_pImpl->m_lstDataMemberPtr->getByIndex(l - 1);
}

IxSqlRelation* IxSqlQueryBuilder::nextRelation(long& l) const
{
    if ((!m_pImpl->m_lstSqlRelationPtr) || (l < 0) || (l >= m_pImpl->m_lstSqlRelationPtr->count()))
    {
        return NULL;
    }; ++l; return m_pImpl->m_lstSqlRelationPtr->getByIndex(l - 1);
}

bool IxSqlQueryBuilder::isInitDone() const
{
    return m_pImpl->m_bInitDone;
}

QxSoftDelete& IxSqlQueryBuilder::softDelete()
{
    return m_pImpl->m_oSoftDelete;
}

const QxSoftDelete& IxSqlQueryBuilder::softDelete() const
{
    return m_pImpl->m_oSoftDelete;
}

void IxSqlQueryBuilder::setSoftDelete(const QxSoftDelete& o)
{
    m_pImpl->m_oSoftDelete = o;
}

void IxSqlQueryBuilder::setDataMemberX(IxDataMemberX* p)
{
...
{
    if (!m_pImpl->m_pDataMemberX || !m_pImpl->m_pDataMemberX->getClass() || m_pImpl->m_bInitDone)
    {
        return;
    }
...
bool IxSqlQueryBuilder::findSqlAlias(const QString& key)
{
    ...
    QMutexLocker locker(&IxSqlQueryBuilderImpl::m_mutex);
    if (!IxSqlQueryBuilderImpl::m_lstSqlAlias.contains(key))
    {
        return false;
    }
...
bool IxSqlQueryBuilder::insertIdX(long lIndex, const QVariant& idOwner, const QVariant& idData, void* ptr)
{
    ...
    if (!m_pImpl->m_pIdX || sIdOwner.isEmpty() || sIdData.isEmpty())
    {
        qAssert(false); return false;
    }
    if ((lIndex < 0) || (lIndex >= m_pImpl->m_pIdX->count()))
    {
        qAssert(false); return false;
    }
    ...
    if (!ptr || !pHash || pHash->contains(idX))
    {
        qAssert(false); return false;
    }
...
{
    if (!qx::QxSqlDatabase::getSingleton()->getAutoReplaceSqlAliasIntoQuery())
    {
        return;
    }
    if (m_pImpl->m_lstSqlQueryAlias.count() <= 0)
    {
        return;
    }
    QHashIterator<QString, QString> itr(m_pImpl->m_lstSqlQueryAlias);
    sql = (QStringLiteral(" ") + sql);
    while (itr.hasNext())
    {
        itr.next();
        QString sBefore = (QStringLiteral(" %1.").arg(itr.key()));
        QString sAfter = (QStringLiteral(" %1.").arg(itr.value()));
...
bool IxSqlQueryBuilder::verifyColumns(const QStringList& columns) const
{
    ...
    for (int i = 0; i < columns.count(); i++)
    {
        ...
        {
            QString sErrorMsg = QStringLiteral("column '%1' not found in table '%2'").arg(columns.at(i), m_pImpl->m_sTableName);
            xerror(qPrintable(sErrorMsg));
            qAssertMsg(false, qPrintable(sErrorMsg));
            return false;
        }
    }
    return true;
...
void IxSqlQueryBuilder::sql_CreateTable(QString& sql, IxSqlQueryBuilder& builder)
{
    ...
    sql = QStringLiteral("CREATE TABLE %1 (").arg(qx::IxDataMember::getSqlTableName(table));
    ...
    {
        params.setIndex(l2); pRelation->createTable(params);
    }
...
void IxSqlQueryBuilder::sql_DeleteById(QString& sql, IxSqlQueryBuilder& builder, bool bSoftDelete)
{
    ...
    qx::QxSoftDelete oSoftDelete = builder.getSoftDelete();
    QString table = builder.table();
    if (bSoftDelete && !oSoftDelete.isEmpty())
    {
        sql = QStringLiteral("UPDATE %1 SET %2").arg(qx::IxDataMember::getSqlTableName(table)).arg(oSoftDelete.buildSqlQueryToUpdate());
    }
    else
    {
        sql = QStringLiteral("DELETE FROM ") + qx::IxDataMember::getSqlFromTable(table);
    }
    sql += IxSqlQueryBuilder::addSqlCondition(sql);
    sql += pId->getSqlNameEqualToPlaceHolder(QStringLiteral(""), QStringLiteral(" AND "));
}
...
void IxSqlQueryBuilder::sql_Exist(QString& sql, IxSqlQueryBuilder& builder)
{
    ...
    if (!oSoftDelete.isEmpty())
    {
        sql += QStringLiteral(", ") + oSoftDelete.buildSqlTablePointName();
    }
    sql += QStringLiteral(" FROM ") + qx::IxDataMember::getSqlFromTable(table);
    sql += QStringLiteral(" WHERE ") + pId->getSqlAliasEqualToPlaceHolder(table, true);
    if (!oSoftDelete.isEmpty())
    {
        sql += QStringLiteral(" AND ") + oSoftDelete.buildSqlQueryToFetch();
    }
}
...
void IxSqlQueryBuilder::sql_FetchAll(QString& sql, IxSqlQueryBuilder& builder)
{
    ...
    if (pId)
    {
        sql += (pId->getSqlTablePointNameAsAlias(table) + QStringLiteral(", "));
    }
    while ((p = builder.nextData(l1)))
    {
        sql += (p->getSqlTablePointNameAsAlias(table) + QStringLiteral(", "));
    }
    if (!oSoftDelete.isEmpty())
    {
        l1++; sql += (oSoftDelete.buildSqlTablePointName() + QStringLiteral(", "));
    }
    while ((pRelation = builder.nextRelation(l2)))
    {
        params.setIndex(l2); pRelation->lazySelect(params);
    }
    sql = sql.left(sql.count() - 2); // Remove last ", "
    sql += QStringLiteral(" FROM ") + qx::IxDataMember::getSqlFromTable(table) + QStringLiteral(", ");
    while ((pRelation = builder.nextRelation(l3)))
    {
        params.setIndex(l3); pRelation->lazyFrom(params);
    }
    sql = sql.left(sql.count() - 2); // Remove last ", "
    while ((pRelation = builder.nextRelation(l4)))
    {
        params.setIndex(l4); pRelation->lazyJoin(params);
    }
    if (!oSoftDelete.isEmpty())
    {
        sql += QStringLiteral(" WHERE ") + oSoftDelete.buildSqlQueryToFetch();
    }
    while ((pRelation = builder.nextRelation(l5)))
    {
        params.setIndex(l5); pRelation->lazyWhereSoftDelete(params);
    }
}
...
void IxSqlQueryBuilder::sql_FetchAll(QString& sql, IxSqlQueryBuilder& builder, const QStringList& columns)
{
    ...
    for (int i = 0; i < columns.count(); i++)
    {
        p = pDataMemberX->get_WithDaoStrategy(columns.at(i)); if (p && (p != pId))
        {
            sql += (p->getSqlTablePointNameAsAlias(table) + QStringLiteral(", "));
        }
    }
...
void IxSqlQueryBuilder::sql_FetchAll_WithRelation(qx::QxSqlRelationLinked* pRelationX, QString& sql, IxSqlQueryBuilder& builder)
{
    ...
    if (pId)
    {
        sql += (pId->getSqlTablePointNameAsAlias(table, QStringLiteral(", "), QStringLiteral(""), false, pRelationX->getRootCustomAlias()) + QStringLiteral(", "));
    }
    while ((p = builder.nextData(l)))
    {
        if (pRelationX->checkRootColumns(p->getKey()))
        {
            sql += (p->getSqlTablePointNameAsAlias(table, QStringLiteral(", "), QStringLiteral(""), false, pRelationX->getRootCustomAlias()) + QStringLiteral(", "));
        }
    }
    if (!oSoftDelete.isEmpty())
    {
        l++; sql += (oSoftDelete.buildSqlTablePointName(pRelationX->getRootCustomAlias()) + QStringLiteral(", "));
    }
    pRelationX->hierarchySelect(params);
    sql = sql.left(sql.count() - 2); // Remove last ", "
    sql += QStringLiteral(" FROM ") + qx::IxDataMember::getSqlFromTable(table, pRelationX->getRootCustomAlias()) + QStringLiteral(", ");
    pRelationX->hierarchyFrom(params);
    sql = sql.left(sql.count() - 2); // Remove last ", "
    pRelationX->hierarchyJoin(params);
    if (!oSoftDelete.isEmpty())
    {
        sql += QStringLiteral(" WHERE ") + oSoftDelete.buildSqlQueryToFetch(pRelationX->getRootCustomAlias());
    }
...
void IxSqlQueryBuilder::sql_Insert(QString& sql, IxSqlQueryBuilder& builder)
{
    ...
    sql = QStringLiteral("INSERT INTO ") + qx::IxDataMember::getSqlTableName(table) + QStringLiteral(" (");
    if (pId && !pId->getAutoIncrement())
    {
        tmp = pId->getSqlName(QStringLiteral(", "), QStringLiteral(""), true); if (!tmp.isEmpty())
        {
            sql += tmp + QStringLiteral(", ");
        }
    }
    while ((p = builder.nextData(l1)))
    {
        sql += p->getSqlName(QStringLiteral(", ")) + QStringLiteral(", ");
    }
    while ((pRelation = builder.nextRelation(l2)))
    {
        params.setIndex(l2); pRelation->lazyInsert(params);
    }
    sql = sql.left(sql.count() - 2); // Remove last ", "
    ...
    if (pId && !pId->getAutoIncrement())
    {
        tmp = pId->getSqlPlaceHolder(QStringLiteral(""), -1, QStringLiteral(", "), QStringLiteral(""), true); if (!tmp.isEmpty())
        {
            sql += tmp + QStringLiteral(", ");
        }
    }
    while ((p = builder.nextData(l1)))
    {
        sql += p->getSqlPlaceHolder(QStringLiteral(""), -1, QStringLiteral(", ")) + QStringLiteral(", ");
    }
    while ((pRelation = builder.nextRelation(l2)))
    {
        params.setIndex(l2); pRelation->lazyInsert_Values(params);
    }
    sql = sql.left(sql.count() - 2); // Remove last ", "
    sql += QStringLiteral(")");
}
...
void IxSqlQueryBuilder::sql_Update(QString& sql, IxSqlQueryBuilder& builder)
{
    ...
    qx::QxSqlRelationParams params(0, 0, (&sql), (&builder), NULL, NULL);
    QString table = builder.table(); QString tmp;
    sql = QStringLiteral("UPDATE %1 SET ").arg(qx::IxDataMember::getSqlTableName(table));
    if (!pId->getAutoIncrement() || (pId->getAutoIncrement() && builder.getAddAutoIncrementIdToUpdateQuery()))
    {
        tmp = pId->getSqlNameEqualToPlaceHolder(QStringLiteral(""), QStringLiteral(", "), true); if (!tmp.isEmpty())
        {
            sql += tmp + QStringLiteral(", ");
        }
    }
    while ((p = builder.nextData(l1)))
    {
        sql += p->getSqlNameEqualToPlaceHolder(QStringLiteral(""), QStringLiteral(", ")) + QStringLiteral(", ");
    }
    while ((pRelation = builder.nextRelation(l2)))
    {
        params.setIndex(l2); pRelation->lazyUpdate(params);
    }
    sql = sql.left(sql.count() - 2); // Remove last ", "
    sql += QStringLiteral(" WHERE ") + pId->getSqlNameEqualToPlaceHolder(QStringLiteral("_bis"), QStringLiteral(" AND "));
}
...
void IxSqlQueryBuilder::sql_Update(QString& sql, IxSqlQueryBuilder& builder, const QStringList& columns)
{
    ...
    qx::IxDataMemberX* pDataMemberX = builder.getDataMemberX(); qAssert(pDataMemberX);
    QString table = builder.table();
    sql = QStringLiteral("UPDATE ") + qx::IxDataMember::getSqlTableName(table) + QStringLiteral(" SET ");
    if (!pId->getAutoIncrement() || (pId->getAutoIncrement() && builder.getAddAutoIncrementIdToUpdateQuery()))
    {
        sql += pId->getSqlNameEqualToPlaceHolder(QStringLiteral(""), QStringLiteral(", ")) + QStringLiteral(", ");
    }
    for (int i = 0; i < columns.count(); i++)
    {
        p = pDataMemberX->get_WithDaoStrategy(columns.at(i)); if (p && (p != pId))
...
void IxSqlQueryBuilder::resolveOutput_FetchAll(void* t, QSqlQuery& query, IxSqlQueryBuilder& builder)
{
    ...
    if (pId)
    {
        ...
        {
            pId->fromVariant(t, query.value(i), i, qx::cvt::context::e_database);
        }
    }
    while ((p = builder.nextData(l1)))
    {
        p->fromVariant(t, query.value(l1 + iOffset - 1), -1, qx::cvt::context::e_database);
    }
    iOffset = (builder.getDataCount() + iOffset + (oSoftDelete.isEmpty() ? 0 : 1));
    qx::QxSqlRelationParams params(0, iOffset, NULL, (&builder), (&query), t);
    while ((pRelation = builder.nextRelation(l2)))
    {
        params.setIndex(l2); pRelation->lazyFetch_ResolveOutput(params);
    }
}
...
void IxSqlQueryBuilder::resolveOutput_FetchAll(void* t, QSqlQuery& query, IxSqlQueryBuilder& builder, const QStringList& columns)
{
    ...
    for (int i = 0; i < columns.count(); i++)
    {
        p = pDataMemberX->get_WithDaoStrategy(columns.at(i)); if (p && (p != pId))
        {
            p->fromVariant(t, query.value(idx + iOffset), -1, qx::cvt::context::e_database); idx++;
        }
    }
}
...
void IxSqlQueryBuilder::resolveOutput_FetchAll_WithRelation(qx::QxSqlRelationLinked* pRelationX, void* t, QSqlQuery& query, IxSqlQueryBuilder& builder)
{
    ...
    if (pId)
    {
        ...
        {
            sId += query.value(i).toString() + QStringLiteral("|");
        }; vId = sId;
    }
    ...
    if (!bByPass)
    {
        ...
        while ((p = builder.nextData(l)))
        {
            if (pRelationX->checkRootColumns(p->getKey()))
            {
                p->fromVariant(t, query.value(lCurrIndex + iOffsetId), -1, qx::cvt::context::e_database); lCurrIndex++;
            }
        }
        if (bComplex)
        {
            builder.insertIdX(0, vId, vId, t);
        }
    }
    ...
    else if (pRelationX->getRootColumnsCount() > 0)
    {
        ...
        while ((p = builder.nextData(l)))
        {
            if (!pRelationX->checkRootColumns(p->getKey()))
            {
                iOffset = (iOffset - 1); lRootColumnsOffset++;
            }
        }
...
void IxSqlQueryBuilder::resolveInput_Insert(void* t, QSqlQuery& query, IxSqlQueryBuilder& builder)
{
    ...
    if (pId && !pId->getAutoIncrement())
    {
        pId->setSqlPlaceHolder(query, t, QStringLiteral(""), QStringLiteral(""), true);
    }
    while ((p = builder.nextData(l1)))
    {
        p->setSqlPlaceHolder(query, t);
    }
    while ((pRelation = builder.nextRelation(l2)))
    {
        params.setIndex(l2); pRelation->lazyInsert_ResolveInput(params);
    }
}
...
void IxSqlQueryBuilder::resolveInput_Update(void* t, QSqlQuery& query, IxSqlQueryBuilder& builder)
{
    ...
    while ((pRelation = builder.nextRelation(l2)))
    {
        params.setIndex(l2); pRelation->lazyUpdate_ResolveInput(params);
    }
