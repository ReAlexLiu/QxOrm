...
qx_bool QxConvert_FromJson_Helper(const QJsonValue& j, QSqlError& t, const QString& format)
{
    ...
    if (!j.isArray())
    {
        t = QSqlError(); return qx_bool(true);
    }
    QJsonArray arr = j.toArray();
    t = QSqlError(arr.at(1).toString(), arr.at(0).toString(), static_cast<QSqlError::ErrorType>(qRound(arr.at(3).toDouble())), QString::number(qRound(arr.at(2).toDouble())));
    return qx_bool(true);
}
