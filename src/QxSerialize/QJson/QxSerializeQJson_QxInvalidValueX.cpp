...
{
    if (t.m_lstInvalidValues.count() <= 0)
    {
        return QJsonValue();
    }
...
qx_bool QxConvert_FromJson_Helper(const QJsonValue& j, qx::QxInvalidValueX& t, const QString& format)
{
    t = qx::QxInvalidValueX();
    if (!j.isArray())
    {
        return qx_bool(true);
    }
    QJsonArray arr = j.toArray();
    t.m_sCurrentPath = arr.at(0).toString();
    qx::cvt::from_json(arr.at(1), t.m_lstInvalidValues, format);
    return qx_bool(true);
}
