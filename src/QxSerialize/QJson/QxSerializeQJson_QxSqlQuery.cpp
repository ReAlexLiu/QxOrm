...
QJsonValue QxConvert_ToJson_Helper(const qx::QxSqlQuery& t, const QString& format)
{
    ...
    obj.insert(QStringLiteral("list_join_query_user"), qx::cvt::to_json(t.m_lstJoinQueryUser, format));
    obj.insert(QStringLiteral("list_join_query_resolve"), qx::cvt::to_json(t.m_lstJoinQueryToResolve, format));
    ...
    Q_FOREACH(qx::dao::detail::IxSqlElement_ptr pSqlElement, t.m_lstSqlElement)
    {
        ...
        if (!pSqlElement)
        {
            item.insert(QStringLiteral("sql_element_type"), QJsonValue(static_cast<int>(eNoSqlType))); arr.append(QJsonValue(item)); continue;
        }
        qx::dao::detail::IxSqlElement::type_class eTypeSqlElement = pSqlElement->getTypeClass();
        item.insert(QStringLiteral("sql_element_type"), QJsonValue(static_cast<int>(eTypeSqlElement)));
        item.insert(QStringLiteral("sql_element"), qx::cvt::to_json((*pSqlElement), format));
...
qx_bool QxConvert_FromJson_Helper(const QJsonValue& j, qx::QxSqlQuery& t, const QString& format)
{
    ...
    if (obj.contains(QStringLiteral("sql")))
    {
        ...
        if (jsonSql.isString())
        {
            t.m_sQuery.clear(); t.m_sQuery << jsonSql.toString();
        }
    }
    else
    {
    ...
    if (obj.contains(QStringLiteral("params")))
    {
        ...
        {
            QJsonValue objParam = arrParams.at(i); if (!objParam.isObject())
            {
                continue;
            }
            ...
            if (jsonParam.contains(QStringLiteral("type")))
            {
                ...
                if (paramType == QStringLiteral("in"))
                {
                    jsonParamType = QSql::In;
                }
                else if (paramType == QStringLiteral("out"))
                {
                    jsonParamType = QSql::Out;
                }
                else if (paramType == QStringLiteral("in_out"))
                {
                    jsonParamType = QSql::InOut;
                }
                else if (paramType == QStringLiteral("binary"))
                {
            ...
            if (jsonParamKey.isEmpty())
            {
                t.bind(jsonParamValue, jsonParamType);
            }
            else
            {
                t.bind(jsonParamKey, jsonParamValue, jsonParamType);
            }
        }
    }
    else
    ...
    qx::cvt::from_json(obj.value(QStringLiteral("result_position_by_key")), lstResultPosByKey, format);
    ...
    qx::cvt::from_json(obj.value(QStringLiteral("list_join_query_user")), t.m_lstJoinQueryUser, format);
    qx::cvt::from_json(obj.value(QStringLiteral("list_join_query_resolve")), t.m_lstJoinQueryToResolve, format);
    ...
    eTypeSqlElement = static_cast<qx::dao::detail::IxSqlElement::type_class>(qRound(obj.value(QStringLiteral("sql_element_temp_type")).toDouble()));
    if (eTypeSqlElement != qx::dao::detail::IxSqlElement::_no_type)
    {
        ...
        if (t.m_pSqlElementTemp)
        {
            qx::cvt::from_json(obj.value(QStringLiteral("sql_element_temp")), (*t.m_pSqlElementTemp), format);
        }
    }
    ...
    {
        QJsonValue val = arr.at(i); if (!val.isObject())
        {
            continue;
        }
        ...
        eTypeSqlElement = static_cast<qx::dao::detail::IxSqlElement::type_class>(qRound(item.value(QStringLiteral("sql_element_type")).toDouble()));
        if (eTypeSqlElement != qx::dao::detail::IxSqlElement::_no_type)
        {
            pSqlElement = qx::dao::detail::create_sql_element(eTypeSqlElement); qAssert(pSqlElement);
            if (pSqlElement)
            {
                qx::cvt::from_json(item.value(QStringLiteral("sql_element")), (*pSqlElement), format);
            }
        }
