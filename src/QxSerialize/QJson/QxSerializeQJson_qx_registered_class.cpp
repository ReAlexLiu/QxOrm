...
QJsonValue QxSerializeJsonRegistered_Helper::save(IxClass* pClass, const void* pOwner, const QString& format)
{
    if (!pClass || !pOwner)
    {
        qAssert(false); return QJsonValue();
    }
    bool bOnlyId = ((!format.isEmpty()) && ((format == QX_JSON_SERIALIZE_ONLY_ID) || (format == QStringLiteral("mongodb:only_id")) || (format == QStringLiteral("mongodb:relation_id"))));
    bool bCheckInstance = qx::serialization::helper::QxSerializeCheckInstance::contains(pOwner, pClass);
    QJsonObject obj;

    if (bCheckInstance || bOnlyId)
    {
        qx::IxDataMember* pId = pClass->getId(true); if (!pId)
        {
            return QJsonValue();
        }
        QString key = ((format == QStringLiteral("mongodb:only_id")) ? QStringLiteral("_id") : pId->getKey());
        QJsonValue val = pId->toJson(pOwner, format);
        if (format == QStringLiteral("mongodb:relation_id"))
        {
            return val;
        }
    ...
    bool bMongoDB = format.startsWith(QStringLiteral("mongodb"));
    bool bWithFilter = format.startsWith(QStringLiteral("filter:"));
    qx_bool bHierarchyOk = (bWithFilter ? QxSerializeJsonRegistered_initHierarchy_WithFilter(pClass, pOwner, format) : qx_bool(true));
    if (!bHierarchyOk)
    {
        obj.insert(QStringLiteral("error"), bHierarchyOk.getDesc()); return QJsonValue(obj);
    }
    ...
    do
    {
        if (bMongoDB)
        {
            qx::cvt::detail::QxSerializeJsonRegistered_saveHelper_MongoDB(obj, pClass, pOwner, format);
        }
        else if (bWithFilter)
        {
            qx::cvt::detail::QxSerializeJsonRegistered_saveHelper_WithFilter(obj, pClass, pOwner, format); break;
        }
        else
        {
            qx::cvt::detail::QxSerializeJsonRegistered_saveHelper(obj, pClass, pOwner, format);
        }
...
qx_bool QxSerializeJsonRegistered_Helper::load(const QJsonValue& j, IxClass* pClass, void* pOwner, const QString& format)
{
    if (!pClass || !pOwner)
    {
        qAssert(false); return qx_bool(true);
    }
    if (!j.isObject())
    {
        qx::IxDataMember* pId = pClass->getId(true); if (!pId)
    ...
    do
    {
        if (bMongoDB)
        {
            qx::cvt::detail::QxSerializeJsonRegistered_loadHelper_MongoDB(obj, pClass, pOwner, format);
        }
        else if (bWithFilter)
        {
            qx::cvt::detail::QxSerializeJsonRegistered_loadHelper_WithFilter(obj, pClass, pOwner, format); break;
        }
        else
        {
            qx::cvt::detail::QxSerializeJsonRegistered_loadHelper(obj, pClass, pOwner, format);
        }
        pClass = pClass->getBaseClass();
    } while (pClass != NULL);
...
qx_bool QxSerializeJsonRegistered_initHierarchy_WithFilter(IxClass* pClass, const void* pOwner, const QString& format)
{
    ...
    if (!qx::serialization::helper::QxSerializeCheckInstance::isRoot())
    {
        return bHierarchyOk;
    }
    std::shared_ptr<qx::QxSqlRelationLinked> pRelationLinked = qx::QxSqlRelationLinked::getHierarchy(pClass, format.right(format.size() - 7).split(QStringLiteral("|")), bHierarchyOk);
    if (!bHierarchyOk || !pRelationLinked)
    {
        return qx_bool(false, bHierarchyOk.getDesc());
    }
...
void QxSerializeJsonRegistered_saveHelper(QJsonObject& obj, IxClass* pClass, const void* pOwner, const QString& format)
{
    qx::IxDataMemberX* pDataMemberX = (pClass ? pClass->getDataMemberX() : NULL); if (!pDataMemberX)
    {
        return;
    }
    ...
    for (long l = 0; l < pDataMemberX->count(); l++)
    {
        ...
        if (!pDataMember || !pDataMember->getSerialize())
        {
            continue;
        }
...
void QxSerializeJsonRegistered_saveHelper_MongoDB(QJsonObject& obj, IxClass* pClass, const void* pOwner, const QString& format)
{
    ...
    bool bMongoDBColumns = (format.contains(QStringLiteral(":columns{")) && format.contains(QStringLiteral("}")));
    bool bMongoDBChild = format.startsWith(QStringLiteral("mongodb:child"));

    for (long l = 0; l < pDataMemberX->count(); l++)
    {
        qx::IxDataMember* pDataMember = pDataMemberX->get(l);
        if (!pDataMember || !pDataMember->getSerialize() || !pDataMember->getDao())
        {
            continue;
        }
        if (bMongoDBColumns && !pDataMember->getIsPrimaryKey() && !format.contains(QStringLiteral(",%1,").arg(pDataMember->getKey())))
        {
            continue;
        }
        ...
        if ((eRelationType == qx::IxSqlRelation::one_to_many) || (eRelationType == qx::IxSqlRelation::many_to_many))
        {
            continue;
        }
        QString key = ((!bMongoDBChild && pDataMember->getIsPrimaryKey()) ? QStringLiteral("_id") : pDataMember->getKey());
        QString formatTmp = (pRelation ? QStringLiteral("mongodb:relation_id") : (QStringLiteral("mongodb:child:") + format));
        if (pDataMember->getIsPrimaryKey())
        {
            QVariant id = pDataMember->toVariant(pOwner); if (!qx::trait::is_valid_primary_key(id))
            {
                continue;
            }
        }
...
void QxSerializeJsonRegistered_saveHelper_WithFilter(QJsonObject& obj, IxClass* pClass, const void* pOwner, const QString& format)
{
    ...
    if (!pRelationLinked || !pClass)
    {
        QString msg = (QStringLiteral("hierarchy (relations linked) not found : ") + sRelation);
        qAssertMsg(false, qPrintable(msg));
        return;
    }
    ...
    if (pDataMemberId)
    {
        QJsonValue val = pDataMemberId->toJson(pOwner, format); obj.insert(pDataMemberId->getKey(), val);
    }

    if (pRelationLinked->isRoot() && sRelation.isEmpty())
    {
        std::shared_ptr<qx::QxCollection<QString, qx::IxDataMember*> > pDataMemberX = pClass->getSqlDataMemberX(); if (!pDataMemberX)
        {
            return;
        }
        for (long l = 0; l < pDataMemberX->count(); l++)
        {
            ...
            if (!pRelationLinked->checkRootColumns(pDataMember->getKey()))
            {
                continue;
            }
            QJsonValue val = pDataMember->toJson(pOwner, format);
        ...
        for (auto itr = pRelations.begin(); itr != pRelations.end(); ++itr)
        {
            ...
            qx::IxDataMember* pDataMember = pRelation->getDataMember(); if (!pDataMember || !pDataMember->getSerialize())
            {
                continue;
            }
    ...
    else if (pRelations.exist(sRelation))
    {
        ...
        qx::IxSqlRelation* pRelation = std::get<1>(currRelation); if (!pRelation)
        {
            return;
        }
        ...
        while ((pDataMember = pRelation->nextData(l)))
        {
            ...
            if ((columns.count() > 0) && (!columns.contains(pDataMember->getKey())))
            {
                continue;
            }
            QJsonValue val = pDataMember->toJson(pOwner, format);
        ...
        qx::QxSqlRelationLinked::type_ptr pSubRelationLinked = allSubRelationsLinked.value(pRelation->getKey()); if (!pSubRelationLinked)
        {
            return;
        }
        ...
        {
            const qx::QxSqlRelationLinked::type_relation& item = itr->second;
            qx::IxSqlRelation* pSubRelation = std::get<1>(item); if (!pSubRelation)
            {
                continue;
            }
            pDataMember = pSubRelation->getDataMember(); if (!pDataMember || !pDataMember->getSerialize())
            {
                continue;
            }
    ...
    {
        qAssertMsg(false, "unknown error");
    }
}
...
void QxSerializeJsonRegistered_loadHelper(const QJsonObject& obj, IxClass* pClass, void* pOwner, const QString& format)
{
    ...
    for (long l = 0; l < pDataMemberX->count(); l++)
    {
        ...
        if (obj.contains(key))
        {
            pDataMember->fromJson(pOwner, obj.value(key), format);
        }
    }
}
...
void QxSerializeJsonRegistered_loadHelper_MongoDB(const QJsonObject& obj, IxClass* pClass, void* pOwner, const QString& format)
{
    ...
    for (long l = 0; l < pDataMemberX->count(); l++)
    {
        ...
        QString key = ((!bMongoDBChild && pDataMember->getIsPrimaryKey()) ? QStringLiteral("_id") : pDataMember->getKey());
        key = ((bMongoDBChild && pDataMember->getIsPrimaryKey() && (!obj.contains(key))) ? QStringLiteral("_id") : key);
        if (obj.contains(key))
        {
            pDataMember->fromJson(pOwner, obj.value(key), (QStringLiteral("mongodb:child:") + format));
        }
    }
}
...
void QxSerializeJsonRegistered_loadHelper_WithFilter(const QJsonObject& obj, IxClass* pClass, void* pOwner, const QString& format)
{
    ...
    if (!bHierarchyOk)
    {
        QString msg = bHierarchyOk.getDesc();
        xerror("!!! Error in QxSerializeJsonRegistered_loadHelper_WithFilter !!! : '{}'", qPrintable(msg)); return;
    }
    qx::serialization::helper::QxSerializeCheckInstance checker(pOwner, pClass);
    Q_UNUSED(checker);
    ...
    qx::IxDataMember* pDataMemberId = pClass->getId(true);
    if (pDataMemberId && obj.contains(pDataMemberId->getKey()))
    {
        pDataMemberId->fromJson(pOwner, obj.value(pDataMemberId->getKey()), format);
    }
    ...
    if (pRelationLinked->isRoot() && sRelation.isEmpty())
    {
        ...
        for (long l = 0; l < pDataMemberX->count(); l++)
        {
            ...
            if (obj.contains(pDataMember->getKey()))
            {
                pDataMember->fromJson(pOwner, obj.value(pDataMember->getKey()), format);
            }
        }
