...
QJsonValue QxConvert_ToJson_Helper(const QPixmap& t, const QString& format)
{
    if (t.isNull())
    {
        return QJsonValue();
...
qx_bool QxConvert_FromJson_Helper(const QJsonValue& j, QPixmap& t, const QString& format)
{
    t = QPixmap();
    if (j.isNull())
    {
