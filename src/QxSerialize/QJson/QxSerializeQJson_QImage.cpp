...
QJsonValue QxConvert_ToJson_Helper(const QImage& t, const QString& format)
{
    if (t.isNull())
    {
        return QJsonValue();
...
qx_bool QxConvert_FromJson_Helper(const QJsonValue& j, QImage& t, const QString& format)
{
    t = QImage();
    if (j.isNull())
    {
