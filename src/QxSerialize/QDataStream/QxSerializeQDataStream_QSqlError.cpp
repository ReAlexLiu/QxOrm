...
QDataStream& operator<< (QDataStream& stream, const QSqlError& t)
{
    ...
    QString iNumber = t.nativeErrorCode();
...
QDataStream& operator>> (QDataStream& stream, QSqlError& t)
{
    ...
    QString iNumber(QStringLiteral("0")); qint32 iType(0);
    stream >> sDatabaseText;
    stream >> sDriverText;
    stream >> iNumber;
    stream >> iType;

    t = QSqlError(sDriverText, sDatabaseText, static_cast<QSqlError::ErrorType>(iType), iNumber);
    return stream;
}
