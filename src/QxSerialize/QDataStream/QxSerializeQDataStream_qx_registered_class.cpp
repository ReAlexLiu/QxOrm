...
QDataStream& QxSerializeRegistered_Helper::load(QDataStream& stream, IxClass* pClass, void* pOwner)
{
    ...
    {
        xerror("input binary data is not valid");
        return stream;
    }
    if (magic > 13937)
    {
...
{
    IxDataMemberX* pDataMemberX = (pClass ? pClass->getDataMemberX() : NULL); if (!pDataMemberX)
    {
        return;
    }
    ...
    for (long l = 0; l < pDataMemberX->count(); l++)
    {
        ...
        if (pDataMember->getVersion() > static_cast<long>(iVersion))
        {
