...
void IxValidatorX::setClass(IxClass* p)
{
    m_pClass = p;
}
...
{
    if (sPropertyKey.isEmpty() || !m_pClass)
    {
        return NULL;
    }
...
QStringList IxValidatorX::getAllGroup() const
{
    ...
    {
        lstGroup.append(m_lstValidatorByGroup.getKeyByIndex(l));
    }
    return lstGroup;
}
...
{
    if (!m_lstValidatorByGroup.exist(group))
    {
        return QList<IxValidator_ptr>();
    }
...
QxInvalidValueX IxValidatorX::validate(void* pOwner, const QString& sGroup /* = QString() */) const
{
    ...
    if (m_pClass->getBaseClass())
    {
        ...
        if (pAllValidator)
        {
            invalidValues.insert(pAllValidator->validate(pOwner, sGroup));
        }
    }
    ...
    for (long l = 0; l < lstValidator->count(); l++)
    {
        ...
        if (validator)
        {
            validator->validate(pOwner, invalidValues);
        }
    }
...
IxValidator_ptr IxValidatorX::createValidator(IxValidator::validator_type type, const QString& sPropertyKey, const QString& sMessage, const QString& sGroup)
{
    ...
    if (!sMessage.isEmpty())
    {
        pValidator->setMessage(sMessage);
    }
    if (!sGroup.isEmpty())
    {
