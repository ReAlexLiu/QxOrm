...
QString QxInvalidValueX::getCurrentPath() const
{
    return m_sCurrentPath;
}

void QxInvalidValueX::setCurrentPath(const QString& s)
{
    m_sCurrentPath = s;
}

long QxInvalidValueX::count() const
{
    return m_lstInvalidValues.count();
}

void QxInvalidValueX::dump() const
{
    xverbose(qPrintable(text()));
}

QxInvalidValue QxInvalidValueX::at(long l) const
{
    return (((l < 0) || (l >= m_lstInvalidValues.count())) ? QxInvalidValue() : m_lstInvalidValues.at(l));
}
...
QString QxInvalidValueX::text() const
{
    ...
    {
        return QStringLiteral("There is no invalid value");
    }
    QString sResult = QStringLiteral("List of invalid values (%1) :").arg(m_lstInvalidValues.count());

    for (long l = 0; l < m_lstInvalidValues.count(); l++)
    {
        QxInvalidValue invalidValue = m_lstInvalidValues.at(l);
        QString sTemp = QStringLiteral("\n\t- %1 (%2)").arg(invalidValue.getMessage()).arg(invalidValue.getFullName());
        sResult += sTemp;
    }
...
void QxInvalidValueX::insert(const IxValidator* pValidator)
{
    ...
    sMessage.replace(QStringLiteral("%NAME%"), (pDataMember ? pDataMember->getName() : QString()));
    sMessage.replace(QStringLiteral("%CONSTRAINT%"), pValidator->getConstraint().toString());
    QVariantList lstConstraints = pValidator->getConstraints();
    for (long l = 0; l < lstConstraints.count(); l++)
    {
        sMessage.replace(QStringLiteral("%CONSTRAINT_%1%").arg(l), lstConstraints.at(l).toString());
    }
...
{
    if (invalidValue.getMessage().isEmpty())
    {
        qAssert(false);
        return;
    }
...
void QxInvalidValueX::insert(const QxInvalidValueX& other)
{
    for (long l = 0; l < other.count(); l++)
    {
        ...
        sCurrPath += ((!m_sCurrentPath.isEmpty() && !sOtherPath.isEmpty()) ? QStringLiteral(".") : QStringLiteral(""));
