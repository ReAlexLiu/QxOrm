...
const IxValidator* QxInvalidValue::getValidator() const
{
    return m_pValidator;
}

void QxInvalidValue::setValidator(const IxValidator* p)
{
    m_pValidator = p;
}
...
QString QxInvalidValue::getFullName() const
{
    ...
    sResult += ((!m_sPath.isEmpty() && !m_sPropertyName.isEmpty()) ? QStringLiteral(".") : QStringLiteral(""));
