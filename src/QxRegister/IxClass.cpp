...
struct IxClass::IxClassImpl
{
    ...
    void updateNamePtr()
    {
        m_byteName = m_sName.toLatin1(); m_pName = m_byteName.constData();
    }
...
IxClass::~IxClass()
{
    if (QxClassX::isSingletonNull() || m_pImpl->m_sKey.isEmpty())
    {
        return;
    }
    if (!QxClassX::getSingleton()->exist(m_pImpl->m_sKey))
    {
        return;
    }
...
const char* IxClass::getNamePtr() const
{
    return m_pImpl->m_pName;
}

QString IxClass::getDescription() const
{
    return m_pImpl->m_sDescription;
}

long IxClass::getVersion() const
{
    return m_pImpl->m_lVersion;
}

qx::dao::strategy::inheritance IxClass::getDaoStrategy() const
{
    return m_pImpl->m_eDaoStrategy;
}

qx::QxSoftDelete IxClass::getSoftDelete() const
{
    return m_pImpl->m_oSoftDelete;
}

bool IxClass::isFinalClass() const
{
    return m_pImpl->m_bFinalClass;
}

bool IxClass::isDaoReadOnly() const
{
    return m_pImpl->m_bDaoReadOnly;
}

bool IxClass::isRegistered() const
{
    return m_pImpl->m_bRegistered;
}

IxDataMemberX* IxClass::getDataMemberX() const
{
    return m_pImpl->m_pDataMemberX;
}

IxFunctionX* IxClass::getFctMemberX() const
{
    return m_pImpl->m_pFctMemberX.get();
}

IxFunctionX* IxClass::getFctStaticX() const
{
    return m_pImpl->m_pFctStaticX.get();
}

std::shared_ptr<QxCollection<QString, IxSqlRelation*> > IxClass::getSqlRelationX()
{
    if ((m_pImpl->m_pSqlRelationX) || (!m_pImpl->m_pDataMemberX))
    {
        return m_pImpl->m_pSqlRelationX;
    }

    QMutexLocker locker(&m_pImpl->m_mutex);
    if (m_pImpl->m_pSqlRelationX)
    {
        return m_pImpl->m_pSqlRelationX;
    }
    ...
    {
        if ((p = m_pImpl->isValid_SqlRelation(l)))
        {
            m_pImpl->m_pSqlRelationX->insert(p->getKey(), p->getSqlRelation());
        }
    }
    return m_pImpl->m_pSqlRelationX;
}
...
{
    if ((m_pImpl->m_pSqlDataMemberX) || (!m_pImpl->m_pDataMemberX))
    {
        return m_pImpl->m_pSqlDataMemberX;
    }

    QMutexLocker locker(&m_pImpl->m_mutex);
    if (m_pImpl->m_pSqlDataMemberX)
    {
        return m_pImpl->m_pSqlDataMemberX;
    }
    ...
    for (long l = 0; l < lCount; ++l)
    {
        if ((p = m_pImpl->isValid_SqlDataMember(l)))
        {
            m_pImpl->m_pSqlDataMemberX->insert(p->getKey(), p);
        }
    }
    return m_pImpl->m_pSqlDataMemberX;
}

IxValidatorX_ptr& IxClass::getAllValidatorRef()
{
    return m_pImpl->m_pAllValidator;
}

void IxClass::setKey(const QString& sKey)
{
    m_pImpl->m_sKey = sKey;
}

void IxClass::setName(const QString& sName)
{
    m_pImpl->m_sName = sName; m_pImpl->updateNamePtr();
}

void IxClass::setDescription(const QString& sDesc)
{
    m_pImpl->m_sDescription = sDesc;
}

void IxClass::setDaoStrategy(qx::dao::strategy::inheritance eDaoStrategy)
{
    m_pImpl->m_eDaoStrategy = eDaoStrategy;
}

void IxClass::setSoftDelete(const qx::QxSoftDelete& oSoftDelete)
{
    m_pImpl->m_oSoftDelete = oSoftDelete; if (m_pImpl->m_oSoftDelete.getTableName().isEmpty())
    {
        m_pImpl->m_oSoftDelete.setTableName(m_pImpl->m_sName);
    }
}

void IxClass::setDaoReadOnly(bool bDaoReadOnly)
{
    m_pImpl->m_bDaoReadOnly = bDaoReadOnly;
}

void IxClass::setVersion(long l)
{
    m_pImpl->m_lVersion = l;
}

void IxClass::setRegistered(bool b)
{
    m_pImpl->m_bRegistered = b;
}

void IxClass::setFinalClass(bool b)
{
    m_pImpl->m_bFinalClass = b;
}

void IxClass::setDataMemberX(IxDataMemberX* p)
{
    m_pImpl->m_pDataMemberX = p;
}

void IxClass::setFctMemberX(IxFunctionX* p)
{
    m_pImpl->m_pFctMemberX.reset(p);
}

void IxClass::setFctStaticX(IxFunctionX* p)
{
    m_pImpl->m_pFctStaticX.reset(p);
}
...
{
    QString sDump;
    sDump += QStringLiteral("-- class '%1' (name '%2', ").arg(m_pImpl->m_sKey).arg(m_pImpl->m_sName);
    sDump += QStringLiteral("description '%1', version '%2', ").arg(m_pImpl->m_sDescription).arg(m_pImpl->m_lVersion);
    sDump += QStringLiteral("base class '%1')\n").arg(getBaseClass() ? getBaseClass()->getKey() : QStringLiteral(""));

    long lCount = (m_pImpl->m_pDataMemberX ? m_pImpl->m_pDataMemberX->count() : 0);
    sDump += QStringLiteral("\t* list of registered properties (%1)\n").arg(lCount);
    if (m_pImpl->m_pDataMemberX)
    {
        IxDataMember* pId = this->getId();
        for (long l = 0; l < lCount; l++)
        {
            IxDataMember* p = m_pImpl->m_pDataMemberX->get(l); if (!p)
            {
                continue;
            }
            IxSqlRelation* pRelation = p->getSqlRelation();
            QString sInfos = QStringLiteral("%1%2 - %3").arg(p->getKey()).arg((p == pId) ? QStringLiteral(" (id)") : QString()).arg(p->getType());
            sInfos += (pRelation ? (QStringLiteral(" (%1)").arg(pRelation->getDescription())) : QString());
            sDump += QStringLiteral("\t\t%1\n").arg(sInfos);
        }
    }

    lCount = (m_pImpl->m_pFctMemberX ? m_pImpl->m_pFctMemberX->count() : 0);
    sDump += QStringLiteral("\t* list of registered member functions (%1)\n").arg(lCount);
    if (m_pImpl->m_pFctMemberX)
    {
        for (auto itr = m_pImpl->m_pFctMemberX->begin(); itr != m_pImpl->m_pFctMemberX->end(); ++itr)
        {
            IxFunction_ptr p = itr->second; if (!p)
            {
                continue;
            }
    ...
    sDump += QStringLiteral("\t* list of registered static functions (%1)\n").arg(lCount);
