...
QxClassX::QxClassX() : QxSingleton<QxClassX>(QStringLiteral("qx::QxClassX"))
{
...
IxClass* QxClassX::get(const QString& sKey) const
{
    ...
    if (!pClass)
    {
        pClass = (m_lstClass.exist(sKey) ? m_lstClass.getByKey(sKey) : NULL);
    }
...
IxDataMember* QxClassX::getDataMember(const QString& sClassKey, const QString& sDataKey, bool bRecursive /* = true */)
{
    ...
    do
    {
        ...
        sBaseClassKey = ((pClass && pClass->getBaseClass()) ? pClass->getBaseClass()->getKey() : QStringLiteral(""));
...
bool QxClassX::implementIxPersistable(const QString& sKey, bool bTraceIfFalse /* = true */)
{
    ...
    if (!bImplement && bTraceIfFalse)
    {
        xwarn("class '{}' doesn't implement qx::IxPersistable interface", qPrintable(sKey));
    }
    return bImplement;
}
...
void QxClassX::registerAllClasses(bool bInitAllRelations /* = false */)
{
    ...
    {
        IxClass* pClass = pAllClasses->getByIndex(k); if (!pClass)
        {
            continue;
        }
        IxDataMemberX* pDataMemberX = pClass->getDataMemberX(); if (!pDataMemberX)
        {
            continue;
        }
        for (long l = 0; l < pDataMemberX->count_WithDaoStrategy(); l++)
        {
            IxDataMember* pDataMember = pDataMemberX->get_WithDaoStrategy(l); if (!pDataMember)
            {
                continue;
            }
            if (isValid_SqlRelation(pDataMember))
            {
                pDataMember->getSqlRelation()->init();
            }
        }
...
QString QxClassX::dumpAllClasses()
{
    ...
    xverbose("start dump all registered classes ({})", lCount);
    for (auto itr = pAllClasses->begin(); itr != pAllClasses->end(); ++itr)
    {
        IxClass* pClass = itr->second; if (pClass)
        {
            sDump += pClass->dumpClass();
        }
    }
    xverbose("end dump all registered classes");
...
{
    xwarn("be careful with this function, it's just an example and tested only with SQLite database, so it's strongly recommended to write your own function to create your SQL schema");
    QxCollection<QString, IxClass*>* pAllClasses = QxClassX::getAllClasses();
    if (!pAllClasses)
    {
        qAssert(false); return QStringLiteral("");
    ...
    for (long k = 0; k < pAllClasses->count(); k++)
    {
        ...
        if (pClass->isKindOf(QStringLiteral("qx::service::IxParameter")) || pClass->isKindOf(QStringLiteral("qx::service::IxService")))
        {
            continue;
        }
        ...
        sql += pClass->getName() + QStringLiteral(" ");
        ...
        {
            sql += oSoftDelete.buildSqlQueryToCreateTable() + QStringLiteral(", ");
        }
        ...
        for (long l = 0; (pDataMemberX && (l < pDataMemberX->count_WithDaoStrategy())); l++)
        {
            ...
            if (isValid_SqlRelation(p) && (p != pId) && (bCreateTable || (p->getVersion() >= lVersion)))
            {
                ...
                sql += sqlExtraTable + QStringLiteral("\n");
            }
        }
    }

    xverbose("start dump SQL schema ({})", lSqlCount);
    xverbose(qPrintable(sql));
    xverbose("end dump SQL schema");
...
const char* get_sql_type_by_class_name(const char* sClassName, const char* sDefaultValue)
{
    static std::string s;
#ifndef QT_NO_STL
    s = qx::QxClassX::getSqlTypeByClassName(QLatin1String(sClassName)).toStdString();
#else // QT_NO_STL
