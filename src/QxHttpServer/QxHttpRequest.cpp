...
struct QxHttpRequest::QxHttpRequestImpl
{
    ...
    QxHttpRequestImpl(QxHttpTransaction* transaction) : m_port(0), m_transaction(transaction)
    {
        qAssert(m_transaction != NULL); m_guid = QUuid::createUuid().toString();
    }
    ~QxHttpRequestImpl()
    {
        ;
...
QUrl& QxHttpRequest::url()
{
    return m_pImpl->m_url;
}

QString& QxHttpRequest::command()
{
    return m_pImpl->m_command;
}

QString& QxHttpRequest::version()
{
    return m_pImpl->m_version;
}

QByteArray& QxHttpRequest::data()
{
    return m_pImpl->m_data;
}

QHash<QByteArray, QByteArray>& QxHttpRequest::headers()
{
...
QHash<QByteArray, QxHttpCookie>& QxHttpRequest::cookies()
{
    return m_pImpl->m_cookies;
}

QxHttpCookie QxHttpRequest::cookie(const QByteArray& name)
{
    return m_pImpl->m_cookies.value(name);
}

QHash<QString, QVariant>& QxHttpRequest::dispatchParams()
{
    return m_pImpl->m_dispatchParams;
}

QHash<QString, QString>& QxHttpRequest::params()
{
    return m_pImpl->m_params;
}

QString QxHttpRequest::param(const QString& key)
{
    return m_pImpl->m_params.value(key);
}

QString& QxHttpRequest::sourceAddress()
{
    return m_pImpl->m_address;
}

long& QxHttpRequest::sourcePort()
{
    return m_pImpl->m_port;
}

QString QxHttpRequest::guid() const
{
    return m_pImpl->m_guid;
}
