...
struct QxHttpTransaction::QxHttpTransactionImpl
{
    ...
    QxHttpTransactionImpl(QxHttpTransaction* parent) : m_request(parent), m_response(parent), m_socket(NULL), m_headersWritten(false), m_chunkAllowed(true)
    {
        qAssert(parent != NULL);
    }
    ...
    qx_bool writeHeaders(QTcpSocket& socket)
    {
        ...
        if (!writeToSocket(socket, line))
        {
            return qx_bool(500, QStringLiteral("Internal server error : cannot write to socket HTTP response first line '%1' (%2)").arg(QLatin1String(line)).arg(socket.errorString()));
        }
        ...
        {
            itrHeaders.next();
            if (itrHeaders.key().trimmed().isEmpty())
            {
                continue;
            }
            ...
            if (!writeToSocket(socket, line))
            {
                return qx_bool(500, QStringLiteral("Internal server error : cannot write to socket HTTP response header '%1' (%2)").arg(QLatin1String(line)).arg(socket.errorString()));
            }
        }
        ...
        {
            itrCookies.next();
            if (itrCookies.value().name.trimmed().isEmpty())
            {
                continue;
            }
            ...
            if (!writeToSocket(socket, line))
            {
                return qx_bool(500, QStringLiteral("Internal server error : cannot write to socket HTTP response cookie '%1' (%2)").arg(QLatin1String(line)).arg(socket.errorString()));
            }
        }
        ...
        {
            return qx_bool(500, QStringLiteral("Internal server error : cannot write to socket HTTP response header end line (%1)").arg(socket.errorString()));
        }
        return qx_bool(true);
    }
...
{
    qx::service::QxTransaction::clear(); m_pImpl.reset(new QxHttpTransactionImpl(this));
}

qx::QxHttpRequest& QxHttpTransaction::request()
{
    return m_pImpl->m_request;
}

qx::QxHttpResponse& QxHttpTransaction::response()
{
...
qx_bool QxHttpTransaction::writeSocketServer(QTcpSocket& socket)
{
    ...
    QString contentType = QLatin1String(m_pImpl->m_response.header("Content-Type").toLower());
    compress = (compress && (contentType.startsWith(QStringLiteral("text/")) || contentType.startsWith(QStringLiteral("application/json")) || contentType.startsWith(QStringLiteral("application/javascript"))));
    ...
    if (!bWriteHeaders)
    {
        return bWriteHeaders;
    }

    // HTTP response body content
    if (chunked)
    {
        if (!m_pImpl->writeToSocket(socket, "0\r\n\r\n"))
        {
            return qx_bool(500, QStringLiteral("Internal server error : cannot write to socket HTTP last empty chunked data ()").arg(socket.errorString()));
        }
    }
    else if (!m_pImpl->writeToSocket(socket, body))
    {
        return qx_bool(500, QStringLiteral("Internal server error : cannot write to socket HTTP response body content of %1 bytes (%2)").arg(body.count()).arg(socket.errorString()));
    }
    return qx_bool(true);
}
...
qx_bool QxHttpTransaction::readSocketServer(QTcpSocket& socket)
{
    ...
    if (!m_pImpl->waitForReadSocket(socket))
    {
        setMessageReturn(qx_bool(500, QStringLiteral("Internal server error : cannot read socket to parse HTTP request first line (%1)").arg(socket.errorString())));
        return qx_bool(true);
    }
    ...
    if (lst.count() < 3)
    {
        setMessageReturn(qx_bool(400, QStringLiteral("Bad request : invalid HTTP request first line : %1").arg(QLatin1String(line))));
        return qx_bool(true);
    }
    if (!lst.at(2).contains(QStringLiteral("HTTP")))
    {
        setMessageReturn(qx_bool(400, QStringLiteral("Bad request : invalid HTTP request first line, third parameter must contain 'HTTP' : %1").arg(QLatin1String(line))));
        return qx_bool(true);
    }
    ...
    do
    {
        if (!m_pImpl->waitForReadSocket(socket))
        {
            setMessageReturn(qx_bool(500, QStringLiteral("Internal server error : cannot read socket to parse HTTP headers (%1)").arg(socket.errorString())));
            return qx_bool(true);
        }
        line = socket.readLine().trimmed(); if (line.isEmpty())
        {
            break;
        }
        int pos = line.indexOf(':'); if (pos <= 0)
        {
            setMessageReturn(qx_bool(400, QStringLiteral("Bad request : invalid HTTP header : %1").arg(QLatin1String(line))));
            return qx_bool(true);
        }
        ...
        {
            m_pImpl->m_request.cookies().unite(QxHttpCookie::parse(value));
        }
        else
        {
            m_pImpl->m_request.headers().insert(key, value);
        }
    ...
    if (m_pImpl->m_request.version().contains(QStringLiteral("1.0")))
    {
    ...
    if (iContentLength > 0)
    {
        ...
        do
        {
            if (!m_pImpl->waitForReadSocket(socket))
            {
                setMessageReturn(qx_bool(500, QStringLiteral("Internal server error : cannot read socket to get HTTP body content (%1)").arg(socket.errorString()))); return qx_bool(true);
            }
            body += socket.readAll();
    ...
    Q_FOREACH(QByteArray param, listParams)
    {
        ...
        if (pos > 0)
        {
            QByteArray key = param.left(pos).trimmed(); QByteArray value = param.mid(pos + 1).trimmed(); m_pImpl->m_request.params().insert(QUrl::fromPercentEncoding(key), QUrl::fromPercentEncoding(value));
        }
        else if (!param.isEmpty())
        {
            m_pImpl->m_request.params().insert(QUrl::fromPercentEncoding(param), QStringLiteral(""));
        }
    }
...
qx_bool QxHttpTransaction::writeChunked(const QByteArray& data)
{
    ...
    {
        m_pImpl->m_response.data() += data; return qx_bool(true);
    }
    ...
    if (!m_pImpl->writeToSocket((*m_pImpl->m_socket), chunked))
    {
        return qx_bool(500, QStringLiteral("Internal server error : cannot write to socket HTTP chunked data of %1 bytes (%2)").arg(data.count()).arg(m_pImpl->m_socket->errorString()));
    }
    return qx_bool(true);
}
