...
struct QxHttpSessionManager::QxHttpSessionManagerImpl
{
    ...
    QByteArray getSessionId(qx::QxHttpRequest& request, qx::QxHttpResponse& response, const QByteArray& cookieName)
    {
        ...
        if (id.isEmpty())
        {
            id = request.cookie(cookieName).value;
        }
        if ((!id.isEmpty()) && (!m_sessions.contains(id)))
        {
            id = QByteArray();
        }
        return id;
    ...
    qx::QxHttpSession_ptr getSession(qx::QxHttpRequest& request, qx::QxHttpResponse& response, const QByteArray& cookieName, bool autoCreateSession)
    {
        ...
        {
            return createSession(request, response, cookieName);
        }
...
QxHttpSessionManager::QxHttpSessionManager() : QObject(), qx::QxSingleton<QxHttpSessionManager>(QStringLiteral("qx::QxHttpSessionManager")), m_pImpl(new QxHttpSessionManagerImpl())
{
    QObject::connect((&m_pImpl->m_timer), SIGNAL(timeout()), this, SLOT(onCheckSessionTimeOut()));
    m_pImpl->m_timer.start(60000);
}

QxHttpSessionManager::~QxHttpSessionManager()
{
    m_pImpl->m_timer.stop();
}

void QxHttpSessionManager::deleteSession(qx::QxHttpSession* p)
{
    delete p;
}
...
void QxHttpSessionManager::onCheckSessionTimeOut()
{
    ...
    while (itr.hasNext())
    {
        ...
        {
            qAssert(false); itr.remove(); continue;
        }
        if (lastAccess.addMSecs(lTimeOut) < dt)
        {
            itr.remove(); continue;
        }
    }
