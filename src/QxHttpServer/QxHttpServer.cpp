...
struct QxHttpServerDispatchSegment
{
    ...
    enum type_segment
    {
        _static, _variable, _variable_and_type, _variable_and_regexp, _wildcard
    };
...
#if (QT_VERSION >= 0x050000)
    if (m_pImpl->m_pEventDispatcher)
    {
        m_pImpl->m_pThreadPool->setEventDispatcher(m_pImpl->m_pEventDispatcher);
    }
#endif // (QT_VERSION >= 0x050000)
...
void QxHttpServer::setCustomRequestHandler(QxHttpServer::type_fct_custom_request_handler fct)
{
    ...
    if (!fct)
    {
        m_pImpl->m_fctCustomRequestHandler.reset(); return;
    }
...
void QxHttpServer::beforeDispatching(QxHttpServer::type_fct_custom_request_handler fct)
{
    ...
    if (!fct)
    {
        m_pImpl->m_fctBeforeDispatching.reset(); return;
    }
...
void QxHttpServer::afterDispatching(QxHttpServer::type_fct_custom_request_handler fct)
{
    ...
    if (!fct)
    {
        m_pImpl->m_fctAfterDispatching.reset(); return;
    }
...
void QxHttpServer::onError(const QString& err, qx::service::QxTransaction_ptr transaction)
{
    ...
    if (transaction && (!httpTransaction))
    {
        qAssertMsg(false, "unable to get HTTP transaction using std::dynamic_pointer_cast");
    }
...
void QxHttpServer::onServerIsRunning(bool bIsRunning, qx::service::QxServer* pServer)
{
    ...
    if (bIsRunning)
    {
        ...
        msg += QStringLiteral("port: ") + QString::number(serverSettings->getPort());
        msg += QStringLiteral(", threads: ") + QString::number(serverSettings->getThreadCount());
#ifndef QT_NO_SSL
        msg += QStringLiteral(", SSL: ") + (serverSettings->getSSLEnabled() ? QStringLiteral("enabled") : QStringLiteral("disabled"));
#else // QT_NO_SSL
        msg += ", SSL: disabled";
#endif // QT_NO_SSL
        msg += QStringLiteral(")");
    ...
    {
        xinfo("HTTP server has been stopped");
    }
...
void QxHttpServer::onCustomRequestHandler(qx::service::QxTransaction_ptr transaction)
{
    ...
    if (transaction && (!httpTransaction))
    {
        qAssertMsg(false, "unable to get HTTP transaction using std::dynamic_pointer_cast"); return;
    }
    if (!httpTransaction)
    {
        qAssertMsg(false, "HTTP transaction is empty"); return;
    }
    ...
    if (fctBeforeDispatching && (*fctBeforeDispatching))
    {
        try
        {
            (*fctBeforeDispatching)(request, response);
        }
        catch (const qx::exception& x)
        {
            qx_bool xb = x.toQxBool(); httpTransaction->setMessageReturn(xb);
        }
        catch (const std::exception& e)
        {
            httpTransaction->setMessageReturn(qx_bool(500, QLatin1String(e.what())));
        }
        catch (...)
        {
            httpTransaction->setMessageReturn(qx_bool(500, QStringLiteral("Unknown server error (before dispatching)")));
        }
    }
    ...
    try
    {
        ...
        if (itemDispatch && itemDispatch->m_fct)
        {
            itemDispatch->m_fct(request, response);
        }
        else if (fctCustomHandler && (*fctCustomHandler))
        {
            (*fctCustomHandler)(request, response);
        }
        else
        {
            httpTransaction->setMessageReturn(qx_bool(501, QStringLiteral("Not implemented : no route found for URL (%1 - %2) : you must provide a request handler (callback) using qx::QxHttpServer::dispatch() or qx::QxHttpServer::setCustomRequestHandler() method").arg(request.command()).arg(request.url().path())));
        }
    }
    catch (const qx::exception& x)
    {
    ...
    {
        httpTransaction->setMessageReturn(qx_bool(500, QStringLiteral("Unknown server error")));
    }
    ...
    if (fctAfterDispatching && (*fctAfterDispatching))
    {
        try
        {
            (*fctAfterDispatching)(request, response);
        }
        catch (const qx::exception& x)
        {
            qx_bool xb = x.toQxBool(); httpTransaction->setMessageReturn(xb);
        ...
        {
            httpTransaction->setMessageReturn(qx_bool(500, QStringLiteral("Unknown server error (after dispatching)")));
        }
    }
}
...
{
    QString key = QStringLiteral("%1|%2").arg(command).arg(path);
    if (m_lstDispatchItems.exist(key))
    {
        m_lstDispatchItems.removeByKey(key);
    }
    if ((!fct) || (path.isEmpty()) || (command.isEmpty()))
    {
        return;
    }

    std::shared_ptr<QxHttpServerDispatchItem> item = std::make_shared<QxHttpServerDispatchItem>();
    item->m_command = command; item->m_path = ((path == QStringLiteral("/")) ? QStringLiteral("/*") : path); item->m_fct = fct;
    if (!item->parse())
    {
        qAssert(false); return;
    }
    if (item->m_segments.count() <= 0)
    {
        return;
    }
    if (position < 0)
    {
        m_lstDispatchItems.insert(key, item);
    }
    else
    {
        m_lstDispatchItems.insert(position, key, item);
    }
}
...
std::shared_ptr<QxHttpServerDispatchItem> QxHttpServerDispatcher::find(qx::QxHttpRequest& request) const
{
    ...
    QString requestPath = request.url().path(); if (requestPath == QStringLiteral("/"))
    {
        requestPath = QStringLiteral("/*");
    }
    QStringList requestSegments = requestPath.split(QStringLiteral("/"), Qt::SkipEmptyParts);
    ...
    while (itr.next())
    {
        ...
        const std::shared_ptr<QxHttpServerDispatchItem>& item = itr.value(); if (!item)
        {
            qAssert(false); continue;
        }
        if ((item->m_commandHash != requestCommandHash) && (!item->m_commandWildcard))
        {
            continue;
        }
        if (item->m_segments.count() > requestSegments.count())
        {
            continue;
        }
        if (!item->m_fct)
        ...
        {
            const std::shared_ptr<QxHttpServerDispatchSegment>& segment = item->m_segments.at(i); if (!segment)
            {
                qAssert(false); continue;
            }
            if (!segment->check(requestSegments.at(i), requestSegmentsHash.at(i)))
            {
                bAllSegmentsOk = false; break;
            }
        }
        if (!bAllSegmentsOk)
        {
...
bool QxHttpServerDispatchItem::parse()
{
    ...
    m_commandWildcard = (m_command == QStringLiteral("*"));
    QStringList segments = m_path.split(QStringLiteral("/"), Qt::SkipEmptyParts);
    Q_FOREACH(QString data, segments)
    {
        data = data.trimmed(); if (data.isEmpty())
        {
            continue;
        }
        std::shared_ptr<QxHttpServerDispatchSegment> segment = std::make_shared<QxHttpServerDispatchSegment>();
        segment->m_path = data; if (!segment->parse())
        {
...
bool QxHttpServerDispatchSegment::parse()
{
    ...
    if (m_path.startsWith(QStringLiteral("<")) && m_path.endsWith(QStringLiteral(">")))
    {
        QString path = m_path;
        path.remove(0, 1); path.chop(1);
        int pos = path.indexOf(QStringLiteral(":"));
        if (pos >= 0)
        {
            m_varName = path.left(pos).trimmed();
            m_varType = path.mid(pos + 1).trimmed();
            m_type = QxHttpServerDispatchSegment::_variable_and_type;
            if (m_varType.startsWith(QStringLiteral("{")) && m_varType.endsWith(QStringLiteral("}")))
            {
...
bool QxHttpServerDispatchSegment::check(const QString& path, unsigned int hash) const
{
    ...
    case QxHttpServerDispatchSegment::_variable_and_type:
        ...
        if ((m_varType == QStringLiteral("int")) || (m_varType == QStringLiteral("long")))
        {
            qlonglong val = path.toLongLong(&result, 10); Q_UNUSED(val);
        }
        if ((m_varType == QStringLiteral("float")) || (m_varType == QStringLiteral("double")))
        {
            double val = path.toDouble(&result); Q_UNUSED(val);
        }
        else if (m_varType == QStringLiteral("string"))
        {
            result = (!path.isEmpty());
        }
        break;
...
void QxHttpServer::buildResponseStaticFile(qx::QxHttpRequest& request, qx::QxHttpResponse& response, const QString& serverPath, qlonglong chunkedSize /* = 0 */)
{
    ...
    if (request.command() != QStringLiteral("GET"))
    {
        response.status() = 400;
    ...
    QString filePath = dirServerPath.filePath(QLatin1String(requestPath.right(requestPath.size() - 1)));
    ...
    if (chunkedSize > 0)
    {
        while (!file.atEnd())
        {
            if (!response.writeChunked(file.read(chunkedSize)))
            {
                return;
            }
        }
...
{
    qx::QxRestApi api;
    QString result = api.processRequest((QString)QLatin1String(request.data()));
