...
struct QxHttpResponse::QxHttpResponseImpl
{
    ...
    QxHttpResponseImpl(QxHttpTransaction* transaction) : m_status(200), m_transaction(transaction), m_isChunked(false)
    {
        qAssert(m_transaction != NULL); initHeaders();
    }
    ~QxHttpResponseImpl()
    {
        ;
    ...
    void initHeaders()
    {
        ...
        if (qx::service::QxConnect::getSingleton()->getKeepAlive() != 0)
        {
            m_headers.insert("Connection", "keep-alive");
        }
        else
        {
            m_headers.insert("Connection", "close");
        }
    }
    ...
    QByteArray currentDateTimeGMT()
    {
        ...
        QByteArray day_ = QStringLiteral("%1").arg(dt.date().day(), 2, 10, QLatin1Char('0')).toLatin1();
        QByteArray year = QStringLiteral("%1").arg(dt.date().year(), 4, 10, QLatin1Char('0')).toLatin1();
        QByteArray hour = QStringLiteral("%1").arg(dt.time().hour(), 2, 10, QLatin1Char('0')).toLatin1();
        QByteArray minute = QStringLiteral("%1").arg(dt.time().minute(), 2, 10, QLatin1Char('0')).toLatin1();
        QByteArray second = QStringLiteral("%1").arg(dt.time().second(), 2, 10, QLatin1Char('0')).toLatin1();
...
int& QxHttpResponse::status()
{
    return m_pImpl->m_status;
}

QByteArray& QxHttpResponse::data()
{
    return m_pImpl->m_data;
}

QHash<QByteArray, QByteArray>& QxHttpResponse::headers()
{
...
QByteArray QxHttpResponse::header(const QByteArray& key)
{
    ...
    while (itr.hasNext())
    {
        itr.next(); if (itr.key().trimmed().toLower() == key.trimmed().toLower())
        {
            return itr.value();
        }
    }
...
QHash<QByteArray, QxHttpCookie>& QxHttpResponse::cookies()
{
    return m_pImpl->m_cookies;
}

QxHttpCookie QxHttpResponse::cookie(const QByteArray& name)
{
    return m_pImpl->m_cookies.value(name);
}

qx_bool QxHttpResponse::writeChunked(const QByteArray& data)
{
    return m_pImpl->writeChunked(data);
}

bool QxHttpResponse::isChunked() const
{
    return m_pImpl->m_isChunked;
}
