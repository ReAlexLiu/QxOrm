...
IxFactory::IxFactory(const QString& sKey) : m_sKeyFactory(sKey)
{
    ...
    xverbose("constructor '{}'", qPrintable(m_sKeyFactory));
...
{
#ifdef _QX_TRACE_CONSTRUCTOR_DESTRUCTOR
    xverbose("destructor '{}'", qPrintable(m_sKeyFactory));
#endif // _QX_TRACE_CONSTRUCTOR_DESTRUCTOR

    if (!QxFactoryX::isSingletonNull())
    {
        QxFactoryX::getSingleton()->unregisterFactory(m_sKeyFactory);
    }
}
