...
void QxThread::onIncomingConnection()
{
    ...
    if (socket->setSocketDescriptor(getSocketDescriptor()))
    {
        ...
        {
            Q_EMIT error(QStringLiteral("SSL socket encrypted error : %1").arg(socket->errorString()), QxTransaction_ptr());
        }
#endif // QT_NO_SSL
    }
    else
    {
        Q_EMIT error(QStringLiteral("invalid socket descriptor : cannot start transaction (%1)").arg(socket->errorString()), QxTransaction_ptr());
    }
...
{
    if (m_pTransaction && (m_pTransaction->getForceConnectionStatus() == qx::service::QxTransaction::conn_close))
    {
        return false;
    }
    ...
    do
    {
        ...
        if (hasBeenStopped() || m_bIsDisconnected)
        {
            return false;
        }
...
void QxThread::doProcess(QTcpSocket& socket)
{
    ...
    if (bModeHTTP)
    {
        m_pTransaction = std::make_shared<qx::QxHttpTransaction>();
    }
    else
    {
    ...
    if (!bReadOk)
    {
        Q_EMIT error(QStringLiteral("unable to read request from socket : '%1'").arg(bReadOk.getDesc()), QxTransaction_ptr()); return;
    }
    if (hasBeenStopped() || m_bIsDisconnected)
    {
        return;
    ...
    {
        qx_bool xb = x.toQxBool(); m_pTransaction->setMessageReturn(xb);
    }
    catch (const std::exception& e)
    {
        m_pTransaction->setMessageReturn(qx_bool(QX_ERROR_UNKNOWN, QLatin1String(e.what())));
    }
    catch (...)
    {
    ...
    if (!bWriteOk)
    {
        Q_EMIT error(QStringLiteral("unable to write reply to socket : '%1'").arg(bWriteOk.getDesc()), m_pTransaction);
    }
...
void QxThread::onCustomRequestHandler()
{
    if (m_pTransaction)
    {
        Q_EMIT customRequestHandler(m_pTransaction);
    }
    else
    {
        qAssert(false);
...
void QxThread::onSocketSSLErrors(const QList<QSslError>& errors)
{
    for (int i = 0; i < errors.count(); i++)
    {
        ...
        QString msg = err.errorString();
        xerror(qPrintable(msg));
    }
}
