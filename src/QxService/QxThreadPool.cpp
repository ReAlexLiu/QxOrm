...
bool QxThreadPool::isStopped() const
{
    return m_bIsStopped;
}
...
void QxThreadPool::runServer()
{
    ...
    {
        raiseError(QStringLiteral("cannot run server : '%1").arg(server.errorString()), QxTransaction_ptr()); return;
    }
...
{
    QMutexLocker locker(&m_mutex);
    for (long l = 0; l < m_lstAllServices.count(); l++)
    {
        m_lstAllServices.at(l)->stop();
    }
    for (long l = 0; l < m_lstAllServices.count(); l++)
    {
        m_lstAllServices.at(l)->wait();
    }
    for (long l = 0; l < m_lstAllServices.count(); l++)
    {
        delete m_lstAllServices.at(l);
    }
