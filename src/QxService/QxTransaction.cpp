...
#define QX_TRANSACTION_QDATETIME_FORMAT QStringLiteral("yyyyMMddhhmmsszzz")
...
void QxTransaction::executeServer()
{
    ...
    {
        m_bMessageReturn = qx_bool(QX_ERROR_SERVICE_NOT_SPECIFIED, QStringLiteral("empty service name => cannot instantiate service and execute process")); return;
    }
    if (m_sServiceMethod.isEmpty())
    {
        m_bMessageReturn = qx_bool(QX_ERROR_SERVICE_NOT_SPECIFIED, QStringLiteral("empty service method => cannot execute process")); return;
    }

    qx::service::IxService* ptr = qx::create_nude_ptr<qx::service::IxService>(m_sServiceName);
    if (ptr == NULL)
    {
        m_bMessageReturn = qx_bool(QX_ERROR_SERVICE_INVALID, QStringLiteral("invalid service name => cannot instantiate service and execute process")); return;
    }
    ...
    try
    {
        ...
        if (!bInvokeOk)
        {
            m_bMessageReturn = qx_bool(QX_ERROR_SERVICE_INVALID, QStringLiteral("invalid service method => cannot execute process")); return;
        }
    ...
    {
        m_bMessageReturn = x.toQxBool();
    }
    catch (const std::exception& e)
    {
        QString msg(QLatin1String(e.what())); if (msg.isEmpty())
        {
            msg = QStringLiteral("unexpected error occured executing service method");
        }; m_bMessageReturn = qx_bool(QX_ERROR_UNKNOWN, msg);
    }
    catch (...)
    {
        m_bMessageReturn = qx_bool(QX_ERROR_UNKNOWN, QStringLiteral("unknown error occured executing service method"));
    }
    m_pServiceInstance.reset();
}
...
{
    quint32 uiTransactionSize = 0;
    qx_bool bReadOk = QxTools::readSocket(socket, (*this), uiTransactionSize); if (!bReadOk)
    {
        return bReadOk;
    }
...
{
    if ((pService == NULL) || sMethod.isEmpty())
    {
        qAssert(false); return;
    }
    if (pService->getServiceName().isEmpty())
    {
        pService->setMessageReturn(qx_bool(QX_ERROR_SERVICE_NOT_SPECIFIED, QStringLiteral("empty service name"))); return;
    }
    ...
    if (bSSLEnabled)
    {
        static_cast<QSslSocket*>(socket.get())->connectToHostEncrypted(serverName, serverPort);
    }
    else
    {
    ...
    {
        pService->setMessageReturn(qx_bool(QX_ERROR_SERVICE_SSL_ENCRYPTED, QStringLiteral("SSL socket encrypted error : ") + socket->errorString())); return;
    }
#else // QT_NO_SSL
    socket->connectToHost(serverName, serverPort);
#endif // QT_NO_SSL
    ...
    {
        pService->setMessageReturn(qx_bool(QX_ERROR_SERVER_NOT_FOUND, QStringLiteral("unable to connect to server : ") + socket->errorString())); return;
    }
    ...
    if (!bWriteOk)
    {
        pService->setMessageReturn(qx_bool(QX_ERROR_SERVICE_WRITE_ERROR, QStringLiteral("unable to write request to socket : '%1'").arg(bWriteOk.getDesc()))); 
        return;
    }
    qx_bool bReadOk = readSocketClient(*socket);
    if (!bReadOk)
    {
        pService->setMessageReturn(qx_bool(QX_ERROR_SERVICE_READ_ERROR, QStringLiteral("unable to read reply from socket : '%1'").arg(bReadOk.getDesc()))); 
        return;
    }
...
QString QxTransaction::getInfos() const
{
    QString infos;
#ifndef _QX_NO_JSON
    infos += QStringLiteral("transaction_content (JSON format) :\n%1\n").arg(qx::serialization::json::to_string(*this));
#else // _QX_NO_JSON
    infos += QStringLiteral("transaction_id :\t\t%1\n").arg(m_sTransactionId);
    infos += QStringLiteral("input_transaction_size :\t\t%1\n").arg(QString::number(m_uiInputTransactionSize));
    infos += QStringLiteral("output_transaction_size :\t\t%1\n").arg(QString::number(m_uiOutputTransactionSize));
    infos += QStringLiteral("dt_transaction_begin :\t\t%1\n").arg(m_dtTransactionBegin.toString(QX_TRANSACTION_QDATETIME_FORMAT));
    infos += QStringLiteral("dt_transaction_request_sent :\t\t%1\n").arg(m_dtTransactionRequestSent.toString(QX_TRANSACTION_QDATETIME_FORMAT));
    infos += QStringLiteral("dt_transaction_request_received :\t\t%1\n").arg(m_dtTransactionRequestReceived.toString(QX_TRANSACTION_QDATETIME_FORMAT));
    infos += QStringLiteral("dt_transaction_reply_sent :\t\t%1\n").arg(m_dtTransactionReplySent.toString(QX_TRANSACTION_QDATETIME_FORMAT));
    infos += QStringLiteral("dt_transaction_reply_received :\t\t%1\n").arg(m_dtTransactionReplyReceived.toString(QX_TRANSACTION_QDATETIME_FORMAT));
    infos += QStringLiteral("dt_transaction_end :\t\t%1\n").arg(m_dtTransactionEnd.toString(QX_TRANSACTION_QDATETIME_FORMAT));
    infos += QStringLiteral("ip_source :\t\t%1\n").arg(m_sIpSource);
    infos += QStringLiteral("ip_target :\t\t%1\n").arg(m_sIpTarget);
    infos += QStringLiteral("port_source :\t\t%1\n").arg(QString::number(m_lPortSource));
    infos += QStringLiteral("port_target :\t\t%1\n").arg(QString::number(m_lPortTarget));
    infos += QStringLiteral("service_name :\t\t%1\n").arg(m_sServiceName);
    infos += QStringLiteral("service_method :\t\t%1\n").arg(m_sServiceMethod);
    infos += QStringLiteral("message_return :\t\t%1\n").arg((m_bMessageReturn ? QStringLiteral("1") : QStringLiteral("0")) + (m_bMessageReturn.getDesc().isEmpty() ? QString() : (QStringLiteral(", ") + m_bMessageReturn.getDesc())) + ((m_bMessageReturn.getValue() == 0) ? QString() : (QStringLiteral(", value=") + QString::number(m_bMessageReturn.getValue()))));
#endif // _QX_NO_JSON
    return infos;
}
...
{
    if (!QxConnect::getSingleton()->getSSLEnabled())
    {
...
QDataStream& operator>> (QDataStream& stream, qx::service::QxTransaction& t)
{
    ...
    else
    {
        ...
        {
            ptr->registerClass(); ptr->load(stream);
        }
