...
#endif // (QT_VERSION >= 0x050000)
{
    ...
    if (m_pThreadPool && m_pThreadPool->isStopped())
    {
        return;
    }
    if (!pThread)
    {
        if (m_pThreadPool)
        {
            m_pThreadPool->raiseError(QStringLiteral("no service available : cannot accept incoming connection (increase thread count value)"), QxTransaction_ptr());
        } return;
    }
...
QxThread* QxServer::getAvailable() const
{
    ...
    xwarn("no service available : need to wait (try to increase thread count value)");
