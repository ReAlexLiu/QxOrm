...
QxDataMember_QObject::QxDataMember_QObject(const QMetaObject* pMetaObject, const QString& sKey) : IxDataMember(sKey), m_metaObject(pMetaObject)
{
    ...
    setSqlType(QxClassX::getSqlTypeByClassName(QLatin1String(m_metaProperty.typeName())));
}
...
{
    return QLatin1String(m_metaProperty.typeName());
}
...
QJsonValue QxDataMember_QObject::toJson(const void* pOwner, const QString& sFormat) const
{
    ...
    if (getIsPrimaryKey() && sFormat.startsWith("mongodb"))
    {
        ...
        if (tmp.startsWith("qx_oid:"))
        {
            QJsonObject obj; obj.insert("$oid", QJsonValue(tmp.right(tmp.size() - 7))); return QJsonValue(obj);
        }
    }
...
qx_bool QxDataMember_QObject::fromJson(void* pOwner, const QJsonValue& j, const QString& sFormat)
{
    ...
    if (getIsPrimaryKey() && val.toString().isEmpty() && j.isObject() && sFormat.startsWith("mongodb"))
    {
        ...
        if (obj.contains("$oid"))
        {
            tmp = obj.value("$oid").toString();
        }
        if (!tmp.isEmpty())
        {
            tmp = ("qx_oid:" + tmp); val = tmp;
        }
    }
#endif // _QX_ENABLE_MONGODB
...
{
    Q_UNUSED(pOwner);
    xerror("cannot access to the data-member pointer with Qt introspection engine");
    qAssert(false);
    return qx::any();
}
