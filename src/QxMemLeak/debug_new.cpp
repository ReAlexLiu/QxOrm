 ...
 * `<code>new_progname = getenv(QStringLiteral("_"));</code>'.
 */
...
static void* alloc_mem(size_t size, const char* file, int line, bool is_array)
{
    ...
    if (ptr == NULL)
    {
        ...
        xinfo("Out of memory when allocating {} bytes", size);
    ...
    if (new_verbose_flag)
    {
        ...
        if (line != 0)
        {
            const char* msg_out_file = "new{}: allocated {} (size {}, {}:{})"; Q_UNUSED(msg_out_file);
            xinfo(msg_out_file, (is_array ? "[]" : ""), pointer, size, ptr->file, ptr->line);
        }
#if (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
        else
        {
            const char* msg_out_no_file = "new{}: allocated {} (size {}, <Unknown>)"; Q_UNUSED(msg_out_no_file);
            xinfo(msg_out_no_file, (is_array ? "[]" : ""), pointer, size);
        }
#endif // (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
    }
...
void __debug_new_recorder::free_pointer(void* pointer, void* addr, bool is_array)
{
    ...
    if (ptr->magic != MAGIC)
    {
        {
            fast_mutex_autolock lock(new_output_lock);
            const char* msg_out = "delete{}: invalid pointer {}"; Q_UNUSED(msg_out);
            xinfo(msg_out, (is_array ? "[]" : ""), pointer);
        }
    ...
    if (is_array != ptr->is_array)
    {
        ...
        if (ptr->line != 0)
        {
            const char* msg_out_file = "{}: pointer {} (size {})\n\tat {}\n\toriginally allocated at {}:{}"; Q_UNUSED(msg_out_file);
            xinfo(msg_out_file, msg, ((char*)ptr + ALIGNED_LIST_ITEM_SIZE), ptr->size, addr, ptr->file, ptr->line);
        }
#if (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
        else
        {
            const char* msg_out_no_file = "{}: pointer {} (size {})\n\tat {}\n\toriginally allocated at <Unknown>"; Q_UNUSED(msg_out_no_file);
            xinfo(msg_out_no_file, msg, ((char*)ptr + ALIGNED_LIST_ITEM_SIZE), ptr->size, addr);
        }
#endif // (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
        fflush(new_output_fp);
        _DEBUG_NEW_ERROR_ACTION;
    ...
    {
        fast_mutex_autolock lock(new_output_lock);
        const char* msg_out = "delete{}: freed {} (size {}, {} bytes still allocated)"; Q_UNUSED(msg_out);
        xinfo(msg_out, (is_array ? "[]" : ""), ((char*)ptr + ALIGNED_LIST_ITEM_SIZE), ptr->size, total_mem_alloc);
    }
    free(ptr);
    return;
}
...
int check_leaks()
{
    ...
    while (ptr != &new_ptr_list)
    {
        ...
        if (ptr->magic != MAGIC)
        {
            const char* msg_out = "warning: heap data corrupt near {}"; Q_UNUSED(msg_out);
            xinfo(msg_out, pointer);
        }
#if _DEBUG_NEW_TAILCHECK
        if (!check_tail(ptr))
        {
            const char* msg_out = "warning: overwritten past end of object at {}"; Q_UNUSED(msg_out);
            xinfo(msg_out, pointer);
        }
#endif
        if (ptr->line != 0)
        {
            const char* msg_out_file = "Leaked object at {} (size {}, {}:{})"; Q_UNUSED(msg_out_file);
            xinfo(msg_out_file, pointer, ptr->size, ptr->file, ptr->line); ++leak_cnt;
        }
#if (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
        else
        {
            const char* msg_out_no_file = "Leaked object at {} (size {}, <Unknown>)"; Q_UNUSED(msg_out_no_file);
            xinfo(msg_out_no_file, pointer, ptr->size); ++leak_cnt;
        }
#endif // (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
        ptr = ptr->next;
    }
    ...
    {
        const char* msg_out_leak_cnt = "**** {} memory leaks found ****"; Q_UNUSED(msg_out_leak_cnt);
        xinfo(msg_out_leak_cnt, leak_cnt);
    }
    return leak_cnt;
}
...
int check_mem_corruption()
{
    ...
    fast_mutex_autolock lock_ptr(new_ptr_lock);
    fast_mutex_autolock lock_output(new_output_lock);
    const char* msg_out_check_mem_start = "**** Checking for memory corruption: START ****"; Q_UNUSED(msg_out_check_mem_start);
    xinfo(msg_out_check_mem_start);
    for (new_ptr_list_t* ptr = new_ptr_list.next; ptr != &new_ptr_list; ptr = ptr->next)
    {
        ...
        if (ptr->magic != MAGIC)
        {
            ...
            if (ptr->line != 0)
            {
                const char* msg_out_file = "Heap data corrupt near {} (size {}, {}:{})"; Q_UNUSED(msg_out_file);
                xinfo(msg_out_file, pointer, ptr->size, ptr->file, ptr->line); ++corrupt_cnt;
            }
#if (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
            else
            {
                const char* msg_out_no_file = "Heap data corrupt near {} (size {}, <Unknown>)"; Q_UNUSED(msg_out_no_file);
                xinfo(msg_out_no_file, pointer, ptr->size); ++corrupt_cnt;
            }
#endif // (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
#if _DEBUG_NEW_TAILCHECK
        }
        else
        {
            if (ptr->line != 0)
            {
                const char* msg_out_file = "Overwritten past end of object at {} (size {}, {}:{})"; Q_UNUSED(msg_out_file);
                xwarn(msg_out_file, pointer, ptr->size, ptr->file, ptr->line); ++corrupt_cnt;
            }
#if (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
            else
            {
                const char* msg_out_no_file = "Overwritten past end of object at {} (size {}, <Unknown>)"; Q_UNUSED(msg_out_no_file);
                xwarn(msg_out_no_file, pointer, ptr->size); ++corrupt_cnt;
            }
#endif // (! _QX_MEM_LEAK_ONLY_KNOWN_SRC_FILE)
        }
#endif
    }
    const char* msg_out_check_mem_end = "**** Checking for memory corruption: {} FOUND ****"; Q_UNUSED(msg_out_check_mem_end);
    xinfo(msg_out_check_mem_end, corrupt_cnt);
    return corrupt_cnt;
}
...
void __debug_new_recorder::_M_process(void* pointer)
{
    ...
    {
        fast_mutex_autolock lock(new_output_lock);
        const char* msg_out = "debug_new used with placement new ({}:{})"; Q_UNUSED(msg_out);
        xwarn(msg_out, _M_file, _M_line);
        return;
    }
#if _DEBUG_NEW_FILENAME_LEN == 0
    ptr->file = _M_file;
...
void operator delete(void* pointer, const char* file, int line) throw()
{
    if (qx::memory::new_verbose_flag)
    {
        ...
        const char* msg_out = "exception thrown on initializing object at {} ({}:{})"; Q_UNUSED(msg_out);
        xinfo(msg_out, pointer, file, line);
    }
...
void operator delete[] (void* pointer, const char* file, int line) throw()
{
    ...
    {
        qx::memory::fast_mutex_autolock lock(qx::memory::new_output_lock);
        const char* msg_out = "exception thrown on initializing objects at {} ({}:{})"; Q_UNUSED(msg_out);
        xinfo(msg_out, pointer, file, line);
    }
    operator delete[](pointer);
}
