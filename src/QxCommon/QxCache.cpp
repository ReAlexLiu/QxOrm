...
QxCache::QxCache() : qx::QxSingleton<QxCache>(QStringLiteral("qx::cache::detail::QxCache")), m_lMaxCost(999999999), m_lCurrCost(0) {  }
...
void QxCache::updateCost()
{
   while ((m_lCurrCost > m_lMaxCost) && (m_cache.count() > 0))
   {
      ...
      m_lCurrCost -= lCost;
      QString sMsg = QStringLiteral("qx::cache : auto remove object in cache '%1'").arg(sKey);
      xinfo(qPrintable(sMsg));
   }
}
