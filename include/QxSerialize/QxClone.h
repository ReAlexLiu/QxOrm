...
#define QX_STR_CLONE_SERIALIZATION_ERROR "serialization error : '{}'"
#define QX_STR_CLONE_DESERIALIZATION_ERROR "deserialization error : '{}'"
...
T* clone_to_nude_ptr(const T& obj)
{
    ...
    try
    {
        oar << obj; bSerializeOk = ioss.good();
    }
    catch (const boost::archive::archive_exception& e)
    {
        xfatal(QX_STR_CLONE_SERIALIZATION_ERROR, e.what());
    }
    ...
    {
        xfatal(QX_STR_CLONE_SERIALIZATION_ERROR, "unknown error");
    }
    if (!bSerializeOk)
    {
        qAssert(false);
    ...
    try
    {
        iar >> (*pClone); bDeserializeOk = ioss.good();
    }
    catch (const boost::archive::archive_exception& e)
    {
        xfatal(QX_STR_CLONE_DESERIALIZATION_ERROR, e.what());
    }
    catch (const std::exception& e)
    {
        xfatal(QX_STR_CLONE_DESERIALIZATION_ERROR, e.what());
    ...
    {
        xfatal(QX_STR_CLONE_DESERIALIZATION_ERROR, "unknown error");
    }
...
T* clone_to_nude_ptr(const T& obj)
{
    ...
    if (baClone.isEmpty())
    {
        qAssertMsg(false, "an error occurred during QDataStream serialization process"); 
        return NULL;
    }
    T* pClone = new T();
