...
void dump(const T& t, bool bJsonFormat = false)
{
    ...
    xinfo("start dump '{}'", qPrintable(sName));
    xinfo(qPrintable(sDump));
    xinfo("end dump '{}'", qPrintable(sName));
}
...
{
#ifdef _QX_NO_JSON
    xwarn("not implemented when _QX_ENABLE_BOOST_SERIALIZATION compilation option is not defined (XML format) and _QX_NO_JSON compilation option is defined (JSON format)");
