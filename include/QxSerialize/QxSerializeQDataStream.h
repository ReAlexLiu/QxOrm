...
inline QByteArray to_byte_array(const T& obj, void* owner = NULL, unsigned int flags = 1 /* boost::archive::no_header */)
{
    ...
    {
        err = QStringLiteral("serialization error '%ERR%'").replace(QStringLiteral("%ERR%"), QLatin1String(e.what()));
    }
    catch (...)
    {
        err = QStringLiteral("serialization error '%ERR%'").replace(QStringLiteral("%ERR%"), QStringLiteral("unknown error"));
    }
    if (!err.isEmpty())
    {
        xwarn(qPrintable(err)); 
        ba.clear();
    }
    return ba;
}
...
inline qx_bool from_byte_array(T& obj, const QByteArray& data, unsigned int flags = 1 /* boost::archive::no_header */)
{
    ...
    try
    {
        stream >> obj; result = true;
    }
    catch (const std::exception& e)
    {
        result.setDesc(QStringLiteral("deserialization error '%ERR%'").replace(QStringLiteral("%ERR%"), QLatin1String(e.what())));
    }
    catch (...)
    {
        result.setDesc(QStringLiteral("deserialization error '%ERR%'").replace(QStringLiteral("%ERR%"), QStringLiteral("unknown error")));
    }
    if (!result.getDesc().isEmpty())
    {
        QString msg = result.getDesc(); 
        xwarn(qPrintable(msg));
    }
    return result;
}
...
{
    return QLatin1String(qx::serialization::qt::to_byte_array(obj, NULL, flags).toBase64());
}
