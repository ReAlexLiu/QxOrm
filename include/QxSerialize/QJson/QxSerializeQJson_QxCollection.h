...
struct QxConvert_ToJson< qx::QxCollection<Key, Value> >
{
   static inline QJsonValue toJson(const qx::QxCollection<Key, Value> & t, const QString & format)
   {
      ...
      {
         QJsonObject obj;
         val = qx::cvt::to_json(t.getKeyByIndex(l), format); obj.insert(QStringLiteral("key"), val);
         val = qx::cvt::to_json(t.getByIndex(l), format); obj.insert(QStringLiteral("value"), val);
         arr.append(obj);
      }
