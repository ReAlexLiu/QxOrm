...
public:
    ...
    virtual ~QxClientAsync()
    {
        if (isRunning())
        {
            xinfo("thread is running : wait thread finished");
            wait();
        }
    }
    ...
    {
        qAssert(!isRunning()); m_pService = service; m_sServiceMethod = method;
    }
...
protected:
    ...
    virtual void run()
    {
        qx::service::execute_client(m_pService.get(), m_sServiceMethod);
    }
