...
public:
    ...
    QString getSqlPlaceHolder(const QString& sAppend = QString(), int iIndexName = 0, const QString& sSep = QStringLiteral(", "), const QString& sOtherName = QString(), bool bCheckFKPartOfPK = false) const;
    ...
    QString getSqlAliasEqualToPlaceHolder(const QString& sTable = QString(), bool bClauseWhere = false, const QString& sAppend = QString(), const QString& sSep = QStringLiteral(" AND "), bool bCheckFKPartOfPK = false) const;
    QString getSqlNameEqualToPlaceHolder(const QString& sAppend = QString(), const QString& sSep = QStringLiteral(" AND "), bool bCheckFKPartOfPK = false) const;
    QString getSqlTablePointNameAsAlias(const QString& sTable, const QString& sSep = QStringLiteral(", "), const QString& sSuffixAlias = QString(), bool bCheckFKPartOfPK = false, const QString& sCustomAlias = QString()) const;
    QString getSqlName(const QString& sSep = QStringLiteral(", "), const QString& sOtherName = QString(), bool bCheckFKPartOfPK = false) const;
    QString getSqlNameAndTypeAndParams(const QString& sSep = QStringLiteral(", "), const QString& sOtherName = QString(), bool bCheckFKPartOfPK = false) const;
...
public:
    ...
    T* getValuePtr(void* pOwner, bool* bOk = NULL)
    {
        ...
        {
            xwarn("cannot access data-member pointer"); return NULL;
        }
        qx::any a = this->getDataPtr(pOwner);
        try
        {
            T* t = qx::any_cast<T*>(a); if (bOk)
            {
                (*bOk) = (t != NULL);
            }; return t;
        }
        ...
        {
            Q_UNUSED(err);
            xfatal("bad any cast exception"); return NULL;
        }
        catch (...)
        {
            xfatal("unknown cast exception"); return NULL;
        }
    }
...
private:
    ...
    struct qxCannotAccessDataPointer
    {
        ...
        {
            Q_UNUSED(pData); Q_UNUSED(pOwner);
            xwarn("type T not supported");
            if (bOk)
            {
                (*bOk) = false;
            }; return T();
        ...
        {
            Q_UNUSED(pData); Q_UNUSED(pOwner); Q_UNUSED(val);
            xwarn("type T not supported"); return false;
        }
    };
