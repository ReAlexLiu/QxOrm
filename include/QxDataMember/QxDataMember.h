...
public:
    ...
    {
        return (pOwner ? (&((static_cast<Owner*>(pOwner))->*m_pData)) : NULL);
    }
    inline const DataType* getData(const void* pOwner) const
    {
        return (pOwner ? (&((static_cast<const Owner*>(pOwner))->*m_pData)) : NULL);
    }
    ...
    {
        return qx::cvt::to_variant((*getData(pOwner)), sFormat, iIndexName, ctx);
    }
    ...
    {
        return qx::cvt::from_variant(v, (*getData(pOwner)), sFormat, iIndexName, ctx);
    }
    virtual QString getType() const
    {
        return QLatin1String(qx::trait::get_class_name<DataType>::get());
    }

#ifndef _QX_NO_JSON
    virtual QJsonValue toJson(const void* pOwner, const QString& sFormat) const
    {
        return qx::cvt::to_json((*getData(pOwner)), sFormat);
    }
    virtual qx_bool fromJson(void* pOwner, const QJsonValue& j, const QString& sFormat)
    {
        return qx::cvt::from_json(j, (*getData(pOwner)), sFormat);
    }
#endif // _QX_NO_JSON
...
protected:
    ...
    {
        return static_cast<void*>(const_cast<DataType*>(getData(pOwner)));
    }
    virtual void* getDataVoidPtr(void* pOwner)
    {
        return static_cast<void*>(getData(pOwner));
    }
...
private:
    ...
    struct qxCompareDataMember
    {
        ...
        {
            return (dataMember.toVariant(pOwner1, QStringLiteral("")) == dataMember.toVariant(pOwner2, QStringLiteral("")));
        }
    };
