...
if (! bValid) { QString sMsgDebug = bValid.getDesc(); xdebug(qPrintable(sMsgDebug)); qAssert(false); } \
return bValid;

#define QX_FUNCTION_INVOKE_START_WITH_OWNER() \
if (ret) { (* ret) = qx::any(); } \
qx_bool bValid = pThis->isValid<T, Owner>(pOwner, params, NULL); \
if (! bValid) { QString sMsgDebug = bValid.getDesc(); xdebug(qPrintable(sMsgDebug)); qAssert(false); return bValid; }
...
if (! bTmp) { QString sMsgDebug = bTmp.getDesc(); xdebug(qPrintable(sMsgDebug)); qAssert(false); return bTmp; } }
