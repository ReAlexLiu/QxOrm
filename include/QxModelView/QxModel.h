...
public:
    ...
    enum
    {
        qx_is_valid = (qx::trait::is_qx_registered<T>::value && std::is_base_of<qx::IxModel, B>::value)
    };
...
public:
    ...
    QxModel(QObject* parent = 0) : B(parent)
    {
        qx::QxModel<T, B>::init();
    }
    QxModel(qx::IxModel* other, QObject* parent) : B(parent)
    {
        qx::QxModel<T, B>::initFrom(other);
    }
    virtual ~QxModel()
    {}
...
protected:
    ...
    void initFrom(qx::IxModel* pOther)
    {
        ...
        if (this->m_pParent)
        {
            this->m_eAutoUpdateDatabase = this->m_pParent->getAutoUpdateDatabase();
        }
...
public:
    ...
    {
        IxDataMember* pDataMember = this->getDataMember(column); if (!pDataMember)
        {
            return;
        }
    ...
    {
        return m_pDirtyRow.get();
    }

    virtual void setShowEmptyLine(bool b)
    {
        if (b == getShowEmptyLine())
        {
            return;
        }
        if (b)
...
public:
     ...
     * \param relation List of relationships keys to be fetched (eager fetch instead of default lazy fetch for a relation) : use QStringLiteral("|") separator to put many relationships keys into this parameter
     * \param pDatabase Connection to database (you can manage your own connection pool for example, you can also define a transaction, etc.); if NULL, a valid connection for the current thread is provided by qx::QxSqlDatabase singleton class (optional parameter)
    ...
    virtual QSqlError qxFetchById(const QVariant& id, const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        {
            xwarn("data member id not registered"); qAssert(false);
        }
        if (!this->m_pDataMemberId)
        {
            this->m_lastError = QSqlError("problem with 'qxFetchById()' method : 'data member id not registered'", QStringLiteral(""), QSqlError::UnknownError); return this->m_lastError;
        }
        ...
        {
            this->m_lastError = qx::dao::fetch_by_id_with_relation(relation, (*pItem), this->database(pDatabase));
        }
    ...
    virtual QSqlError qxFetchAll(const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::fetch_all(tmp, this->database(pDatabase), this->m_lstColumns);
        }
        else
        {
            this->m_lastError = qx::dao::fetch_all_with_relation(relation, tmp, this->database(pDatabase));
        }
    ...
    virtual QSqlError qxFetchByQuery(const qx::QxSqlQuery& query, const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::fetch_by_query(query, tmp, this->database(pDatabase), this->m_lstColumns);
        }
        else
        {
            this->m_lastError = qx::dao::fetch_by_query_with_relation(relation, query, tmp, this->database(pDatabase));
        }
    ...
    {
        type_ptr pItem = getRowItemAt(row); if (!pItem)
        {
            return QSqlError();
        }
        if (relation.count() == 0)
     ...
     * \param relation List of relationships keys to be inserted in others tables of database : use QStringLiteral("|") separator to put many relationships keys into this parameter
     * \param pDatabase Connection to database (you can manage your own connection pool for example, you can also define a transaction, etc.); if NULL, a valid connection for the current thread is provided by qx::QxSqlDatabase singleton class (optional parameter)
     * \return Empty QSqlError object (from Qt library) if no error occurred; otherwise QSqlError contains a description of database error executing SQL query
     */
    virtual QSqlError qxInsert(const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::insert(m_model, this->database(pDatabase));
        }
        else
        {
            this->m_lastError = qx::dao::insert_with_relation(relation, m_model, this->database(pDatabase));
        }
        if (!this->m_lastError.isValid())
        {
            this->updateAllKeys();
    ...
    virtual QSqlError qxInsertRow(int row, const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::insert((*pItem), this->database(pDatabase));
        }
        else
        {
            this->m_lastError = qx::dao::insert_with_relation(relation, (*pItem), this->database(pDatabase));
        }
        if (!this->m_lastError.isValid())
        {
            updateKey(row);
    ...
    virtual QSqlError qxUpdate(const qx::QxSqlQuery& query = qx::QxSqlQuery(), const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::update_by_query(query, m_model, this->database(pDatabase), this->m_lstColumns);
        }
        else
        {
            this->m_lastError = qx::dao::update_by_query_with_relation(relation, query, m_model, this->database(pDatabase));
        }
        if (!this->m_lastError.isValid())
        {
            this->updateAllKeys();
    ...
    virtual QSqlError qxUpdateRow(int row, const qx::QxSqlQuery& query = qx::QxSqlQuery(), const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        type_ptr pItem = m_model.getByIndex(row); if (!pItem)
        {
            return QSqlError();
        }
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::update_by_query(query, (*pItem), this->database(pDatabase), this->m_lstColumns);
        }
        else
        {
            this->m_lastError = qx::dao::update_by_query_with_relation(relation, query, (*pItem), this->database(pDatabase));
        }
        if (!this->m_lastError.isValid())
        {
            updateKey(row);
    ...
    virtual QSqlError qxSave(const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::save(m_model, this->database(pDatabase));
        }
        else
        {
            this->m_lastError = qx::dao::save_with_relation(relation, m_model, this->database(pDatabase));
        }
        if (!this->m_lastError.isValid())
        {
            this->updateAllKeys();
    ...
    virtual QSqlError qxSaveRow(int row, const QStringList& relation = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (relation.count() == 0)
        {
            this->m_lastError = qx::dao::save((*pItem), this->database(pDatabase));
        }
        else
        {
            this->m_lastError = qx::dao::save_with_relation(relation, (*pItem), this->database(pDatabase));
        }
        if (!this->m_lastError.isValid())
        {
            updateKey(row);
    ...
    virtual QSqlError qxSaveRowData(int row, const QStringList& column = QStringList(), QSqlDatabase* pDatabase = NULL)
    {
        if (!this->m_pDataMemberId)
        {
            this->m_lastError = QSqlError("problem with 'qxSaveRowData()' method : 'data member id not registered'", QStringLiteral(""), QSqlError::UnknownError); return this->m_lastError;
        }
        type_ptr pItem = getRowItemAt(row); if (!pItem)
        {
            return QSqlError();
        ...
        if (bExist)
        {
            bExist = qx::dao::exist((*pItem), this->database(pDatabase));
        }
        if (bExist)
        {
            this->m_lastError = qx::dao::update((*pItem), this->database(pDatabase), column);
        }
        else
        {
            this->m_lastError = qx::dao::insert((*pItem), this->database(pDatabase));
    ...
    virtual QSqlError qxDeleteById(const QVariant& id, QSqlDatabase* pDatabase = NULL)
    {
        ...
        if (!this->m_pDataMemberId)
        {
            this->m_lastError = QSqlError("problem with 'qxDeleteById()' method : 'data member id not registered'", QStringLiteral(""), QSqlError::UnknownError); return this->m_lastError;
        }
        this->m_pDataMemberId->fromVariant(pItem.get(), id);
...
protected:
    ...
    type_ptr getRowItemAt(int row) const
    {
        ...
        if (m_pDirtyRow && (m_pDirtyRow->first == row))
        {
            return m_pDirtyRow->second;
        }
        return type_ptr();
    }
    ...
    {
        return getRowItemAt(row).get();
    }

    virtual void dumpModelImpl(bool bJsonFormat) const
    {
        qx::dump(m_model, bJsonFormat);
    }
    ...
    virtual void updateShowEmptyLine()
    {
        if (m_pDirtyRow)
        {
            m_pDirtyRow->first = m_model.count();
        }
    }
    ...
    virtual void insertDirtyRowToModel()
    {
        ...
        if (this->m_pParent)
        {
            this->m_pParent->saveChildRelations(this);
        }
        updateKey(row);
        addDirtyRow();
    }
    ...
    void updateKey(int row)
    {
        ...
        type_ptr pItem = m_model.getByIndex(row); if (!pItem || !this->m_pDataMemberId)
        {
            return;
        }
        ...
        if ((vCurrPrimaryKey == vNextPrimaryKey) || (!vNextPrimaryKey.isValid()))
        {
            return;
        }
        if (!qx::trait::is_valid_primary_key(vNextPrimaryKey))
        {
            return;
        }
        type_primary_key updatedPrimaryKey;
        qx::cvt::from_variant(vNextPrimaryKey, updatedPrimaryKey);
        if (m_model.exist(updatedPrimaryKey))
        {
            return;
        }
...
protected:
    ...
    virtual QString toJson_Helper(int row) const
    {
        if (row == -1)
        {
            return qx::serialization::json::to_string(m_model);
        }
        type_ptr pItem = getRowItemAt(row); if (!pItem)
        {
            return QString();
    ...
    virtual bool fromJson_Helper(const QString& json, int row)
    {
        if (row == -1)
        {
            this->clear();
            type_collection tmp;
            if (!qx::serialization::json::from_string(tmp, json))
            {
                return false;
            }
            this->beginResetModel();
        ...
        if (!qx::serialization::json::from_string((*pItem), json))
        {
            return false;
        }
    ...
    virtual QVariant getRelationshipValues_Helper(int row, const QString& relation, bool bLoadFromDatabase, const QString& sAppendRelations)
    {
        ...
        IxDataMember* pDataMember = this->m_pDataMemberX->get_WithDaoStrategy(relation); if (!pDataMember)
        {
            return QVariant();
        }
        IxSqlRelation* pRelation = (pDataMember->hasSqlRelation() ? pDataMember->getSqlRelation() : NULL); if (!pRelation)
        {
            return QVariant();
        }
        type_ptr pItem = m_model.getByIndex(row); if (!pItem)
        ...
        QJsonValue json = pDataMember->toJson(pItemTemp.get()); if (json.isNull())
        {
            return QVariant();
        }
        if (json.isArray())
    ...
    virtual bool setRelationshipValues_Helper(int row, const QString& relation, const QVariant& values)
    {
        ...
        if ((values.type() != QVariant::List) && (values.type() != QVariant::Map))
        {
            return false;
        }
        if (!this->m_pDataMemberId || !this->m_pDataMemberX || !this->m_pDataMemberX->exist(relation))
        ...
        {
            json = QJsonArray::fromVariantList(values.toList());
        }
        else if (values.type() == QVariant::Map)
        {
            json = QJsonObject::fromVariantMap(values.toMap());
        }
        if (!pDataMember->fromJson(pItem.get(), json))
        {
            return false;
        }
