...
public:
   ...
   inline void setDate(const QDate & date)            { m_neutral = QStringLiteral(""); m_date = date; update(); }
...
private:
   ...
   void update()
   {
      ...
      else if (m_date.isValid()) { m_neutral = m_date.toString(QLatin1String(format())); }
      else { qAssert(m_neutral.size() == QLatin1Literal(format()).size()); m_date = QDate::fromString(m_neutral, QLatin1String(format())); qAssert(m_date.isValid()); }
   }
