...
public:
    ...
    * \param relation List of relationships keys to fetch (eager fetch instead of default lazy fetch for a relation) : use QStringLiteral("|") separator to put many relationships keys into this parameter
    * \param pDatabase Connection to database (you can manage your own connection pool for example, you can also define a transaction, etc.); if NULL, a valid connection for the current thread is provided by qx::QxSqlDatabase singleton class
    ...
    * \param relation List of relationships keys to insert in others tables of database : use QStringLiteral("|") separator to put many relationships keys into this parameter
    * \param pDatabase Connection to database (you can manage your own connection pool for example, you can also define a transaction, etc.); if NULL, a valid connection for the current thread is provided by qx::QxSqlDatabase singleton class
    * \return Empty QSqlError object (from Qt library) if no error occurred; otherwise QSqlError contains a description of database error executing SQL query
    *
    * qx::IxPersistable * p = ...;<br>
    ...
    * \param relation List of relationships keys to update in others tables of database : use QStringLiteral("|") separator to put many relationships keys into this parameter
    * \param pDatabase Connection to database (you can manage your own connection pool for example, you can also define a transaction, etc.); if NULL, a valid connection for the current thread is provided by qx::QxSqlDatabase singleton class
    * \return Empty QSqlError object (from Qt library) if no error occurred; otherwise QSqlError contains a description of database error executing SQL query
    *
    * qx::IxPersistable * p = ...;<br>
    ...
    * \param relation List of relationships keys to insert or update in others tables of database : use QStringLiteral("|") separator to put many relationships keys into this parameter
    * \param pDatabase Connection to database (you can manage your own connection pool for example, you can also define a transaction, etc.); if NULL, a valid connection for the current thread is provided by qx::QxSqlDatabase singleton class
...
{ \
   ...
   { \
      ...
      if (! pDataMemberId) { xerror("method : 'data member id not registered'"); qAssert(false); } \
      if (! pDataMemberId) { return QSqlError("problem with 'qxFetchById()' method : 'data member id not registered'", QStringLiteral(""), QSqlError::UnknownError); } \
...
{ \
   ...
   if (! list_typed) { xerror("method : 'dynamic_cast failed using collection qx::QxCollection< type_primary_key, std::shared_ptr<type> >'"); qAssert(false); } \
   if (! list_typed) { return QSqlError("problem with 'qxFetchAll()' method : 'dynamic_cast failed using collection qx::QxCollection< type_primary_key, std::shared_ptr<type> >'", QStringLiteral(""), QSqlError::UnknownError); } \
   QSqlError err; \
...
{ \
   QX_PERSISTABLE_CAST_COLLECTION(className) \
   if (! list_typed) { xerror("method : 'dynamic_cast failed using collection qx::QxCollection< type_primary_key, std::shared_ptr<type> >'"); qAssert(false); } \
   if (! list_typed) { return QSqlError("problem with 'qxFetchByQuery()' method : 'dynamic_cast failed using collection qx::QxCollection< type_primary_key, std::shared_ptr<type> >'", QStringLiteral(""), QSqlError::UnknownError); } \
   QSqlError err; \
...
{ \
   ...
   { \
      ...
      qx::IxDataMember * pDataMemberId = (pDataMemberX ? pDataMemberX->getId_WithDaoStrategy() : NULL); \
      if (! pDataMemberId) { xerror("method : 'data member id not registered'"); qAssert(false); } \
      if (! pDataMemberId) { return QSqlError("problem with 'qxDeleteById()' method : 'data member id not registered'", QStringLiteral(""), QSqlError::UnknownError); } \
      pDataMemberId->fromVariant(this, id, -1, qx::cvt::context::e_database); \
   } \
...
{ \
   ...
   { \
      ...
      qx::IxDataMember * pDataMemberId = (pDataMemberX ? pDataMemberX->getId_WithDaoStrategy() : NULL); \
      if (! pDataMemberId) { xerror("method : 'data member id not registered'"); qAssert(false); } \
      if (! pDataMemberId) { return QSqlError("problem with 'qxDestroyById()' method : 'data member id not registered'", QStringLiteral(""), QSqlError::UnknownError); } \
      pDataMemberId->fromVariant(this, id, -1, qx::cvt::context::e_database); \
   } \
...
{ \
   QX_PERSISTABLE_CAST_COLLECTION(className) \
   if (! list_typed) { xerror("method : 'dynamic_cast failed using collection qx::QxCollection< type_primary_key, std::shared_ptr<type> >'"); qAssert(false); } \
   if (! list_typed) { return QSqlError("problem with 'qxExecuteQuery()' method : 'dynamic_cast failed using collection qx::QxCollection< type_primary_key, std::shared_ptr<type> >'", QStringLiteral(""), QSqlError::UnknownError); } \
