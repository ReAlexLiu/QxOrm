...
public:
   ...
   sql_error(const QSqlError & err) : std::exception(), m_error(err){if (! m_error.text().isEmpty() && (m_error.type() == QSqlError::NoError)){m_error = QSqlError(m_error.driverText(), m_error.databaseText(), QSqlError::UnknownError);};m_errorMessage = m_error.text().toLocal8Bit();}
