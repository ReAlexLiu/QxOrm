...
public:
   ...
   inline void setTime(const QTime & time)            { m_neutral = QLatin1Literal(""); m_time = time; update(); }
...
private:
   ...
   void update()
   {
      ...
      else if (m_time.isValid()) { m_neutral = m_time.toString(QLatin1String(format())); }
      else { qAssert(m_neutral.size() == QLatin1String(format()).size()); m_time = QTime::fromString(m_neutral, QLatin1String(format())); qAssert(m_time.isValid()); }
   }
