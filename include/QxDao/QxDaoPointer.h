...
if (bDirty) { qDebug("test dirty 1 : blog is dirty => '%s'", qPrintable(lstDiff.join(QStringLiteral("|")))); }
...
if (bDirty) { qDebug("test dirty 2 : container is dirty => '%s'", qPrintable(lstDiff.join(QStringLiteral("|")))); }

author_ptr_dirty = container_isdirty->at(2);
author_ptr_dirty->m_birthdate = QDate(1998, 03, 06);
bDirty = container_isdirty.isDirty(lstDiff);
qAssert(bDirty && (lstDiff.count() == 2));
if (bDirty) { qDebug("test dirty 3 : container is dirty => '%s'", qPrintable(lstDiff.join(QStringLiteral("|")))); }
...
public:
   ...
   {
      lstDiff.clear();
      if (m_pWork.isNull() || m_pOriginal.isNull()) { lstDiff.append(QStringLiteral("*")); return true; }
