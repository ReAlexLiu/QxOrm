...
#define QX_REPOSITORY_COLLECTION_DYNAMIC_CAST_ERROR   QSqlError("qx::QxRepository<T> : 'invalid collection pointer, dynamic_cast failed'", QStringLiteral(""), QSqlError::UnknownError)
#define QX_REPOSITORY_POINTER_DYNAMIC_CAST_ERROR      QSqlError("qx::QxRepository<T> : 'invalid pointer, dynamic_cast failed'", QStringLiteral(""), QSqlError::UnknownError)
#define QX_REPOSITORY_QOBJECT_BASE_CLASS_ERROR        QSqlError("qx::QxRepository<T> : 'invalid pointer, need to inherit from QObject class to use qx::IxRepository interface'", QStringLiteral(""), QSqlError::UnknownError)
...
public:
   ...
   QxRepository() : IxRepository(false, QStringLiteral("")) {  }
   QxRepository(const QSqlDatabase & database) : IxRepository(false, QStringLiteral(""), database) {  }
   QxRepository(QxSession * pSession) : IxRepository(false, QStringLiteral(""), pSession) {  }
