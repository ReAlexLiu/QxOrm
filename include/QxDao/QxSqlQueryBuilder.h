...
#define QX_SQL_ERR_NO_DATA_MEMBER_REGISTERED    "'qx::register_class()' not called or no data member registered"
#define QX_SQL_ERR_NO_ID_REGISTERED             "no id registered"
...
QString key = QxClass<type_sql>::getSingleton()->getKey() + joinQueryHash + QStringLiteral(oper); \
...
qx::dao::detail::IxDao_Timer timer(this->getDaoHelper(), qx::dao::detail::IxDao_Helper::timer_build_sql); \
QString joinQueryHash = (this->getDaoHelper() ? this->getDaoHelper()->qxQuery().getJoinQueryHash() : QString()); \
QString key = QxClass<type_sql>::getSingleton()->getKey() + joinQueryHash + this->getHashRelation() + QStringLiteral(oper); \
...
public:
    ...
    {
        static_assert(qx::trait::is_qx_registered<type_sql>::value, "qx::trait::is_qx_registered<type_sql>::value");
    }
...
public:
    ...
    virtual IxSqlQueryBuilder& buildSql(const QStringList& columns = QStringList(), QxSqlRelationLinked* pRelationX = NULL)
    {
        ...
        sql = QStringLiteral("SELECT COUNT(*) FROM ") + qx::IxDataMember::getSqlFromTable(this->table());
        if (!this->softDelete().isEmpty())
        {
            sql += QStringLiteral(" WHERE ") + this->softDelete().buildSqlQueryToFetch();
        }
...
public:
    ...
    virtual IxSqlQueryBuilder& buildSql(const QStringList& columns = QStringList(), QxSqlRelationLinked* pRelationX = NULL)
    {
        ...
        if (!this->getDataId())
        {
            xerror(QX_SQL_ERR_NO_ID_REGISTERED); qAssert(false); return (*this);
        }
...
public:
    ...
    virtual IxSqlQueryBuilder& buildSql(const QStringList& columns = QStringList(), QxSqlRelationLinked* pRelationX = NULL)
    {
        ...
        sql = QStringLiteral("DELETE FROM ") + this->table();
        this->setSqlQuery(sql, key);
        return (*this);
    }
...
public:
    ...
    virtual IxSqlQueryBuilder& buildSql(const QStringList& columns = QStringList(), QxSqlRelationLinked* pRelationX = NULL)
    {
        ...
        if (!this->softDelete().isEmpty())
        {
            sql = QStringLiteral("UPDATE ") + this->table() + QStringLiteral(" SET ") + this->softDelete().buildSqlQueryToUpdate();
        }
        else
        {
            qAssert(false);
...
public:
    ...
    virtual IxSqlQueryBuilder& buildSql(const QStringList& columns = QStringList(), QxSqlRelationLinked* pRelationX = NULL)
    {
        ...
        if (!this->getDataId())
        {
            xerror(QX_SQL_ERR_NO_ID_REGISTERED);
            qAssert(false);
            return (*this);
        }
