...
public:
   ...
   virtual QString createExtraTable() const                                   { return QStringLiteral(""); }
   ...
   virtual void * eagerFetch_ResolveOutput(QxSqlRelationParams & params) const
   {
      ...
      for (int i = 0; i < pForeign->getNameCount(); i++)
      { QVariant v = query.value(lOffsetOld + lOffsetId + i); pForeign->fromVariant((& item_val), v, QStringLiteral(""), i, qx::cvt::context::e_database); }
...
private:
   ...
   void forceParentIdToAllChildren(QxSqlRelationParams & params) const
   {
      ...
      while (itr != itr_end)
      {
         ...
         { pForeign->fromVariant((& item_val), vIdOwner.at(i), QStringLiteral(""), i, qx::cvt::context::e_database); }
