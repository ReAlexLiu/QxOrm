...
public:
    ...
    {
        this->setRelationType(qx::IxSqlRelation::many_to_many); this->setExtraTable(sExtraTable); this->setForeignKeyOwner(sForeignKeyOwner); this->setForeignKeyDataType(sForeignKeyDataType); this->verifyParameters();
    }
    virtual ~QxSqlRelation_ManyToMany()
    {
        static_assert(is_data_container, "is_data_container");
    }
    ...
    {
        Q_UNUSED(params); return QSqlError();
    }
    ...
    virtual void* eagerFetch_ResolveOutput(QxSqlRelationParams& params) const
    {
        ...
        for (int i = 0; i < pId->getNameCount(); i++)
        {
            QVariant v = query.value(lOffsetOld + i); qx::cvt::from_variant(v, item.key(), QStringLiteral(""), i, qx::cvt::context::e_database);
        }
        for (int i = 0; i < pId->getNameCount(); i++)
        {
            QVariant v = query.value(lOffsetOld + i); pId->fromVariant((&item_val), v, QStringLiteral(""), i, qx::cvt::context::e_database);
        }
        ...
        while ((p = this->nextData(lIndex)))
        {
            ...
            {
                p->fromVariant((&item_val), query.value(lCurrIndex + lOffsetRelation), -1, qx::cvt::context::e_database); lCurrIndex++;
            }
        }
...
private:
    ...
    QSqlError insertIntoExtraTable(QxSqlRelationParams& params) const
    {
        ...
        QStringList lstForeignKeyOwner = this->getForeignKeyOwner().split(QStringLiteral("|"));
        QStringList lstForeignKeyDataType = this->getForeignKeyDataType().split(QStringLiteral("|"));
        ...
        sql += ") VALUES (";
        sql += pIdOwner->getSqlPlaceHolder("", -1, ", ", this->getForeignKeyOwner()) + ", " + pIdData->getSqlPlaceHolder("", -1, ", ", this->getForeignKeyDataType()) + QStringLiteral(")");
        ...
        while (itr != itr_end)
        {
            pIdOwner->setSqlPlaceHolder(queryInsert, params.owner(), QStringLiteral(""), this->getForeignKeyOwner());
            pIdData->setSqlPlaceHolder(queryInsert, (&item.value_qx()), QStringLiteral(""), this->getForeignKeyDataType());
            if (!queryInsert.exec())
            {
                return queryInsert.lastError();
            }
