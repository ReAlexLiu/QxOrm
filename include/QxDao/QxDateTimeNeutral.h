...
public:
   ...
   inline void setDateTime(const QDateTime & dt)      { m_neutral = QStringLiteral(""); m_dt = dt; update(); }
...
private:
   ...
   void update()
   {
      ...
      else if (m_dt.isValid()) { m_neutral = m_dt.toString(QLatin1String(format())); }
      else { qAssert(m_neutral.size() == QLatin1String(format()).size()); m_dt = QDateTime::fromString(m_neutral, QLatin1String(format())); qAssert(m_dt.isValid()); }
   }
