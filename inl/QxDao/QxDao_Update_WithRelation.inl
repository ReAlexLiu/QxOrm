...
struct QxDao_Update_WithRelation_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("update with relation"), new qx::QxSqlQueryBuilder_Update<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
...
struct QxDao_Update_WithRelation_Container
{
    ...
    static QSqlError update(const QStringList& relation, const qx::QxSqlQuery& query, T& t, QSqlDatabase* pDatabase)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("update with relation"), new qx::QxSqlQueryBuilder_Update<type_item>());
        if (!dao.isValid())
        {
            return dao.error();
        }
