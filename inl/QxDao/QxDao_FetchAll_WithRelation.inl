...
struct QxDao_FetchAll_WithRelation_Generic
{
    ...
    {
        type_dao_helper dao(t, pDatabase, QStringLiteral("fetch all with relation"), new qx::QxSqlQueryBuilder_FetchAll_WithRelation<T>(), (&query));
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            qx::dao::on_before_fetch<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            QString json = qx::serialization::json::to_string(t, 1, QStringLiteral("mongodb:only_id"));
            ...
            qx::serialization::json::from_string(t, json, 1, QStringLiteral("mongodb"));
            qx::dao::on_after_fetch<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            return dao.error();
        ...
        if (!query.isEmpty())
        {
            dao.addQuery(true); sql = dao.builder().getSqlQuery();
        }
        if (!dao.exec())
        {
...
private:
    ...
    struct fetcherHelper
    {
        static void insertNewItem(T& t, type_dao_helper& dao, const QString& json)
        {
            ...
            qx::dao::detail::IxDao_Timer timer((&dao), qx::dao::detail::IxDao_Helper::timer_cpp_build_instance);
            qx::serialization::json::from_string(item_val_tmp, json, 1, QStringLiteral("mongodb:columns{,_id,%1,}").arg(pId->getKey()));
            for (int i = 0; i < (pId ? pId->getNameCount() : 0); i++)
            {
                QVariant id = pId->toVariant((&item_val_tmp), QStringLiteral(""), i, qx::cvt::context::e_database);
                qx::cvt::from_variant(id, item.key(), QStringLiteral(""), i, qx::cvt::context::e_database);
            }
            ...
            qx::dao::on_before_fetch<type_value_qx>((&item_val), (&dao)); if (!dao.isValid())
            {
                return;
            }
            qx::serialization::json::from_string(item_val, json, 1, QStringLiteral("mongodb"));
            qx::dao::on_after_fetch<type_value_qx>((&item_val), (&dao)); if (!dao.isValid())
            {
                return;
            }
    ...
    struct fetcherHelper<true, dummy>
    {
        static void insertNewItem(T& t, type_dao_helper& dao, const QString& json)
        {
            ...
            for (int i = 0; i < (pId ? pId->getNameCount() : 0); i++)
            {
                QVariant id = pId->toVariant((&item_val), QStringLiteral(""), i, qx::cvt::context::e_database);
                qx::cvt::from_variant(id, item.key(), QStringLiteral(""), i, qx::cvt::context::e_database);
            }
            qx::dao::on_after_fetch<type_value_qx>((&item_val), (&dao)); if (!dao.isValid())
            {
...
struct QxDao_FetchAll_WithRelation_Container
{
    ...
    static QSqlError fetchAll(const QStringList& relation, const qx::QxSqlQuery& query, T& t, QSqlDatabase* pDatabase)
    {
        ...
        type_dao_helper dao(t, pDatabase, QStringLiteral("fetch all with relation"), new qx::QxSqlQueryBuilder_FetchAll_WithRelation<type_value_qx>(), (&query));
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        while (dao.nextRecord())
        {
            ...
            if (pId)
            {
                QString sId; for (int i = 0; i < pId->getNameCount(); i++)
                {
                    sId += dao.query().value(i).toString() + QStringLiteral("|");
                };
                vId = sId;
            }
    ...
    struct insertHelper
    {
        static void insertNewItem(T& t, type_dao_helper& dao)
        {
            ...
            if (pId)
            {
                ...
                {
                    QVariant v = dao.query().value(i); qx::cvt::from_variant(v, item.key(), QStringLiteral(""), i, qx::cvt::context::e_database);
                }
            }
            type_value* pValue = type_generic_container::insertItem(t, item);
            type_value_qx& item_val = (pValue ? (*static_cast<type_value_qx*>(pValue)) : item.value_qx());
...
struct QxDao_FetchAll_WithRelation
{
    ...
    static inline QSqlError fetchAll(const QString& relation, const qx::QxSqlQuery& query, T& t, QSqlDatabase* pDatabase)
    {
        ...
        if (!relation.isEmpty())
        {
            lst = relation.split(QStringLiteral("|"));
        }
