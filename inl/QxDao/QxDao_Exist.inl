...
struct QxDao_Exist_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("exist"), new qx::QxSqlQueryBuilder_Exist<T>());
        if (!dao.isValid())
        {
        ...
        if (dao.isMongoDB())
        {
            long cnt = 0; QString json = qx::serialization::json::to_string(t, 1, QStringLiteral("mongodb:only_id")); qx_query query(json);
            qx::dao::mongodb::QxMongoDB_Helper::count((&dao), dao.getDataMemberX()->getClass(), cnt, (&query)); if (!dao.isValid())
            {
                return qx_bool(false);
            }
        ...
        if (!dao.prepare(sql))
        {
            dao.errFailed(true); return qx_bool(false);
        }
        ...
        if (!dao.exec(true))
        {
            dao.errFailed(); return qx_bool(false);
        }
...
struct QxDao_Exist_Container
{
    ...
    static qx_bool exist(T& t, QSqlDatabase* pDatabase)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("exist"), new qx::QxSqlQueryBuilder_Exist<type_item>());
        if (!dao.isValid())
        {
            return qx_bool(false);
        }
        ...
        if (sql.isEmpty())
        {
            dao.errEmpty(); return qx_bool(false);
        }
        if (!dao.prepare(sql))
        {
            dao.errFailed(true); return qx_bool(false);
...
private:
    ...
    struct existItem_Helper<U, false>
    {
        static bool exist(U& item, qx::dao::detail::QxDao_Helper_Container<T>& dao)
        {
            ...
            if (dao.isMongoDB())
            {
                long cnt = 0; QString json = qx::serialization::json::to_string(item, 1, QStringLiteral("mongodb:only_id")); qx_query query(json);
                qx::dao::mongodb::QxMongoDB_Helper::count((&dao), dao.getDataMemberX()->getClass(), cnt, (&query)); if (!dao.isValid())
                {
                    return false;
                }
