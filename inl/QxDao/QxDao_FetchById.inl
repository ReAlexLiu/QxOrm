...
struct QxDao_FetchById_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("fetch by id"), new qx::QxSqlQueryBuilder_FetchById<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
...
struct QxDao_FetchById_Container
{
    ...
    static QSqlError fetchById(T& t, QSqlDatabase* pDatabase, const QStringList& columns)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("fetch by id"), new qx::QxSqlQueryBuilder_FetchById<type_item>());
        if (!dao.isValid())
        {
            return dao.error();
        }
...
private:
    ...
    struct fetchItem_Helper<U, false>
    {
        static bool fetch(U& item, qx::dao::detail::QxDao_Helper_Container<T>& dao)
        {
            ...
            if (!dao.exec(true))
            {
                dao.errFailed(); return false;
            }
            if (!dao.nextRecord())
            {
                dao.errNoData(); return false;
            }
