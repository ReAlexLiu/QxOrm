...
struct QxDao_Insert_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("insert"), new qx::QxSqlQueryBuilder_Insert<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            qx::dao::on_before_insert<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            QString insertedId;
            ...
            qx::dao::on_after_insert<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            return dao.error();
...
struct QxDao_Insert_Container
{
    ...
    static QSqlError insert(T& t, QSqlDatabase* pDatabase)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("insert"), new qx::QxSqlQueryBuilder_Insert<type_item>());
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            ...
            qx::dao::mongodb::QxMongoDB_Helper::insertMany((&dao), dao.getDataMemberX()->getClass(), itemsAsJson, insertedId); if (!dao.isValid())
            {
                return dao.error();
            }
            dao.qxQuery().queryAt(2, QStringLiteral("<done>"));
            dao.itemsAsJson().clear(); dao.itemsAsJson().append(insertedId);
            for (typename T::iterator it = t.begin(); it != t.end(); ++it)
            {
                if (!insertItem((*it), dao))
                {
        ...
        static bool insert(U& item, qx::dao::detail::QxDao_Helper_Container<T>& dao)
        {
            ...
            if (dao.isMongoDB())
            {
                if (dao.qxQuery().queryAt(2) == QStringLiteral("<done>"))
                {
                    ...
                    {
                        QString oid = dao.itemsAsJson().takeFirst(); if (!oid.isEmpty() && dao.getDataId())
                        {
                ...
                qx::dao::on_before_insert<U>((&item), (&dao)); if (!dao.isValid())
                {
                    return false;
                }
                dao.itemsAsJson().append(qx::serialization::json::to_string(item, 1, QStringLiteral("mongodb")));
                return dao.isValid();
            }
        #endif // _QX_ENABLE_MONGODB
            ...
            qx::dao::on_after_insert<U>((&item), (&dao)); if (!dao.isValid())
            {
                return false;
            }
