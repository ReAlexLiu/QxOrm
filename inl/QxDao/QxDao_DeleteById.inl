...
struct QxDao_DeleteById_Generic
{
    ...
    static QSqlError deleteById(T& t, QSqlDatabase* pDatabase, bool bVerifySoftDelete)
    {
        ...
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("delete by id"), pBuilder);
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            qx::dao::on_before_delete<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            qx::dao::mongodb::QxMongoDB_Helper::deleteOne((&dao), dao.getDataMemberX()->getClass(), qx::serialization::json::to_string(t, 1, QStringLiteral("mongodb:only_id")), NULL); if (!dao.isValid())
            {
                return dao.error();
            }
            qx::dao::on_after_delete<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            return dao.error();
...
struct QxDao_DeleteById_Container
{
    ...
    static QSqlError deleteById(T& t, QSqlDatabase* pDatabase, bool bVerifySoftDelete)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("delete by id"), pBuilder);
        if (!dao.isValid())
        {
            return dao.error();
        }
    ...
    struct deleteItem_Helper<U, false>
    {
        static bool deleteById(U& item, qx::dao::detail::QxDao_Helper_Container<T>& dao)
        {
            ...
            if (dao.isMongoDB())
            {
                ...
                {
                    qx::dao::on_after_delete<U>((&item), (&dao)); return dao.isValid();
                }
                qx::dao::on_before_delete<U>((&item), (&dao)); if (!dao.isValid())
                {
                    return false;
                }
            ...
            qx::dao::on_after_delete<U>((&item), (&dao)); if (!dao.isValid())
            {
                return false;
            }
