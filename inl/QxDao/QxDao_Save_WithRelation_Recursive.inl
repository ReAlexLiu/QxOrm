...
struct QxDao_Save_WithRelation_Recursive_Generic
{
    ...
    {
        QStringList relation(QStringLiteral("*"));
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("save with relation recursive"), new qx::QxSqlQueryBuilder_Update<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (!pRelationParams)
        {
            params.setDatabase((&dao.database())); params.setSaveMode(eSaveMode); params.setRecursiveMode(true);
        }
        else
        {
...
struct QxDao_Save_WithRelation_Recursive_Container
{
    ...
    static QSqlError save(T& t, qx::dao::save_mode::e_save_mode eSaveMode, QSqlDatabase* pDatabase, qx::QxSqlRelationParams* pRelationParams)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("save with relation recursive"), new qx::QxSqlQueryBuilder_Update<type_item>());
        if (!dao.isValid())
        {
            return dao.error();
        }
...
private:
    ...
    struct saveItem_Helper<U, false>
    {
        static bool save(qx::dao::save_mode::e_save_mode eSaveMode, qx::QxSqlRelationParams* pRelationParams, U& item, qx::dao::detail::QxDao_Helper_Container<T>& dao)
        {
            ...
            {
                params = (*pRelationParams); params.setOwner(&item);
            }
