...
struct QxDao_FetchAll_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("fetch all"), new qx::QxSqlQueryBuilder_FetchAll<T>(), (&query));
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            ...
            qx::dao::mongodb::QxMongoDB_Helper::findOne((&dao), dao.getDataMemberX()->getClass(), json, (&query)); if (!dao.isValid())
            {
                return dao.error();
            }

            qx::dao::detail::IxDao_Timer timer((&dao), qx::dao::detail::IxDao_Helper::timer_cpp_build_instance);
            QString ctx = QStringLiteral("mongodb") + ((columns.count() > 0) ? QStringLiteral(":columns{,%1,}").arg(columns.join(QStringLiteral(",")) : QString());
...
struct QxDao_FetchAll_MongoDB_Fetcher : public qx::dao::mongodb::QxMongoDB_Fetcher
{
    ...
    virtual void fetch(const QString& json)
    {
        ...
        qx::dao::on_before_fetch<type_value_qx>((&item_val), (&m_dao)); if (!m_dao.isValid())
        {
            return;
        }
        QString ctx = QStringLiteral("mongodb") + ((columns.count() > 0) ? QStringLiteral(":columns{,%1,}").arg(columns.join(QStringLiteral(",")) : QString());
        ...
        qx::dao::on_after_fetch<type_value_qx>((&item_val), (&m_dao)); if (!m_dao.isValid())
        {
            return;
        }
...
struct QxDao_FetchAll_Container
{
    ...
    static QSqlError fetchAll(const qx::QxSqlQuery& query, T& t, QSqlDatabase* pDatabase, const QStringList& columns)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("fetch all"), new qx::QxSqlQueryBuilder_FetchAll<type_item>(), (&query));
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        {
            insertNewItem(t, dao); if (!dao.isValid())
            {
                return dao.error();
            }
        }
