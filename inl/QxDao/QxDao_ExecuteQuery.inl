...
struct QxDao_ExecuteQuery_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("execute custom sql query or stored procedure"), new qx::QxSqlQueryBuilder_Count<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            ...
            qx::dao::mongodb::QxMongoDB_Helper::executeCommand((&dao), dao.getDataMemberX()->getClass(), (&query)); if (!dao.isValid())
            {
                return dao.error();
            }
            QString json = query.response().toString(); qx::serialization::json::from_string(t, json, 1, QStringLiteral("mongodb"));
        ...
        if (!pDataMemberX)
        {
            qAssert(false); return dao.error();
        }
...
struct QxDao_ExecuteQuery_Container
{
    ...
    static QSqlError executeQuery(qx::QxSqlQuery& query, T& t, QSqlDatabase* pDatabase)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("execute custom sql query or stored procedure"), pBuilder);
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        {
            insertNewItem(t, dao, vColumnToFetch); if (!dao.isValid())
            {
                return dao.error();
            }
        }
...
private:
    ...
    static void insertNewItem(T& t, qx::dao::detail::QxDao_Helper_Container<T>& dao, QVector< QPair<int, qx::IxDataMember*> >& vColumnToFetch)
    {
        ...
        qx::cvt::from_variant(vId, item.key(), QStringLiteral(""), -1, qx::cvt::context::e_database);
