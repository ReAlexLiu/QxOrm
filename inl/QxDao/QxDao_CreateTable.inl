...
struct QxDao_CreateTable
{
    ...
    {
        T t; Q_UNUSED(t);
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("create table"), new qx::QxSqlQueryBuilder_CreateTable<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }

        if (dao.database().driverName() != QStringLiteral("QSQLITE"))
        {
            QString sWarningMsg = QStringLiteral("-- WARNING -- the function qx::dao::create_table<T>() can be used only with a SQLite database to create examples or prototypes, for other databases, it is recommended :");
            sWarningMsg += QStringLiteral("\n\t - to use QxEntityEditor application and its DDL SQL database schema export plugin ;");
        ...
        while ((pRelation = dao.builder().nextRelation(index)))
        {
            ...
            if (!bExtraTable)
            {
                dao.updateError(queryCreateTable.lastError()); break;
            }
        }
