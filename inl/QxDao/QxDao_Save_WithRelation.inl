...
struct QxDao_Save_WithRelation_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("save with relation"), new qx::QxSqlQueryBuilder_Update<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
...
struct QxDao_Save_WithRelation_Container
{
    ...
    static QSqlError save(const QStringList& relation, T& t, QSqlDatabase* pDatabase)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("save with relation"), new qx::QxSqlQueryBuilder_Update<type_item>());
        if (!dao.isValid())
        {
            return dao.error();
        }
