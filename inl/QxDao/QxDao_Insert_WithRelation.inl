...
struct QxDao_Insert_WithRelation_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("insert with relation"), new qx::QxSqlQueryBuilder_Insert<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
...
struct QxDao_Insert_WithRelation_Container
{
    ...
    static QSqlError insert(const QStringList& relation, T& t, QSqlDatabase* pDatabase)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("insert with relation"), new qx::QxSqlQueryBuilder_Insert<type_item>());
        if (!dao.isValid())
        {
            return dao.error();
        }
