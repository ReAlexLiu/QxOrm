...
struct QxDao_Count
{
    ...
    {
        T t; Q_UNUSED(t);
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("count"), new qx::QxSqlQueryBuilder_Count<T>(), (&query));
        if (!dao.isValid())
        {
            return 0;
        }
...
struct QxDao_Count_WithRelation
{
    ...
    static QSqlError count(long& lCount, const QStringList& relation, const qx::QxSqlQuery& query, QSqlDatabase* pDatabase)
    {
        ...
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("count with relation"), new qx::QxSqlQueryBuilder_Count_WithRelation<T>(), (&query));
        if (!dao.isValid())
        {
            return dao.error();
        }
