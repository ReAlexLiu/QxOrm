...
struct QxDao_FetchById_WithRelation_Generic
{
    ...
    {
        type_dao_helper dao(t, pDatabase, QStringLiteral("fetch by id with relation"), new qx::QxSqlQueryBuilder_FetchById_WithRelation<T>());
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            ...
            qx::dao::mongodb::QxMongoDB_Helper::findOne((&dao), dao.getDataMemberX()->getClass(), json, NULL); if (!dao.isValid())
            {
                return dao.error();
            }
...
struct QxDao_FetchById_WithRelation_Container
{
    ...
    static QSqlError fetchById(const QStringList& relation, T& t, QSqlDatabase* pDatabase)
    {
        ...
        type_dao_helper dao(t, pDatabase, QStringLiteral("fetch by id with relation"), new qx::QxSqlQueryBuilder_FetchById_WithRelation<type_value_qx>());
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            ...
            qx::dao::mongodb::QxMongoDB_Helper::findMany((&dao), dao.getDataMemberX()->getClass(), itemsAsJson, NULL, NULL); if (!dao.isValid())
            {
                return dao.error();
            }
            dao.qxQuery().queryAt(2, QStringLiteral("<done>"));
            for (typename T::iterator it = t.begin(); it != t.end(); ++it)
            {
                if (!fetchItem((*it), dao))
                {
        ...
        static bool fetch(U& item, type_dao_helper& dao)
        {
            ...
            if (dao.isMongoDB())
            {
                if (dao.qxQuery().queryAt(2) == QStringLiteral("<done>"))
                {
                    ...
                    {
                        QString json = dao.itemsAsJson().takeFirst(); if (!json.isEmpty())
                        {
                            qx::serialization::json::from_string(item, json, 1, QStringLiteral("mongodb"));
                        }
                    }
                ...
                {
                    qx::dao::detail::IxDao_Timer timer((&dao), qx::dao::detail::IxDao_Helper::timer_cpp_build_instance);
                    qx::dao::on_before_fetch<U>((&item), (&dao)); if (!dao.isValid())
                    {
                        return false;
                    }
