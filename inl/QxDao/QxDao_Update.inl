...
struct QxDao_Update_Generic
{
    ...
    {
        qx::dao::detail::QxDao_Helper<T> dao(t, pDatabase, QStringLiteral("update"), new qx::QxSqlQueryBuilder_Update<T>(), (&query));
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            qx::dao::on_before_update<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            QString ctx = QStringLiteral("mongodb") + ((columns.count() > 0) ? QStringLiteral(":columns{,%1,}").arg(columns.join(QStringLiteral(",")) : QString());
            qx::dao::mongodb::QxMongoDB_Helper::updateOne((&dao), dao.getDataMemberX()->getClass(), qx::serialization::json::to_string(t, 1, ctx), (&query)); if (!dao.isValid())
            {
                return dao.error();
            }
            qx::dao::on_after_update<T>((&t), (&dao)); if (!dao.isValid())
            {
                return dao.error();
            }
            return dao.error();
        ...
        if (!query.isEmpty())
        {
            dao.addQuery(false); sql = dao.builder().getSqlQuery();
        }
        if (!pDatabase)
        {
            dao.transaction();
...
struct QxDao_Update_Container
{
    ...
    static QSqlError update(const qx::QxSqlQuery& query, T& t, QSqlDatabase* pDatabase, const QStringList& columns)
    {
        ...
        qx::dao::detail::QxDao_Helper_Container<T> dao(t, pDatabase, QStringLiteral("update"), new qx::QxSqlQueryBuilder_Update<type_item>(), (&query));
        if (!dao.isValid())
        {
            return dao.error();
        }
        ...
        if (dao.isMongoDB())
        {
            ...
            qx::dao::mongodb::QxMongoDB_Helper::updateMany((&dao), dao.getDataMemberX()->getClass(), itemsAsJson, (&query)); if (!dao.isValid())
            {
                return dao.error();
            }
            dao.qxQuery().queryAt(2, QStringLiteral("<done>")); dao.itemsAsJson().clear();
            for (typename T::iterator it = t.begin(); it != t.end(); ++it)
            {
                if (!updateItem((*it), dao))
                {
...
private:
    ...
    struct updateItem_Helper<U, false>
    {
        static bool update(U& item, qx::dao::detail::QxDao_Helper_Container<T>& dao)
        {
            ...
            if (dao.isMongoDB())
            {
                if (dao.qxQuery().queryAt(2) == QStringLiteral("<done>"))
                {
                    qx::dao::on_after_update<U>((&item), (&dao)); return dao.isValid();
                }
                QString ctx = QStringLiteral("mongodb") + ((columns.count() > 0) ? QStringLiteral(":columns{,%1,}").arg(columns.join(QStringLiteral(",")) : QString());
                qx::dao::on_before_update<U>((&item), (&dao)); if (!dao.isValid())
                {
                    return false;
                }
            ...
            if (!dao.qxQuery().isEmpty())
            {
                dao.qxQuery().resolve(dao.query());
            }
            if (!dao.exec(true))
            {
                dao.errFailed();
            ...
            qx::dao::on_after_update<U>((&item), (&dao)); if (!dao.isValid())
            {
                return false;
            }
