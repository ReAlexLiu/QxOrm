...
IxDataMember* QxDataMemberX<T>::initId(IxDataMember* pId, long lVersion)
{
    ...
    pId->setSqlType(QLatin1String(qx::trait::get_sql_type<typename QxDataMemberX<T>::type_primary_key>::get()));
...
IxDataMember* QxDataMemberX<T>::add(V U::* pData, const QString& sKey, long lVersion /* = 0 */, bool bSerialize /* = true */, bool bDao /* = true */)
{
    ...
    pNewDataMember->setSqlType(QLatin1String(qx::trait::get_sql_type<V>::get()));
...
IxDataMember* QxDataMemberX<T>::add(const QString& sKey, long lVersion)
{
    ...
    {
        xwarn("Qt introspection engine works only with QObject class");
        qAssert(false);
        return NULL;
    }
    if (exist_WithDaoStrategy(sKey))
...
IxDataMember* QxDataMemberX<T>::id(typename QxDataMemberX<T>::type_primary_key T::* pDataMemberId, const QString& sKey, long lVersion /* = 0 */)
{
    ...
    {
        xwarn("id (primary key) already defined '{}'", qPrintable(getId_WithDaoStrategy()->getName()));
    }
...
IxDataMember* QxDataMemberX<T>::id(const QString& sKey, long lVersion)
{
    ...
    if (getId_WithDaoStrategy())
    {
        xwarn("id (primary key) already defined '%s'", qPrintable(getId_WithDaoStrategy()->getName()));
    }
    if (exist_WithDaoStrategy(sKey) || getId_WithDaoStrategy())
    {
        qAssert(false);
...
QxDataMemberX<qx::trait::no_base_class_defined>::QxDataMemberX() : IxDataMemberX(), QxSingleton< QxDataMemberX<qx::trait::no_base_class_defined> >(QStringLiteral("qx::QxDataMemberX_no_base_class_defined"))
{
    ;
}
